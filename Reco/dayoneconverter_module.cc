...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      int makepatrectrack(std::vector<gar::rec::TPCCluster>& trackTPCClusters,
                          gar::rec::TrackPar& trackpar);
      void digitizeCaloHitsSimple(gar::sdp::CaloDeposit cd,
                                  float* fcpos,
                                  float& energy,
                                  float& time);
    ...
    // More initializers here.
    {
      ...
      if (fUseOnlyTrueMuonHits) { consumes<std::vector<simb::MCParticle>>(fG4ModuleLabel); }
    ...
    void dayoneconverter::produce(art::Event& e)
    {
      ...
      if (true) {
        ...
        if (ntpcclus != 0 && debug)
          std::cout << "Time start and end: " << TPCClusterCol->at(0).Time() << " "
                    << TPCClusterCol->at(TPCClusterCol->size() - 1).Time() << std::endl;
        ...
        if (ntpcclus != 0) {
          ...
          if (TPCClusterColTimeBlock.size() != 0)
            TPCClusterColTime.push_back(TPCClusterColTimeBlock);
        }
        ...
        for (size_t t = 0; t < TPCClusterColTime.size(); t++) {
          ...
          std::sort(TPCClusterColTime.at(t).begin(),
                    TPCClusterColTime.at(t).end(),
                    [](const gar::rec::TPCCluster& a, const gar::rec::TPCCluster& b) -> bool {
          ...
          while (done == 0) {
                    ...
                    for (size_t k = j + 1; k < unusedTPCplanes.size(); ++k) {
                      for (size_t kt : unusedTPCplanes.at(k)) {
                        ...
                        for (size_t kt2 : unusedTPC) {
                          ...
                          if ((TMath::Abs(zcur - k2pos[2]) > fZCut2) && (tpcclusindexb > -1) &&
                              (dbest < fRCut)) {
                          ...
                          int retcode = util::TrackPropagator::DistXYZ(
                            tpt.TrackParBeg(), tpt.Vertex(), k2pos, dist);
                        ...
                        if (tpcclusindex.size() > bestnpts ||
                            ((tpcclusindex.size() == bestnpts) && (sumr2 < bestsumr2))) {
            ...
            if (debug) {
              ...
              for (size_t j = 0; j < unusedTPCplanes.size(); j++) {
                std::cout << "unusedTPCplane" << j << " : " << unusedTPCplanes.at(j).size()
                          << std::endl;
              }
        ...
        if (debug)
          std::cout << "Found this many tracks: " << trkCol->size() << std::endl
                    << std::endl
                    << std::endl;
      }
    ...
    void dayoneconverter::digitizeCaloHitsSimple(gar::sdp::CaloDeposit cd,
                                                 float* fcpos,
                                                 float& energy,
                                                 float& time)
    {
    ...
    void dayoneconverter::digitizeCaloHitsMu2e(gar::sdp::CaloDeposit cd,
                                               float* fcpos,
                                               float& energy,
                                               float& time)
    {
    ...
    int dayoneconverter::makepatrectrack(std::vector<gar::rec::TPCCluster>& trackTPCClusters,
                                         gar::rec::TrackPar& trackpar)
    {
