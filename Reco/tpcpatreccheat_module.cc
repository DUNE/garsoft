...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      float
        fSortTransWeight; ///< for use in the hit sorting algorithm -- transverse distance weight factor
      float
        fSortDistBack; ///< for use in the hit sorting algorithm -- how far to go back before raising the distance figure of merit
    ...
    void tpcpatreccheat::produce(art::Event& e)
    {
      ...
      const art::FindManyP<gar::rec::Hit> HitsFromTPCClusters(
        TPCClusterHandle, e, fTPCClusterLabel);
      ...
      for (int iside = -1; iside <= 1; iside += 2) {
        ...
        for (size_t iclus = 0; iclus < TPCClusters.size(); ++iclus) {
          ...
          for (size_t ihit = 0; ihit < hits.size(); ++ihit) {
            ...
            int hitside = 1;
            if (chanpos[0] < tpccent.X()) { hitside = -1; }
            ...
            for (size_t i = 0; i < hitides.size(); ++i) {
              ...
              if (cmf == chargemap.end()) { chargemap[trackid] = charge; }
              else {
                cmf->second += charge;
              }
            }
        ...
        for (auto const& tic : trackIDclusmap) {
          ...
          if (TPCClusters_thistrack.size() >= fMinNumTPCClusters) {
            ...
            if (makepatrectrack(TPCClusters_thistrack, trackpar) == 0) {
              ...
              for (size_t iTPCCluster = 0; iTPCCluster < TPCClusters_thistrack.size();
                   ++iTPCCluster) {
    ...
    int tpcpatreccheat::makepatrectrack(std::vector<gar::rec::TPCCluster>& trackTPCClusters,
                                        gar::rec::TrackPar& trackpar)
    {
