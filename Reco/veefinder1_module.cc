...
namespace gar {
  namespace rec {
    ...
    private:
      size_t
        fNTPCClusPerTrack; ///< min number of TPC clusters for a track to be identified as part of a vee
    ...
    {
      std::unique_ptr<std::vector<rec::Vee>> veeCol(new std::vector<rec::Vee>);
          ...
          for (int iEnd = 0; iEnd < 2; ++iEnd) {
            ...
            for (size_t jTrack = iTrack + 1; jTrack < nTrack; ++jTrack) {
              ...
              for (int jEnd = 0; jEnd < 2; ++jEnd) {
                ...
                if (fPrintLevel > 1)
                  std::cout << "veefinder: passed the endpoint distance cut" << std::endl;
                if (fPrintLevel > 1)
                  std::cout << "veefinder: opening angle: " << thisP.Angle(thatP) << std::endl;
                float openang = thisP.Angle(thatP);
                if (openang < fOpenAngMinCut) continue;
                if (openang > fOpenAngMaxCut) continue;
                if (fPrintLevel > 1)
                  std::cout << "veefinder: passed the opening angle cut" << std::endl;
                if (fRequireOppositeCharge) {
                  if (thisCharge == 0 || thatCharge == 0) {
                    if (fPrintLevel > 1) {
                      std::cout << "veefinder: One of the track charges is zero: " << thisCharge
                                << " " << thatCharge << std::endl;
                    }
                  }
                  if (thisCharge == thatCharge) continue;
                  if (fPrintLevel > 1)
                    std::cout << "veefinder: passed the opposite-charge cut" << std::endl;
                }
                ...
                float energy_kshort_hyp =
                  TMath::Sqrt(thisP.Mag2() + m_pi * m_pi) + TMath::Sqrt(thatP.Mag2() + m_pi * m_pi);
                ...
                float energy_lambda1_hyp = TMath::Sqrt(thisP.Mag2() + m_prot * m_prot) +
                                           TMath::Sqrt(thatP.Mag2() + m_pi * m_pi);
                float energy_lambda2_hyp = TMath::Sqrt(thisP.Mag2() + m_pi * m_pi) +
                ...
                veeCol->emplace_back(
                  vpos.data(), covmatvec, chisquared, time, fourmomentumvec.data());
                              ...
                              std::vector<float>& dca)
    {
      ...
      if (tracks.size() > 0) { time /= tracks.size(); }
      ...
      for (int e = 0; e < 3; e++)
        if (A(e, e) == 0) checkdiagonal = false;
    ...
    {
      if (trk.NHits() <
          fNTPCClusPerTrack) // historically-named NHits gets the number of TPC clusters
      {
        return false;
      }
      if (usebeg == TrackEndBeg) {
        if (trk.Momentum_beg() < fPCut) { return false; }
      }
      else {
        if (trk.Momentum_end() < fPCut) { return false; }
      }
    ...
    {
      if (trk.NHits() <
          fNearTrackNTPCClus) // historically-named NHits gets the number of TPC clusters
      {
        return false;
      }
      TVector3 tep(0, 0, 0);
      if (usebeg == TrackEndBeg) { tep.SetXYZ(trk.Vertex()[0], trk.Vertex()[1], trk.Vertex()[2]); }
      else {
        tep.SetXYZ(trk.End()[0], trk.End()[1], trk.End()[2]);
      }
      TVector3 dv = tep - veepos;
      float dve = dv.Mag();
      if (dve < fNearTrackDMinCut || dve > fNearTrackDMaxCut) { return false; }
      if (dv.Angle(veep) > fNearTrackAngCut) { return false; }
      return true;
    }
