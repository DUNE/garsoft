...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      std::string
        fInputTrackLabel; ///< input tracks and associations with TPCClusters.  Get vertices associated with these tracks from the assns
      ...
      bool cathodematch(const gar::rec::Track& trackA,
    ...
    void tpccathodestitch::produce(art::Event& e)
    {
      ...
      double DriftVelocity =
        detProp->DriftVelocity(detProp->Efield(), detProp->Temperature()); // in cm per microsecond
      ...
      std::unique_ptr<std::vector<gar::rec::TPCCluster>> TPCClusterCol(
        new std::vector<gar::rec::TPCCluster>);
      std::unique_ptr<art::Assns<rec::TPCCluster, rec::Track>> TPCClusterTrkAssns(
        new art::Assns<rec::TPCCluster, rec::Track>);
      ...
      std::unique_ptr<art::Assns<rec::Track, rec::Vertex, rec::TrackEnd>> trkVtxAssns(
        new art::Assns<rec::Track, rec::Vertex, rec::TrackEnd>);
      std::unique_ptr<art::Assns<rec::Hit, rec::TPCCluster>> HitTPCClusterAssns(
        new art::Assns<rec::Hit, rec::TPCCluster>);

      // inputs

      auto inputTrackHandle = e.getValidHandle<std::vector<rec::Track>>(fInputTrackLabel);
      auto const& inputTracks = *inputTrackHandle;
      auto inputTPCClusterHandle =
        e.getValidHandle<std::vector<rec::TPCCluster>>(fInputTPCClusterLabel);
      auto const& inputTPCClusters = *inputTPCClusterHandle;

      const art::FindManyP<rec::TPCCluster> TPCClustersFromInputTracks(
        inputTrackHandle, e, fInputTrackLabel);
      const art::FindManyP<rec::TrackIoniz> TrackIonizFromInputTracks(
        inputTrackHandle, e, fInputTrackLabel);
      const art::FindManyP<rec::Vertex, rec::TrackEnd> VerticesFromInputTracks(
        inputTrackHandle, e, fInputVertexLabel);
      const art::FindManyP<rec::Hit> HitsFromInputTPCClusters(
        inputTPCClusterHandle, e, fInputTPCClusterLabel);
      ...
      for (size_t itrack = 0; itrack < inputTracks.size(); ++itrack) {
        ...
        if (uniqueID == inputTracks.at(itrack).getIDNumber()) { Tracks_toskip.push_back(true); }
        else {
          Tracks_toskip.push_back(false);
          uniqueID = inputTracks.at(itrack).getIDNumber();
        }
      ...
      for (size_t itrack = 0; itrack < inputTracks.size(); ++itrack) {
        ...
        size_t nplus = 0;
        size_t nminus = 0;
        for (size_t iTPCCluster = 0; iTPCCluster < TPCClustersFromInputTracks.at(itrack).size();
             ++iTPCCluster) {
          size_t icindex =
            hcim[TPCClustersFromInputTracks.at(itrack).at(iTPCCluster)->getIDNumber()];
          for (size_t ihit = 0; ihit < HitsFromInputTPCClusters.at(icindex).size(); ++ihit) {
            ...
            if (chanpos[0] > xtpccent) { nplus++; }
            else {
              nminus++;
            }
          }
        }
        if (nplus > nminus) { trackside.push_back(1); }
        else {
      ...
      for (size_t itrack = 0; itrack < inputTracks.size(); ++itrack) {
        ...
        for (size_t jtrack = itrack + 1; jtrack < inputTracks.size(); ++jtrack) {
          ...
          if (trackside.at(itrack) == trackside.at(jtrack))
            continue; // don't merge tracks on the same side
          if (cathodematch(inputTracks.at(itrack),
                           inputTracks.at(jtrack),
                           fwdflag.at(itrack),
                           fwdflag.at(jtrack),
                           dx.at(itrack))) {
            if (fPrintLevel > 0) {
              std::cout << "found a merge.  dx= " << dx.at(itrack)
                        << " flip flags: " << fwdflag.at(itrack) << " " << fwdflag.at(jtrack)
                        << std::endl;
            }
      ...
      while (moretodo) {
        ...
        for (const auto& vtxi : vttrackmap) {
          ...
          if (numdx > 0) {
            ...
            moretodo =
              true; // we'll shift its tracks and thus need to go around looking for more vertices
          }
          ...
          for (size_t i = 0; i < vtxi.second.size(); ++i) {
            size_t itrack = vtxi.second.at(i).trkindex;
            if (mergedflag.at(itrack) < 0 && dx.at(itrack) == 0) { dx.at(itrack) = avgdx; }
          }
        }
      }
      ...
      for (size_t itrack = 0; itrack < inputTracks.size(); ++itrack) {
        ...
        if (mergedflag.at(itrack) < 0) // not merged, but make a new shifted track if need be
        {
          ...
          for (size_t iTPCCluster = 0; iTPCCluster < TPCClustersFromInputTracks.at(itrack).size();
            ...
            for (size_t ihit = 0; ihit < HitsFromInputTPCClusters.at(icindex).size(); ++ihit) {
              HitTPCClusterAssns->addSingle(HitsFromInputTPCClusters.at(icindex).at(ihit),
                                            TPCClusterPointer);
            }
          }
          ...
          for (size_t iTrkIoniz = 0; iTrkIoniz < TrackIonizFromInputTracks.at(itrack).size();
               ++iTrkIoniz) {
        ...
        else // merge them -- adjust track parameters and put in the time
        {
          ...
          if (mergedflag.at(itrack) < 0) {
            MF_LOG_WARNING("tpccathodestitch: inconsistent merge flags ")
              << itrack << " " << mergedflag.at(itrack);
            continue;
          }
          ...
          for (size_t iTPCCluster = 0; iTPCCluster < TPCClustersFromInputTracks.at(jtrack).size();
               ++iTPCCluster) {
          ...
          for (size_t ivtx = 0; ivtx < VerticesFromInputTracks.at(itrack).size(); ++ivtx) {
            auto vtxp = VerticesFromInputTracks.at(itrack).at(ivtx);
            auto trkend = *(VerticesFromInputTracks.data(itrack).at(ivtx));
            if (fwdflag.at(itrack) != 1) { trkend = 1 - trkend; }
            rec::IDNumber vtxid = vtxp->getIDNumber();
            size_t ivout = vtxoutmap[vtxid];
            auto const vtxptr = vtxPtrMaker(ivout);
            trkVtxAssns->addSingle(trackpointer, vtxptr, trkend);
    ...
    bool tpccathodestitch::cathodematch(const rec::Track& atrack,
                                        const rec::Track& btrack,
                                        int& afw,
                                        int& bfw,
                                        float& deltaX)
    {
