...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      gar::rec::Cluster CreateCluster(const simb::MCParticle* part,
                                      std::vector<art::Ptr<gar::rec::CaloHit>> hitVec);
    };

    //-----------------------------------------------------------------------------------

    CaloClusterCheater::CaloClusterCheater(fhicl::ParameterSet const& pset) : EDProducer{pset}
    {
    ...
    void CaloClusterCheater::produce(art::Event& e)
    {
      ...
      std::unique_ptr<art::Assns<gar::rec::Cluster, gar::rec::CaloHit>> assn(
      ...
      for (auto hitMapItr : eveCaloHitMap) {
        ...
        evgb::util::CreateAssn(
          *this, e, *clustercol, hitMapItr.second, *assn, clustercol->size() - 1);
    ...
    //-----------------------------------------------------------------------------------
    void CaloClusterCheater::CollectHits(const art::Event& evt,
                                         const std::string& label,
                                         const std::string& instance,
                                         std::vector<art::Ptr<gar::rec::CaloHit>>& hitVector)
    {
    ...
    //-----------------------------------------------------------------------------------
    gar::rec::Cluster CaloClusterCheater::CreateCluster(
