          ...
          for (int fin = 0; fin < fEndsToCheck; ++fin) {
            ...
            for (size_t jTrack = iTrack + 1; jTrack < nTrack; ++jTrack) {
              ...
              for (int fin2 = 0; fin2 < fEndsToCheck; ++fin2) {
                ...
                if (thatTrackEnd == TrackEndBeg) { thatEndinSpace = thatTrack.getXYZBeg(); }
                else {
                ...
                if (gar::rec::fitVertex(vtracks, xyz, chisquared, covmat, time, usebeg, doca) ==
                    0) {
                  if (doca.at(0) < fDCut && doca.at(1) < fDCut) {
                    if (thatTrackEnd == TrackEndBeg) { usedflag_beg.at(jTrack) = 1; }
                    else {
