...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      float
        fClusterDirNhitCut; ///< Do not cut on cluster direction unless you have this many hits or more
    ...
    void TPCECALAssociation::beginJob()
    {
      ...
      if (fVerbosity > 0) {
        ...
        radClusTrack =
          tfs->make<TH1F>("radClusTrack", "(z,y) distance from cluster to track", 100, 0.0, 60.0);
        xClusTrack =
          tfs->make<TH2F>("xClusTrack",
                          ...
                          -200.0,
                          +200.0);
        phiClusTrack = tfs->make<TH1F>(
          "phiClusTrack", "angular distance from cluster to track in endcap", 90, -M_PI, +M_PI);
        rPhiClusTrack = tfs->make<TH1F>(
          "rPhiClusTrack", "distance from cluster to track in endcap", 100, -100.0, +100.0);
        dotClustTrack = tfs->make<TH2F>(
          "dotClustTrack",
          ...
          100.5);
        nEntryVsCut =
          tfs->make<TH1F>("nEntryVsCut",
                          "Number track end & cluster combos vs sequentially applied cut",
                          36,
                          -0.5,
                          35.5);
      }
      ...
      for (size_t iCluster = 0; iCluster < ClusterHandle->size(); ++iCluster) {
        ...
        for (size_t iTrack = 0; iTrack < TrackHandle->size(); ++iTrack) {
          ...
          if (abs(track.Vertex()[0] - ItsInTulsa[0]) > fTrackEndXCut ||
              std::hypot(track.Vertex()[1] - ItsInTulsa[1], track.Vertex()[2] - ItsInTulsa[2]) >
                fTrackEndRCut) {
            outside[TrackEndBeg] = true;
          }
          if (abs(track.End()[0] - ItsInTulsa[0]) > fTrackEndXCut ||
              std::hypot(track.End()[1] - ItsInTulsa[1], track.End()[2] - ItsInTulsa[2]) >
                fTrackEndRCut) {
          ...
          for (gar::rec::TrackEnd iEnd = TrackEndBeg; iEnd >= TrackEndEnd; --iEnd) {
            ...
            if (outside[iEnd]) {
              ...
              if (inBarrel) {
                ...
                if (fInstanceName.compare("MuID") == 0) {
                  looneyExtrap =
                    !fGeo->PointInMuIDBarrel(trackXYZ) && !fGeo->PointInMuIDEndcap(trackXYZ);
                }
                ...
                if (abs(cutQuantity) > maxXdisplacement / 2.0 + fBarrelXCut) { continue; }
                ...
                float radiansInDrift = trackPar[2] * maxXdisplacement / tan(trackPar[4]);
                ...
                int errcode =
                  util::TrackPropagator::PropagateToX(trackPar, trackEnd, xClus, retXYZ1);
                if (errcode != 0) {
                  // This track-cluster match fails.
                  continue;
                }
                ...
                if (abs(extrapRphiErr) > fEndcapRphiCut) { continue; }
