...
namespace gar {
  namespace rec {
    ...
    protected:
      void CollectDigiHits(const art::Event& evt,
      ...
      std::array<double, 3U> CalculateStripHitPosition(float x,
                                                       float y,
                                                       float z,
                                                       std::pair<float, float> hitTime,
                                                       raw::CellID_t cID);

      float CorrectStripHitTime(float x,
                                float y,
                                float z,
                                std::pair<float, float> hitTime,
                                raw::CellID_t cID);
    ...
    private:
      ...
      float
        fMIPThreshold; ///< zero-suppression threshold (in case the raw digits need to be zero-suppressed)
      ...
      std::unique_ptr<util::SiPMUtils>
        fSiPMUtils; ///< pointer to the Util fcn containing the SiPM desaturation function
    ...
    void SiPMHitFinder::produce(art::Event& e)
    {
      ...
      std::unique_ptr<art::Assns<rec::CaloHit, raw::CaloRawDigit>> RecoDigiHitsAssns(
        new art::Assns<rec::CaloHit, raw::CaloRawDigit>);
      ...
      for (std::vector<art::Ptr<gar::raw::CaloRawDigit>>::const_iterator
             iter = artDigiHits.begin(),
             iterEnd = artDigiHits.end();
           iter != iterEnd;
           ++iter) {
        ...
        if (hitMIP < fMIPThreshold) {
          MF_LOG_DEBUG("SiPMHitFinder")
            << "Signal under the " << fMIPThreshold << " MIP threshold" << std::endl;
          continue;
        }
        ...
        if (fGeo->isTile(point, cellID)) {
          ...
          if (fUseTimePositionReco) {
            std::array<double, 3> strip_pos =
              this->CalculateStripHitPosition(x, y, z, hitTime, cellID);
            pos[0] = strip_pos[0];
            pos[1] = strip_pos[1];
            pos[2] = strip_pos[2];
        ...
        rec::CaloHit hit(
          fSamplingCorrection * energy * CLHEP::MeV / CLHEP::GeV, time, pos, cellID, layer);
        hitCol->emplace_back(hit);

        MF_LOG_DEBUG("SiPMHitFinder")
          << "recohit " << &hit << " with cellID " << cellID << " has energy "
          << fSamplingCorrection * energy * CLHEP::MeV / CLHEP::GeV << " pos (" << pos[0] << ", "
          << pos[1] << ", " << pos[2] << ")";
    ...
    //----------------------------------------------------------------------------
    void SiPMHitFinder::CollectDigiHits(const art::Event& evt,
                                        const std::string& label,
                                        const std::string& instance,
                                        std::vector<art::Ptr<gar::raw::CaloRawDigit>>& hitVector)
    {
    ...
    float SiPMHitFinder::CalibrateToMIP(unsigned int ADC)
    {
      ...
      if (ADC <= 0) { return calibrated_ADC; }
    ...
    float SiPMHitFinder::CalibratetoMeV(float x, float y, float z, double MIP)
    {
      ...
      if (MIP <= 0) { return calibrated_MIP; }
    ...
    //----------------------------------------------------------------------------
    std::array<double, 3> SiPMHitFinder::CalculateStripHitPosition(float x,
                                                                   float y,
                                                                   float z,
                                                                   std::pair<float, float> hitTime,
                                                                   raw::CellID_t cID)
    {
      ...
      std::array<double, 3> local_back =
        fGeo->ReconstructStripHitPosition(point, pointLocal, xlocal, cID);
    ...
    //----------------------------------------------------------------------------
    float SiPMHitFinder::CorrectStripHitTime(float x,
                                             float y,
                                             float z,
                                             std::pair<float, float> hitTime,
                                             raw::CellID_t cID)
