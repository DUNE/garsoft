...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      float
        fTPCClusterResolRPhi; ///< uncertainty term used by the Kalman Filter for the RPhi coordinate
      ...
      float
        fSortTransWeight; ///< for use in hit sorting algorithm #1 -- transverse distance weight factor
      float
        fSortDistBack; ///< for use in hit sorting algorithm #1 -- how far to go back before raising the distance figure of merit
      ...
      double CalculateChi2(RVec<AliExternalTrackParam4D> ParamMC,
                           RVec<AliExternalTrackParam4D> ParamIn,
                           double xstart,
                           double ystart,
                           double* GArCenter);

      void CalculateTrackInfo(float& length,
                              std::vector<std::pair<float, float>>& dSigdX,
                              std::vector<TVector3>& trajpts,
                              std::vector<int> TPCClusterList,
                              std::vector<TPCCluster> TPCClusters,
                              RVec<AliExternalTrackParam4D> ParamIn,
                              double xstart,
                              double ystart,
                              double* GArCenter);
    ...
    ALICEKalmanTrackFit::ALICEKalmanTrackFit(fhicl::ParameterSet const& p) : EDProducer{p}
    {
      ...
      fTPCClusterResolRPhi =
        p.get<float>("TPCClusterResolRPhi",
                     0.4); // Ideally should be equal to typical residual but more work needed
    ...
    void ALICEKalmanTrackFit::produce(art::Event& e)
    {
      ...
      std::unique_ptr<art::Assns<gar::rec::TPCCluster, gar::rec::Track>> TPCClusterTrkAssns(
        new ::art::Assns<gar::rec::TPCCluster, gar::rec::Track>);
      std::unique_ptr<std::vector<rec::TrackIoniz>> ionCol(new std::vector<rec::TrackIoniz>);
      std::unique_ptr<art::Assns<rec::TrackIoniz, rec::Track>> ionTrkAssns(
        new ::art::Assns<rec::TrackIoniz, rec::Track>);
      std::unique_ptr<std::vector<rec::TrackTrajectory>> trajCol(
        new std::vector<rec::TrackTrajectory>);
      std::unique_ptr<art::Assns<rec::TrackTrajectory, rec::Track>> trajTrkAssns(
        new ::art::Assns<rec::TrackTrajectory, rec::Track>);
      ...
      const art::FindManyP<gar::rec::TPCCluster> TPCClustersFromPatRecTracks(
        patrecTrackHandle, e, fPatRecLabel);

      for (size_t itrack = 0; itrack < patrecTracks.size(); ++itrack) {
        std::vector<gar::rec::TPCCluster> TPCClusters;
        for (size_t iTPCCluster = 0; iTPCCluster < TPCClustersFromPatRecTracks.at(itrack).size();
             ++iTPCCluster) {
          TPCClusters.push_back(*TPCClustersFromPatRecTracks.at(itrack).at(
            iTPCCluster)); // make our own local copy of TPCClusters.  Maybe we can skip this?
        }
        ...
        if (ALICEKalmanFitBothWays(TPCClusters,
                                   trackparams,
                                   trackions,
                                   tracktrajs,
                                   PIDHypothesis,
                                   SortHypothesis,
                                   RecoSuccess) ==
            0) // to think about -- unused TPCClusters?  Or just ignore them in the fit?
        {
          ...
          for (size_t ireco = 0; ireco < trackparams.size(); ++ireco) {
            ...
            for (size_t iTPCCluster = 0; iTPCCluster < TPCClusters.size(); ++iTPCCluster) {
              TPCClusterTrkAssns->addSingle(TPCClustersFromPatRecTracks.at(itrack).at(iTPCCluster),
                                            trackpointer);
            }
    ...
    double ALICEKalmanTrackFit::CalculateChi2(RVec<AliExternalTrackParam4D> ParamMC,
                                              RVec<AliExternalTrackParam4D> ParamIn,
                                              double xstart,
                                              double ystart,
                                              double* GArCenter)
    {
      ...
      for (size_t t = 0; t < ParamIn.size(); ++t) {
        ...
        float IROC_OROC_boundary =
          (euclid->GetIROCOuterRadius() + euclid->GetOROCInnerRadius()) / 2.0;
    ...
    void ALICEKalmanTrackFit::CalculateTrackInfo(float& length,
                                                 ...
                                                 double* GArCenter)
    {
      for (size_t t = 0; t < ParamIn.size(); t++) {
        ...
        trajpts.emplace_back(
          xyz[2], xyz[1] + (GArCenter[1] - ystart), xyz[0] + (GArCenter[2] - xstart));
        ...
        if (t != 0) {
          ...
          if (xyz_prev[0] == 0 || xyz_prev[1] == 0 || xyz_prev[2] == 0) {
            xyz_prev[0] =
              TPCClusters[TPCClusterList[t - 1]].Position()[2] - (GArCenter[2] - xstart);
            xyz_prev[1] =
              TPCClusters[TPCClusterList[t - 1]].Position()[1] - (GArCenter[1] - ystart);
            xyz_prev[2] = TPCClusters[TPCClusterList[t - 1]].Position()[0];
          }

          float d_length =
            TMath::Sqrt(TMath::Sq(xyz[0] - xyz_prev[0]) + TMath::Sq(xyz[1] - xyz_prev[1]) +
                        TMath::Sq(xyz[2] - xyz_prev[2]));
          length += d_length;
...

    {
      ...
      float lbtmp = 0;
      if (fSortAlg == 1) {
        gar::rec::sort_TPCClusters_along_track(
          TPCClusters, hlf, hlb, fPrintLevel, lftmp, lbtmp, fSortTransWeight, fSortDistBack);
      }
      else if (fSortAlg == 2) {
        gar::rec::sort_TPCClusters_along_track2(
          TPCClusters, hlf, hlb, fPrintLevel, lftmp, lbtmp, fSortDistCut);
      }
      else {
        throw cet::exception("ALICEKalmanTrackFit_module")
          << "Sort Algorithm switch not understood: " << fSortAlg;
      }
      ...
      geom.fBz =
        -10 *
        magfield[0]; ///ALICE assumes magnetic field opposite directions and in dT rather that T
      ...
      for (size_t k = 0; k < hlb.size(); k++) {
        TVector3 pos(TPCClusters[hlb[k]].Position()[0],
                     TPCClusters[hlb[k]].Position()[1],
                     TPCClusters[hlb[k]].Position()[2]);
        TrkClusterXYZb_NDGAr.push_back(pos);
      }
      for (size_t k = 0; k < hlf.size(); k++) {
        TVector3 pos(TPCClusters[hlf[k]].Position()[0],
                     TPCClusters[hlf[k]].Position()[1],
                     TPCClusters[hlf[k]].Position()[2]);
      ...
      double ystart = 0;
      size_t size_dis = 30;
      if (int(TrkClusterXYZf_NDGAr.size() - 1) < 30)
        size_dis = int(TrkClusterXYZf_NDGAr.size() - 1);
      ...
      BuildParticlePoints(
        particle_f, GArCenter, geom, TrkClusterXYZf_NDGAr, PDGcode, xstart, ystart);
      ...
      if (int(TrkClusterXYZb_NDGAr.size() - 1) < 30)
        size_dis_b = int(TrkClusterXYZb_NDGAr.size() - 1);
      double displacex_b =
        (TrkClusterXYZb_NDGAr.at(size_dis_b).Z() - TrkClusterXYZb_NDGAr.at(0).Z());
      double displacey_b =
        (TrkClusterXYZb_NDGAr.at(size_dis_b).Y() - TrkClusterXYZb_NDGAr.at(0).Y());
      ...
      for (size_t h = 0; h < particle_h.size(); ++h) {
        ...
        if ((particle_h[h].fStatusMaskIn[0] & fastParticle::kTrackisOK) != fastParticle::kTrackisOK)
          paramSt = particle_h[h].fParamMC[0];
        ...
        if (cfrotb > 0)
          ...
          tparbeg[3] = TMath::Pi() - TMath::ASin(sfrotb);
        tparbeg[4] = TMath::ATan(paramSt.GetParameter()[3]);
        tparbeg[5] = xyz_start[2] + GArCenter[0];

        chisqbackwards =
          CalculateChi2(particle_h[h].fParamMC, particle_h[h].fParamIn, xinit, yinit, GArCenter);
        CalculateTrackInfo(lengthbackwards,
                           dSigdXs_BAK,
                           trajpts_BAK,
                           TPClist,
                           TPCClusters,
                           particle_h[h].fParamIn,
                           xinit,
                           yinit,
                           GArCenter);
        float mA[15];
        ...
        AliExternalTrackParam4D paramEnd =
          particle_h[h].fParamOut[particle_h[h].fParamOut.size() - 1];
        if ((particle_h[h].fStatusMaskOut[particle_h[h].fStatusMaskOut.size() - 1] &
             fastParticle::kTrackisOK) != fastParticle::kTrackisOK)
          paramEnd = particle_h[h].fParamMC[particle_h[h].fParamMC.size() - 1];
        ...
        if (cfrot)
          ...
          tparend[3] = TMath::Pi() - TMath::ASin(sfrot);
        tparend[4] = TMath::ATan(paramEnd.GetParameter()[3]);
        tparend[5] = xyz_end[2] + GArCenter[0];

        chisqforwards =
          CalculateChi2(particle_h[h].fParamMC, particle_h[h].fParamOut, xinit, yinit, GArCenter);
        CalculateTrackInfo(lengthforwards,
                           dSigdXs_FWD,
                           trajpts_FWD,
                           TPClist,
                           TPCClusters,
                           particle_h[h].fParamOut,
                           xinit,
                           yinit,
                           GArCenter);
        float mB[15];
        ...
        if (!(((particle_h[h].fStatusMaskIn[0] & fastParticle::kTrackisOK) ==
               fastParticle::kTrackisOK) ||
              ((particle_h[h].fStatusMaskOut[particle_h[h].fStatusMaskOut.size() - 1] &
                fastParticle::kTrackisOK) == fastParticle::kTrackisOK))) {
          success = false;
        } /// discard track if either side is unsuccessfull
