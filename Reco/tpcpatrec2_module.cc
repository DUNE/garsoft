...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      std::string
        fVecHitLabel;  ///< label of module to get the vector hits and associations with hits
      int fPrintLevel; ///< debug printout:  0: none, 1: selected, 2: all
      float
        fVecHitMatchCos; ///< matching condition for pairs of vector hits cos angle between directions
      ...
      float
        fVecHitMatchLambda; ///< matching condition for pairs of vector hits -- dLambda (radians)
      unsigned int
        fInitialTPNTPCClusters; ///< number of hits to use for initial trackpar estimate, if present
      ...
      float
        fConvAngleCut; ///< cut on angle diff for the conversion finder to split a cluster of VH's into two tracks

      // criteria for associating vector hits together to form clusters
      bool vhclusmatch(const std::vector<gar::rec::VecHit>& vechits,
    ...
    void tpcpatrec2::produce(art::Event& e)
    {
      ...
      std::unique_ptr<art::Assns<gar::rec::VecHit, gar::rec::Track>> vhTrkAssns(
        new ::art::Assns<gar::rec::VecHit, gar::rec::Track>);
      ...
      const art::FindManyP<gar::rec::TPCCluster> TPCClustersFromVecHits(
        vechitHandle, e, fVecHitLabel);
      ...
      for (size_t ivh = 0; ivh < vechits.size(); ++ivh) {
        if (fPrintLevel > 1) {
          std::cout << " vhprint " << vechits[ivh].Position()[0] << " "
                    << vechits[ivh].Position()[1] << " " << vechits[ivh].Position()[2] << " "
                    << vechits[ivh].Direction()[0] << " " << vechits[ivh].Direction()[1] << " "
                    << vechits[ivh].Direction()[2] << " " << std::endl;
        }

        std::vector<size_t> clusmatchlist;
        for (size_t iclus = 0; iclus < vhclusters.size(); ++iclus) {
          if (vhclusmatch(vechits, vhclusters[iclus], ivh)) { clusmatchlist.push_back(iclus); }
        }
      ...
      for (size_t iclus = 0; iclus < vhclusters.size(); ++iclus) {
        ...
        for (size_t ivh = 0; ivh < vhclusters[iclus].size(); ++ivh) {
          for (size_t iTPCCluster = 0;
               iTPCCluster < TPCClustersFromVecHits.at(vhclusters[iclus][ivh]).size();
               ++iTPCCluster) {
            TPCClusterptrs.push_back(
              TPCClustersFromVecHits.at(vhclusters[iclus][ivh]).at(iTPCCluster));
    ...
    bool tpcpatrec2::vhclusmatch(const std::vector<gar::rec::VecHit>& vechits,
                                 std::vector<size_t>& cluster,
                                 size_t vhindex)
    {
      ...
      for (size_t ivh = 0; ivh < cluster.size(); ++ivh) {
        ...
        if (fPrintLevel > 1) {
          std::cout << "Testing vh " << ivh << " in a cluster of size: " << cluster.size()
                    << std::endl;
        }
        ...
        if (((vhpos - vhtestpos).Cross(vhtestdir)).Mag() > fVecHitMatchPEX) {
          if (fPrintLevel > 1) {
            std::cout << "PEX failure: " << ((vhpos - vhtestpos).Cross(vhtestdir)).Mag()
                      << std::endl;
          }
          continue;
        }
        ...
        if (eta > fVecHitMatchEta) {
          if (fPrintLevel > 1) { std::cout << "Eta failure: " << eta << std::endl; }
          continue;
        }
        ...
        if (vhdir.Dot(vhtestdir) * vhdir.X() * vhtestdir.X() < 0 && TMath::Abs(vhdir.X()) > 0.01 &&
            TMath::Abs(vhtestdir.X()) > 0.01) {
          if (fPrintLevel > 1) { std::cout << "lambda sign failure" << std::endl; }
          continue;
        }

        if (fPrintLevel > 1) { std::cout << " vh cluster match " << std::endl; }
        foundmatch = true;
      }
    ...
    int tpcpatrec2::makepatrectrack(std::vector<gar::rec::TPCCluster>& trackTPCClusters,
                                    gar::rec::TrackPar& trackpar)
    {
      ...
      if (fSortAlg == 1) {
        gar::rec::sort_TPCClusters_along_track(trackTPCClusters,
                                               hlf,
                                               hlb,
                                               fPrintLevel,
                                               lengthforwards,
        ...
        gar::rec::sort_TPCClusters_along_track2(
          trackTPCClusters, hlf, hlb, fPrintLevel, lengthforwards, lengthbackwards, fSortDistCut);
      }
      else {
        throw cet::exception("tpcpatrec2_module")
          << "Sort Algorithm switch not understood: " << fSortAlg;
      }

      std::vector<float> tparbeg(6, 0);
      float xother = 0;
      if (gar::rec::initial_trackpar_estimate(trackTPCClusters,
                                              hlf,
                                              tparbeg[2],
                                              tparbeg[4],
                                              tparbeg[3],
