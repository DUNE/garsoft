...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      std::array<double, 3> CalculateStripHitPosition(float x,
                                                      float y,
                                                      float z,
    ...
    CaloStripSplitter::CaloStripSplitter(fhicl::ParameterSet const& p) : EDProducer{p}
    {
      ...
      if (fSaveStripEnds) {
        MF_LOG_INFO("CaloStripSplitter_module") << " Saving Strip ends flag turned on! ";
      }
          ...
          for (auto const& it : unsplitHits) {
            ...
            if (not fGeo->isTile(point, cellID)) {
              ...
              std::array<double, 3> strip_pos =
                this->CalculateStripHitPosition(newpos[0], newpos[1], newpos[2], time, cellID);
    ...
    //----------------------------------------------------------------------------
    void CaloStripSplitter::CollectHits(const art::Event& evt,
                                        const std::string& label,
                                        const std::string& instance,
                                        std::vector<art::Ptr<gar::rec::CaloHit>>& hitVector)
    {
    ...
    //----------------------------------------------------------------------------
    std::array<double, 3> CaloStripSplitter::CalculateStripHitPosition(float x,
                                                                       float y,
                                                                       float z,
                                                                       std::pair<float, float> time,
                                                                       raw::CellID_t cID)
    ...
    //----------------------------------------------------------------------------
    float CaloStripSplitter::CorrectStripHitTime(float x,
                                                 float y,
                                                 float z,
                                                 std::pair<float, float> hitTime,
                                                 raw::CellID_t cID)
