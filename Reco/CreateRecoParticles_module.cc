...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      void  ParticlesFromTracks(const art::Event &evt,
      ...
      std::unique_ptr<gar::rec::alg::TruncatedIonizationCalculator>  fTPCIonizationAlg;   ///< algorithm to compute TPC mean ionization
      std::unique_ptr<gar::rec::alg::ECALMuonBDT>                    fECALMuonBDT;        ///< algorithm to compute ECAL/MuID muon scores
      std::unique_ptr<gar::rec::alg::ECALToFAlg>                     fECALToFAlg;         ///< algorithm to compute ECAL arrival times
  ...
  namespace rec {
    ...
    void CreateRecoParticles::produce(::art::Event& evt)
    {
      ...
      // Output collections
      std::unique_ptr< std::vector<rec::RecoParticle> > RecoParticleCol(new std::vector<rec::RecoParticle>);

      std::unique_ptr< art::Assns<rec::RecoParticle,rec::Track> > RecoParticleTrkAssns(new ::art::Assns<rec::RecoParticle,rec::Track>);
      std::unique_ptr< art::Assns<rec::RecoParticle,rec::Cluster> > RecoParticleECalAssns(new ::art::Assns<rec::RecoParticle,rec::Cluster>);
      std::unique_ptr< art::Assns<rec::RecoParticle,rec::Cluster> > RecoParticleMuIDAssns(new ::art::Assns<rec::RecoParticle,rec::Cluster>);
      ...
      std::vector< rec::RecoParticle* > particleVector;
      this->ParticlesFromTracks(evt, particleVector, MapRecoParticleTrackPtr, MapRecoParticleECalPtr, MapRecoParticleMuIDPtr);
    ...
    //--------------------------------------------------------------------------
    void CreateRecoParticles::PrepareClusterMap(art::Handle< std::vector<rec::Cluster> > &RecoClusterHandle, std::unordered_map<ClusterId, Int_t> &IdToIndexMap)
    {
    ...
    //--------------------------------------------------------------------------
    void CreateRecoParticles::ParticlesFromTracks(const art::Event &evt,
                                                  std::vector< rec::RecoParticle* > &particleVector,
                                                  std::unordered_map<size_t, art::Ptr<rec::Track>> &MapRecoParticleTrackPtr,
                                                  std::unordered_map<size_t, std::vector< art::Ptr<rec::Cluster>> > &MapRecoParticleECalPtr,
                                                  std::unordered_map<size_t, std::vector< art::Ptr<rec::Cluster>> > &MapRecoParticleMuIDPtr)
    {
      ...
      if (!TrackHandle) {
        throw cet::exception("CreateRecoParticles") << " No rec::Track branch."
      ...
      art::FindOneP<rec::TrackIoniz>* findIonization = new art::FindOneP<rec::TrackIoniz>(TrackHandle, evt, fTrackLabel);
      ...
      if (!RecoECALHandle) {
        throw cet::exception("ECALAna") << " No rec::Cluster branch."
                                        << " Line " << __LINE__ << " in file " << __FILE__ << std::endl;
      }
      ...
      if (!RecoMuIDHandle) {
        throw cet::exception("ECALAna") << " No rec::Cluster MuID branch."
                                        << " Line " << __LINE__ << " in file " << __FILE__ << std::endl;
      }

      // Get associations between tracks and ECAL clusters and between ECAL clusters and ECAL hits
      art::FindManyP<rec::Cluster, rec::TrackEnd>* findManyTrackECAL = new art::FindManyP<rec::Cluster, rec::TrackEnd>(TrackHandle, evt, ecalassntag);
      art::FindManyP<gar::rec::CaloHit>* findManyECALRecoHit = new art::FindManyP<gar::rec::CaloHit>(RecoECALHandle, evt, ecalclustertag);

      // Get associations between tracks and MuID clusters and between MuID clusters and MuID hits
      art::FindManyP<rec::Cluster, rec::TrackEnd>* findManyTrackMuID = new art::FindManyP<rec::Cluster, rec::TrackEnd>(TrackHandle, evt, ecalassntagmuid);
      art::FindManyP<gar::rec::CaloHit>* findManyMuIDRecoHit = new art::FindManyP<gar::rec::CaloHit>(RecoMuIDHandle, evt, ecalclustertagmuid);

      // Get associations between Tracks and Vertices
      art::FindManyP<rec::Vertex, rec::TrackEnd>* findManyTracksVertices = NULL;
      findManyTracksVertices = new art::FindManyP<rec::Vertex, rec::TrackEnd>(TrackHandle, evt, fVertexLabel);
      ...
      for (size_t iTrack = 0; iTrack < TrackHandle->size(); ++iTrack) {
          ...
          if ( findManyTrackECAL->isValid() ) {
            ...
            if (nECALedTracks > 0) {
              iEnd = *(findManyTrackECAL->data(iTrack).at(0));     // in trackecalassns2 the used track end is always the same for one track...
              particle->setTrackEndECALed(iEnd); // just fill this if there are associations, if not it's just a -1
            }
          }
          ...
          if (findManyTracksVertices->isValid()) {
            ...
            if (nVertex > 0) particle->setTrackEndVertexed(*(findManyTracksVertices->data(iTrack).at(0))); // just fill this if there are associations, if not it's just a -1
    ...
    //--------------------------------------------------------------------------
    void CreateRecoParticles::GuessParticleDirection(std::vector< rec::RecoParticle* > &particleVector,
                                                     art::Handle< std::vector<rec::Track> > &RecoTrackHandle)
    {
      ...
      if (highest_momentum_ecaled > 0.0) {
        for (int i=0; i<3; ++i) candidate_trackend_vertexed[i] = position_trackend_ecaled[i];
      // If no particle was ECaled use the position of the highest momentum vertexed particle
        ...
        for (int i=0; i<3; ++i) candidate_trackend_vertexed[i] = position_trackend_vertexed[i];
      ...
      for ( auto const& particle : particleVector ) {
        ...
        float distance_track_begin = std::hypot(candidate_trackend_vertexed[0]-track_ptr->Vertex()[0],
