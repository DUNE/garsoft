...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      size_t
        fNPasses; ///< number of passes to reassign TPCClusters from vector hits with poor chisquareds

      std::vector<float> fMaxVecHitLen; ///< maximum vector hit length in patrec alg 2, in cm
      std::vector<float>
        fVecHitRoad; ///< max dist from a vector hit to a TPCCluster to assign it. for patrec alg 2.  in cm.
      std::vector<unsigned int>
        fVecHitMinTPCClusters; ///< minimum number of TPCClusters on a vector hit for it to be considered
      std::vector<float>
        fTPCClusterRCut; ///< only take TPCClusters within rcut of the center of the detector
      std::vector<float>
        fTPCClusterGapCut;       ///< in cm -- skip TPC clusters within this distance of a gap
      ...
      bool vh_TPCClusmatch(int iTPCCluster,
                           vechit_t& vechit,
      ...
      void fitlines6ls(std::vector<double>& x,
                       std::vector<double>& y,
                       std::vector<double>& z,
                       TVector3& pos,
                       TVector3& dir,
                       float& chi2ndf);
      void fitlinesPCA(std::vector<double>& x,
                       std::vector<double>& y,
                       std::vector<double>& z,
                       TVector3& pos,
                       TVector3& dir,
                       float& chi2ndf);
      void fitline(std::vector<double>& x,
                   std::vector<double>& y,
                   double& lambda,
                   double& intercept,
                   double& chi2ndf);
    ...
    void tpcvechitfinder2::produce(art::Event& e)
    {
      ...
      std::unique_ptr<art::Assns<gar::rec::TPCCluster, gar::rec::VecHit>> TPCClusterVHAssns(
        new ::art::Assns<gar::rec::TPCCluster, gar::rec::VecHit>);
      ...
      for (size_t ipass = 0; ipass < fNPasses; ++ipass) {
        for (size_t iTPCClusterxs = 0; iTPCClusterxs < hsi.size(); ++iTPCClusterxs) {
          int iTPCCluster =
            hsi[iTPCClusterxs]; // access TPCClusters sorted in X but keep original indices
          ...
          if (rclus > geo->GetIROCInnerRadius()) {
            ...
            UInt_t isector =
              TMath::Floor(phisc); // assumes the sector boundary is at phi=0  // goes from 0 to 17
          ...
          for (size_t ivh = 0; ivh < vhtmp.size(); ++ivh) {
            ...
            if (vh_TPCClusmatch(iTPCCluster,
                                vhtmp[ivh],
                                TPCClusters,
                                proposedvh,
                                ipass)) // updates vechit with this TPCCluster if matched
            {
          ...
          if (!matched) // make a new vechit if we haven't found one yet
          {
            ...
            vh.dir.SetXYZ(
              0, 0, 0);    // new vechit with just one TPCCluster; don't know the direction yet
        ...
        if (fNPasses > 1 && ipass < fNPasses - 1) {
          ...
          for (size_t ivh = 0; ivh < vhtmp.size(); ++ivh) {
            if (fPrintLevel > 1) {
              std::cout << "about to push vh: " << ivh << " " << vhtmp[ivh].c2ndf << " "
                        << vhtmp[ivh].TPCClusterindex.size() << std::endl;
            }
            if (vhtmp[ivh].c2ndf < fC2Cut.at(ipass) &&
                vhtmp[ivh].TPCClusterindex.size() >= (size_t)fVecHitMinTPCClusters.at(ipass)) {
              vhtmp2.push_back(vhtmp[ivh]);
            }
            else {
              for (size_t iTPCCluster = 0; iTPCCluster < vhtmp[ivh].TPCClusterindex.size();
                   ++iTPCCluster) {
          ...
          vhtmp = vhtmp2;
          if (fPrintLevel > 1) {
            std::cout << "left with " << vhtmp.size() << " vec hits and " << hsi.size()
                      << " TPCClusters to reassociate " << std::endl;
          }
        }
      }
    ...
    bool tpcvechitfinder2::vh_TPCClusmatch(int iTPCCluster,
                                           vechit_t& vechit,
                                           const std::vector<gar::rec::TPCCluster>& TPCClusters,
                                           vechit_t& proposedvh,
                                           size_t ipass)
    ...
    void tpcvechitfinder2::fitlinesdir(std::vector<TVector3>& hlist,
                                       TVector3& pos,
                                       TVector3& dir,
                                       float& chi2ndf)
    {
      ...
      if (fLineFitAlg == 0) { fitlines6ls(x, y, z, pos, dir, chi2ndf); }
      else if (fLineFitAlg == 1) {
        fitlinesPCA(x, y, z, pos, dir, chi2ndf);
      }
      else {
        throw cet::exception("tpcvechitfinder2_module.cc: ununderstood LineFitAlg parameter: ")
          << fLineFitAlg;
      }
    }

    void tpcvechitfinder2::fitlines6ls(std::vector<double>& x,
                                       std::vector<double>& y,
                                       std::vector<double>& z,
                                       TVector3& pos,
                                       TVector3& dir,
                                       float& chi2ndf)
    {
      ...
      if (slopesumx < slopesumy && slopesumx < slopesumz) {
        ...
        chi2ndf =
          chi2ndf_xy +
          chi2ndf_xz; // not really a chisquared per ndf, but perhaps good enough to pick which VH to assign a TPCCluster to
      }
        ...
        chi2ndf =
          chi2ndf_yx +
          chi2ndf_yz; // not really a chisquared per ndf, but perhaps good enough to pick which VH to assign a TPCCluster to
      }
      else {
        ...
        chi2ndf =
          chi2ndf_zx +
          chi2ndf_zy; // not really a chisquared per ndf, but perhaps good enough to pick which VH to assign a TPCCluster to
      }
      dir *= 1.0 / dir.Mag();
    ...
    void tpcvechitfinder2::fitline(std::vector<double>& x,
                                   std::vector<double>& y,
                                   double& slope,
                                   double& intercept,
                                   double& chi2ndf)
    {
      ...
      if (n > 2) {
        ...
        chi2ndf =
          1.0; // a choice here -- if we can add the TPCCluster to an existing VH or make a "perfect-chisquare" 2-TPCCluster VH?
        // define a 2-TPCCluster VH to be not perfect.
      }
    }

    void tpcvechitfinder2::fitlinesPCA(std::vector<double>& x,
                                       std::vector<double>& y,
                                       std::vector<double>& z,
                                       TVector3& pos,
                                       TVector3& dir,
