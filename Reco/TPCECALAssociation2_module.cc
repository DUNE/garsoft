...
namespace gar {
  namespace rec {
    ...
    void TrackAssn::set_chisq_data(gar::rec::TrackEnd iEnd,
                                   int clusid,
                                   float chisq,
                                   float chisq_cut)
    {
    ...
    {
      if (iEnd == TrackEndBeg) { return chisq_fwd; }
      else {
        return chisq_bak;
      }
    }
    ...
    void TrackAssn::check_direction()
    {
      ...
      if (n_chisq_fwd > n_chisq_bak) { candidate_end = TrackEndBeg; }
        ...
        if (chisq_fwd_max > chisq_bak_max) { candidate_end = TrackEndEnd; }
        else {
    ...
    {
      if (candidate_end == TrackEndBeg) { return chisq_fwd; }
      else {
        return chisq_bak;
      }
    }
    ...
    {
      if (candidate_end == TrackEndBeg) { return n_chisq_fwd; }
      else {
        return n_chisq_bak;
      }
    }
    ...
    void TPCECALAssociation2::beginJob()
    {
      ...
      if (fGlobalTimeCorrection && fClusterTimeCorrection) {
        throw cet::exception("TPCECALAssociation2")
          << " Job started with conflicting configuration!" << std::endl;
      }
    ...
    void TPCECALAssociation2::produce(art::Event& e)
    {
      ...
      for (size_t iTrack = 0; iTrack < TrackHandle->size(); ++iTrack) {
        ...
        for (gar::rec::TrackEnd iEnd = TrackEndBeg; iEnd >= TrackEndEnd; --iEnd) {
          ...
          float phi_max = -1.;
          if (trackPar[2] > 0) { phi_max = trackPar[3] + fMaxAngle * TMath::Pi(); }
          else {
          ...
          for (size_t iCluster = 0; iCluster < ClusterHandle->size(); ++iCluster) {
            ...
            ROOT::Math::WrappedFunction<std::function<float(float)>>
              helix_circle_intersections_wrapped(helix_circle_intersections_to_wrap);
            ...
            try {
              ...
              float projected[3];
              if (fGlobalTimeCorrection) { helix(root, trackPar, trackEnd, projected, ecal_t0); }
            ...
            trackassn.set_chisq_data(
              iEnd,
              iCluster,
              chisq,
              fChisqCut); // this will fill (or not, depending on the cut value) the appropriate chisq data vector (either FWD or BAK)
        ...
        if (fCheatDirection) {
          ...
          if (trakt.size() > 0) {
            ...
            if (end_begin_dist >= end_end_dist) { trackassn.set_direction(TrackEndBeg); }
            else {
    ...
    float TPCECALAssociation2::get_global_ecal_t0(
    ...
    float TPCECALAssociation2::helix_circle_intersections(float phi,
                                                          float y0,
                                                          float z0,
                                                          float R,
                                                          float phi0,
                                                          float r)
    {
      return TMath::Power(y0 - R * (TMath::Cos(phi) - TMath::Cos(phi0)) - ItsInTulsa[1], 2) +
             TMath::Power(z0 + R * (TMath::Sin(phi) - TMath::Sin(phi0)) - ItsInTulsa[2], 2) -
             TMath::Power(r, 2);
    }

    void TPCECALAssociation2::helix(float phi,
                                    float* trackPar,
                                    float* trackEnd,
                                    float* projected,
                                    float t0)
