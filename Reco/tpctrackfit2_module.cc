...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      float
        fKalCurvStepUncSq; ///< constant uncertainty term on each step of the Kalman fit -- squared, for curvature
      float
        fKalPhiStepUncSq; ///< constant uncertainty term on each step of the Kalman fit -- squared, for phi
      float
        fKalLambdaStepUncSq; ///< constant uncertainty term on each step of the Kalman fit -- squared, for lambda
      ...
      float
        fTPCClusterResolX; ///< drift direction contribution to determine step size (resolution of a TPCCluster)
      unsigned int
        fInitialTPNTPCClusters; ///< number of TPCClusters to use for initial trackpar estimate, if present
    ...
    tpctrackfit2::tpctrackfit2(fhicl::ParameterSet const& p) : EDProducer{p}
    {
      ...
      fTPCClusterResolYZ =
        p.get<float>("TPCClusterResolYZ", 1.0); // TODO -- think about what this value is
    ...
    void tpctrackfit2::produce(art::Event& e)
    {
      ...
      for (size_t itrack = 0; itrack < patrecTracks.size(); ++itrack) {
        ...
        TrackIoniz trackions;
        TrackTrajectory tracktraj;
        if (KalmanFitBothWays(TPCClusters, trackparams, trackions, tracktraj) ==
...

    {
      ...
      if (fSortAlg == 1) {
        ...
        throw cet::exception("tpctrackfit2_module")
      ...
      int retcode = KalmanFit(TPCClusters,
                              hlf,
                              tparend,
                              chisqforwards,
                              lengthforwards,
                                ...
                                std::vector<TVector3>& trajpts)
    {
      ...
      P[2][2] = TMath::Sq(
        .5); // curvature of zero gets us to infinite momentum, and curvature of 2 is curled up tighter than the pads
      ...
      for (size_t iTPCCluster = 1; iTPCCluster < nTPCClusters; ++iTPCCluster) {
        ...
        float dxnum = (slope / (fTPCClusterResolYZ * fTPCClusterResolYZ)) *
                        ((yh - parvec[0]) * TMath::Sin(phi) + (zh - parvec[1]) * TMath::Cos(phi)) +
                      (xh - xpos) / (fTPCClusterResolX * fTPCClusterResolX);
        float dxdenom = slope * slope / (fTPCClusterResolYZ * fTPCClusterResolYZ) +
                        1.0 / (fTPCClusterResolX * fTPCClusterResolX);
        ...
        if (fPrintLevel > 0) {
          ...
          std::cout << " Parvec:   y " << parvec[0] << " z " << parvec[1] << " c " << parvec[2]
                    << " phi " << parvec[3] << " lambda " << parvec[4] << std::endl;
        }
        ...
        if (fPrintLevel > 1) {
          std::cout << " Predstep: y " << predstep[0] << " z " << predstep[1] << " c "
                    << predstep[2] << " phi " << predstep[3] << " lambda " << predstep[4]
                    << std::endl;
        }
        ...
        float d_length =
          TMath::Sqrt(dx * dx + TMath::Sq(parvec[0] - yprev) + TMath::Sq(parvec[1] - zprev));
        length += d_length;
      ...
      if (fPrintLevel > 1) {
        std::cout << "Track params at end (y, z, curv, phi, lambda) " << trackparatend[0] << " "
                  << trackparatend[1] << " " << trackparatend[2] << " " << trackparatend[3] << " "
                  << trackparatend[4] << std::endl;
        S.Print();
      }
