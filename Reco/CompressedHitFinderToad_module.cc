...
namespace gar {
  namespace rec {
    ...
    class CompressedHitFinderToad : public art::EDProducer {
    public:
      explicit CompressedHitFinderToad(fhicl::ParameterSet const& p);
      // The compiler-generated destructor is fine for non-base
      // classes without bare pointers or other resource use.

      // Plugins should not be copied or assigned.
      CompressedHitFinderToad(CompressedHitFinderToad const&) = delete;
      CompressedHitFinderToad(CompressedHitFinderToad&&) = delete;
      CompressedHitFinderToad& operator=(CompressedHitFinderToad const&) = delete;
      CompressedHitFinderToad& operator=(CompressedHitFinderToad&&) = delete;
    ...
    private:
      ...
      int
        fADCThreshold; ///< zero-suppression threshold (in case the raw digits need to be zero-suppressed)
      int
        fTicksBefore; ///< zero-suppression ticks before.  Only used if raw waveforms are not zero-suppressed and we must do it
      int
        fTicksAfter; ///< zero-suppression ticks after.  Only used if raw waveforms are not zero-suppressed and we must do it
      ...
      float
        fHitFracADCNewHit; ///< threshold below which if the ADC falls below multiplied by adcmax, to start a new hit
    ...
    CompressedHitFinderToad::CompressedHitFinderToad(fhicl::ParameterSet const& p) : EDProducer{p}
    {
      ...
      fAvgPulseLongHist =
        tfs->make<TH1D>("garAvgPulseLongHist", "Pulse ADC Average;tick;ADC Sum", 1000, -0.5, 999.5);
      fAvgPulseShortHist =
        tfs->make<TH1D>("garAvgPulseShortHist", "Pulse ADC Average;tick;ADC Sum", 200, -0.5, 199.5);
    }

    void CompressedHitFinderToad::produce(art::Event& e)
    {
      ...
      std::unique_ptr<art::Assns<rec::Hit, raw::RawDigit>> hitDigAssns(
        new ::art::Assns<rec::Hit, raw::RawDigit>);
      ...
      for (size_t ird = 0; ird < rdCol->size(); ++ird) {
        ...
        if (rd.Compression() == raw::kNone) {
          ...
          MF_LOG_WARNING("CompressedHitFinder")
            << " Ununderstood compression mode: " << rd.Compression() << " Not making hits.";
        ...
        for (int iBlock = 0; iBlock < nBlocks; ++iBlock) {
          ...
          if (blocksize < 1) {
            throw cet::exception("CompressedHitFinder")
          ...
          double distonetick =
            fDetProp->DriftVelocity() * (fTime->TPCTick2Time(1) - fTime->TPCTick2Time(0));

          for (int jInBlock = 0; jInBlock < blocksize;
               ++jInBlock) // loop over time samples in each block
          {
            ...
            if (t0adcblockhist == 0) { t0adcblockhist = t; }
            int deltat = t - t0adcblockhist;
            if (deltat < fAvgPulseLongHist->GetNbinsX()) { fAvgPulseLongHist->Fill(deltat, a); }
            else {
              t0adcblockhist = 0;
            }
            endT = begT + jhl;
            ...
            if (splithit || jInBlock == (blocksize - 1)) {
              ...
              if (chanposx < fGeo->TPCXCent()) { pos[0] = chanposx + driftdistance; }
              else {
    ...
    DEFINE_ART_MODULE(CompressedHitFinderToad)
