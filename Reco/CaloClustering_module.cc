...
namespace gar {
  namespace rec {
    ...
    private:
      ...
      void CollectTracks(const art::Event& evt,
    ...
    CaloClustering::CaloClustering(fhicl::ParameterSet const& p) : EDProducer{p}
    {
      ...
      if (fClusterAlgo->usesTracks())
        produces<art::Assns<gar::rec::Cluster, gar::rec::Track>>(fInstanceName);
    }
    ...
    void CaloClustering::produce(art::Event& e)
    {
      ...
      if (fClusterAlgo->usesTracks()) this->CollectTracks(e, fTrackLabel, artTrk);
      ...
      std::unique_ptr<std::vector<gar::rec::Cluster>> ClusterCol(
        new std::vector<gar::rec::Cluster>);
      std::unique_ptr<art::Assns<gar::rec::Cluster, gar::rec::CaloHit>> ClusterHitAssns(
        new art::Assns<gar::rec::Cluster, gar::rec::CaloHit>);
      std::unique_ptr<art::Assns<gar::rec::Cluster, gar::rec::Track>> ClusterTrackAssns(
        new art::Assns<gar::rec::Cluster, gar::rec::Track>);

      art::PtrMaker<gar::rec::Cluster> makeClusterPtr(e, fInstanceName);

      MF_LOG_DEBUG("CaloClustering_module") << "Found " << ClusterVec.size() << " Clusters";
      ...
      if (fClusterAlgo->usesTracks()) e.put(std::move(ClusterTrackAssns), fInstanceName);

      return;
    }

    void CaloClustering::CollectHits(const art::Event& evt,
                                     const std::string& label,
                                     const std::string& instance,
                                     std::vector<art::Ptr<gar::rec::CaloHit>>& hitVector)
    {
    ...
    void CaloClustering::CollectTracks(const art::Event& evt,
                                       const std::string& label,
                                       std::vector<art::Ptr<gar::rec::Track>>& trkVector)
    {
