...
class fastTracker {
public:
  static AliExternalTrackParam* makeSeed(double xyz0[3],
                                         double xyz1[3],
                                         double xyz2[3],
                                         double sy,
                                         double sz,
                                         float bz);
  static AliExternalTrackParam* makeSeedMB(double xyz0[3],
                                           double xyz1[3],
                                           double xyz2[3],
                                           double sy,
                                           double sz,
                                           ...
                                           float cross1 = 0,
                                           float cross2 = 0);
  static AliExternalTrackParam* makeSeedMB4(double xyz0[3],
                                            double xyz1[3],
                                            double xyz2[3],
                                            double sy,
                                            double sz,
                                            ...
                                            float mass,
                                            int nSteps = 5,
                                            bool MSsmearing = kTRUE,
                                            float cross1 = 0,
                                            float cross2 = 0);
  static Double_t makeC(Double_t x1,
                        Double_t y1,
                        Double_t x2,
                        Double_t y2,
                        Double_t x3,
                        Double_t y3); // F1
  static Double_t makeSnp(Double_t x1,
                          Double_t y1,
                          Double_t x2,
                          Double_t y2,
                          Double_t x3,
                          Double_t y3); // F2
  static Double_t makeYC(Double_t x1,
                         Double_t y1,
                         Double_t x2,
                         Double_t y2,
                         Double_t x3,
                         Double_t y3); // YC
  static Double_t makeTgln(Double_t x1,
                           Double_t y1,
                           Double_t x2,
                           Double_t y2,
                           Double_t z1,
...
Double_t fastTracker::makeC(Double_t x1,
                            Double_t y1,
                            Double_t x2,
                            Double_t y2,
                            Double_t x3,
                            Double_t y3)
{
  ...
  if (TMath::Abs(det) < 1e-10) { return 100; }
  //
...
Double_t fastTracker::makeSnp(Double_t x1,
                              Double_t y1,
                              Double_t x2,
                              Double_t y2,
                              Double_t x3,
...
Double_t fastTracker::makeYC(Double_t x1,
                             Double_t y1,
                             Double_t x2,
                             Double_t y2,
                             Double_t x3,
...
//_____________________________________________________________________________
Double_t fastTracker::makeTgln(Double_t x1,
                               Double_t y1,
                               Double_t x2,
                               Double_t y2,
                               Double_t z1,
...
AliExternalTrackParam* fastTracker::makeSeed(double xyz0[3],
                                             double xyz1[3],
                                             double xyz2[3],
                                             double sy,
                                             double sz,
                                             float bz)
{
  ...
  //
  Double_t f20 =
    (makeSnp(xyz0[0], xyz0[1] + sy, xyz1[0], xyz1[1], xyz2[0], xyz2[1]) - param[2]) / sy;
  Double_t f22 =
    (makeSnp(xyz0[0], xyz0[1], xyz1[0], xyz1[1] + sy, xyz2[0], xyz2[1]) - param[2]) / sy;
  Double_t f23 =
    (makeSnp(xyz0[0], xyz0[1], xyz1[0], xyz1[1], xyz2[0], xyz2[1] + sy) - param[2]) / sy;
  //
  //  makeTgln(xyz0[0],xyz0[1],xyz1[0],xyz1[1],xyz0[2],xyz1[2],param[4]);
  Double_t f30 =
    (makeTgln(xyz0[0], xyz0[1] + sy, xyz1[0], xyz1[1], xyz0[2], xyz1[2], param[4]) - param[3]) / sy;
  Double_t f31 =
    (makeTgln(xyz0[0], xyz0[1], xyz1[0], xyz1[1], xyz0[2] + sz, xyz1[2], param[4]) - param[3]) / sz;
  Double_t f32 =
    (makeTgln(xyz0[0], xyz0[1], xyz1[0], xyz1[1] + sy, xyz0[2], xyz1[2], param[4]) - param[3]) / sy;
  Double_t f34 =
    (makeTgln(xyz0[0], xyz0[1], xyz1[0], xyz1[1], xyz0[2], xyz1[2] + sz, param[4]) - param[3]) / sz;
  c[0] = sy2;
  c[1] = 0.;
  c[2] = sz2;
  c[3] = f20 * sy2;
  ...
  c[6] = f30 * sy2;
  c[7] = f31 * sz2;
  c[8] = f30 * sy2 * f20 + f32 * sy2 * f22;
  c[9] = f30 * sy2 * f30 + f31 * sz2 * f31 + f32 * sy2 * f32 + f34 * sz2 * f34;
  c[10] = f40 * sy2;
  c[11] = 0.;
  c[12] = f40 * sy2 * f20 + f42 * sy2 * f22 + f43 * sy2 * f23;
...
/// \param mass
/// \return
AliExternalTrackParam* fastTracker::makeSeedMB(double xyz0[3],
                                               double xyz1[3],
                                               double xyz2[3],
                                               double sy,
                                               double sz,
                                               ...
                                               float cross2)
{
  ...
  for (int i = 1; i < 3; i++) {
    ...
    for (int j = 0; j < nSteps; j++) {
      ...
      //propStatus &=
      paramFull.CorrectForMeanMaterial(
        crossLength * xx0tocm / nSteps, crossLength * xrhotocm / nSteps, mass, kFALSE);
      //propStatus &=
      paramFullnoMC.PropagateTo((xyz[i - 1][0] + ((j + 1) * (xyz[i][0] - xyz[i - 1][0]) / nSteps)),
                                bz);
    }
    ...
    if (i == 1) {
      for (int iCovar = 0; iCovar < 15; iCovar++) {
        deltaCovar[iCovar] =
          paramFull.GetCovariance()[iCovar] - paramFullnoMC.GetCovariance()[iCovar];
      }
    }
  }
...
AliExternalTrackParam* fastTracker::makeSeedMB4(double xyz0[3],
                                                double xyz1[3],
                                                double xyz2[3],
                                                double sy,
                                                double sz,
                                                ...
                                                float cross2)
{
  ...
  for (int i = 1; i < 3; i++) {
    ...
    for (int j = 0; j < nSteps; j++) {
      //propStatus &=
      paramFull.PropagateTo(
        (xyz[i - 1][0] + ((j + 1) * (xyz[i][0] - xyz[i - 1][0]) / nSteps)), bz, 1);
      if (i == 2) {
        //propStatus &=
        paramFull.CorrectForMeanMaterial(crossLength * xx0tocm / 5.,
                                         crossLength * xrhotocm / 5.,
                                         mass,
                                         0.01,
                                         0 + 0x2 * MSsmearing);
      }
      //propStatus &=
      paramFullnoMC.PropagateTo(
        (xyz[i - 1][0] + ((j + 1) * (xyz[i][0] - xyz[i - 1][0]) / nSteps)), bz, 1);
    }
