    ...
    namespace alg {
          ...
          for (In other = first + 1; other != last; other++) {
            ...
            if (pred->mergeHits((*first), (*other))) {
              ...
              if ((*first)->second == 0 && (*other)->second == 0) { // no cluster exists
                ...
                if ((*first)->second !=
                    (*other)->second) // this is a bug fix for old gcc 3.2 compiler !
                  (*first)->second->mergeClusters((*other)->second);
              }
              else { // one cluster exists

                if ((*first)->second != 0) { (*first)->second->addHit((*other)); }
                else {
      ...
      */
      ...
      public:
        ...
        GenericCluster(GenericHit<T>* hit) { addHit(hit); }
      ...
      */
      ...
      public:
        ~GenericHitVec()
        {
          for (typename GenericHitVec::iterator i = Vector::begin(); i != Vector::end(); i++)
            delete *i;
        }
      };
      ...
      */
      ...
      void addToGenericHitVec(GenericHitVec<T>& v, CaloHitVec vec, Pred pred)
      {
        ...
        for (unsigned int i = 0; i < vec.size(); i++) {
          ...
          if (pred(hit)) { v.push_back(new GenericHit<T>(hit)); }
        }
      }
      ...
      */
      ...
      void addToGenericHitVec(GenericHitVec<T>& v, CaloHitVec vec, Pred pred, Order order)
      {
        ...
        for (unsigned int i = 0; i < vec.size(); i++) {
          ...
          if (pred(hit)) { v.push_back(new GenericHit<T>(hit, order(hit))); }
        }
      }
