...
public:
  ...
  Double_t Beta()
  {
    Double_t p2 = GetP();
    p2 *= p2;
    return TMath::Sqrt(p2 / (p2 + fMass * fMass));
  }
  ...
  Bool_t CorrectForMeanMaterial(Double_t xOverX0,
                                Double_t xTimesRho,
                                Double_t mass,
                                Float_t stepFraction = 0.01,
                                int mcSwitch = 0,
                                Double_t (*f)(Double_t) = AliExternalTrackParam::BetheBlochSolid);
  static Double_t dPdxEulerStep(
    Double_t p,
    Double_t mass,
    Double_t xTimesRho,
    Double_t step,
    Double_t (*fundEdx)(Double_t) = AliExternalTrackParam::BetheBlochSolid);
...
public:
  ...
  {
    fLayerRadius.resize(maxLayer);
    fLayerX0.resize(maxLayer);
    fLayerRho.resize(maxLayer);
    fLayerIndex.resize(maxLayer);
    fLayerResolRPhi.resize(maxLayer);
    fLayerResolZ.resize(maxLayer);
  }
  void setLayerRadiusPower(int layer0,
                           int layerN,
                           float r0,
                           float rN,
                           float power,
...
public:
  ...
  {
    fLayerIndex.reserve(nLayers);
    fDirection.reserve(nLayers);
    fParamIn.reserve(nLayers);
    fParamInRot.reserve(nLayers);
    fParamOut.reserve(nLayers);
    fParamRefit.reserve(nLayers);
    fParamMC.reserve(nLayers);
    fStatusMaskMC.reserve(nLayers);
    fStatusMaskIn.reserve(nLayers);
    fStatusMaskOut.reserve(nLayers);
    fStatusMaskRefit.reserve(nLayers);
    fChi2.resize(nLayers);
    fLoop.reserve(nLayers);
    fNPointsMC.reserve(nLayers);
    fNPointsIn.reserve(nLayers);
    fNPointsOut.reserve(nLayers);
    fNPointsRefit.reserve(nLayers);
    fMaxLayer = 0;
    fDecayLength = 0;
  }
  ...
  RVec<AliExternalTrackParam4D> fParamRefit; //   "reconstructed" Param Refit
  ...
  RVec<AliExternalTrackParam4D> fParamInRot; //   "reconstructed" Param In - in non rotated frame
