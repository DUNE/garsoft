...
Int_t BuildParticlePoints(fastParticle& particle,
                          double Center[3],
                          fastGeometry geom,
                          ...
                          double displaceY)
{
  ...
  for (size_t k = 0; k < trajxyz.size(); k++) {
    ...
    Double_t radius = sqrt((xyz_conv[1] - displaceY) * (xyz_conv[1] - displaceY) +
                           (xyz_conv[0] - displaceX) * (xyz_conv[0] - displaceX));
    ...
    uint indexR =
      uint(std::upper_bound(geom.fLayerRadius.begin(), geom.fLayerRadius.end(), radius) -
           geom.fLayerRadius.begin());
    particle.fLayerIndex[count] = indexR;
    if (k != 0) {
      double x_now = particle.fParamMC[count].GetX();
      double x_prev = particle.fParamMC[count - 1].GetX();
      if ((x_now / x_prev) > 1)
        particle.fDirection[count] = +1;
      else
        particle.fDirection[count] = -1;
    }
    else
