...
*/
  ...
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "Triton", "Triton", 2.80925, kFALSE, khShGev / (12.33 * kYear2Sec), 3, "Ion", ionCode);
  }
  ...
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "Alpha", "Alpha", 3.727379, kTRUE, khShGev / (12.33 * kYear2Sec), 6, "Ion", ionCode);
  }
  ...
  ionCode = 1010010030;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("HyperTriton", "HyperTriton", 2.99131, kFALSE, 2.5e-15, 3, "Ion", ionCode);
  }

  ionCode = -1010010030;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiHyperTriton", "AntiHyperTriton", 2.99131, kFALSE, 2.5e-15, 3, "Ion", ionCode);
  }

  ionCode = 1010010040;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("Hyperhydrog4", "Hyperhydrog4", 3.931, kFALSE, 2.5e-15, 3, "Ion", ionCode);
  }

  ionCode = -1010010040;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiHyperhydrog4", "AntiHyperhydrog4", 3.931, kFALSE, 2.5e-15, 3, "Ion", ionCode);
  }

  ionCode = 1010020040;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("Hyperhelium4", "Hyperhelium4", 3.929, kFALSE, 2.5e-15, 6, "Ion", ionCode);
  }

  ionCode = -1010020040;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiHyperhelium4", "AntiHyperhelium4", 3.929, kFALSE, 2.5e-15, 6, "Ion", ionCode);
  }

  ionCode = 1010020050;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("Hyperhelium5", "Hyperhelium5", 4.841, kFALSE, 2.5e-15, 6, "Ion", ionCode);
  }

  ionCode = -1010020050;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiHyperhelium5", "AntiHyperhelium5", 4.841, kFALSE, 2.5e-15, 6, "Ion", ionCode);
  }

  ionCode = 1020010040;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "DoubleHyperhydrogen4", "DoubleHyperhydrogen4", 4.106, kFALSE, 2.5e-15, 6, "Ion", ionCode);
  }

  ionCode = -1020010040;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("DoubleAntiHyperhydrogen4",
  ...
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "LambdaNeutron", "LambdaNeutron", 2.054, kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = -1010000020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiLambdaNeutron", "AntiLambdaNeutron", 2.054, kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = 1020000020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("Hdibaryon", "Hdibaryon", 2.23, kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = -1020000020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiHdibaryon", "AntiHdibaryon", 2.23, kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = 1010000030;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "LambdaNeutronNeutron", "LambdaNeutronNeutron", 2.99, kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = -1010000030;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("AntiLambdaNeutronNeutron",
                       "AntiLambdaNeutronNeutron",
                       2.99,
                       kFALSE,
                       2.5e-15,
                       0,
  ...
  ionCode = 1020010020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("Xi0Proton", "Xi0Proton", 2.248, kFALSE, 5e-15, 3, "Ion", ionCode);
  }

  ionCode = -1020010020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("AntiXi0Proton", "AntiXi0Proton", 2.248, kFALSE, 5e-15, 3, "Ion", ionCode);
  }

  ionCode = 1030000020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("OmegaProton", "OmegaProton", 2.592, kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = -1030000020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiOmegaProton", "AntiOmegaProton", 2.592, kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = 1030010020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("OmegaNeutron", "OmegaNeutron", 2.472, kFALSE, 0.003, 3, "Special", ionCode);
  }

  ionCode = -1030010020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiOmegaNeutron", "AntiOmegaNeutron", 2.472, kFALSE, 0.003, 3, "Special", ionCode);
  }

  ionCode = 1060020020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("OmegaOmega", "OmegaOmega", 3.229, kFALSE, 2.5e-15, 6, "Special", ionCode);
  }

  ionCode = -1060020020;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiOmegaOmega", "AntiOmegaOmega", 3.229, kFALSE, 2.5e-15, 6, "Special", ionCode);
  }

  ionCode = 1010010021;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "Lambda1405Proton", "Lambda1405Proton", 2.295, kFALSE, 0.05, 3, "Special", ionCode);
  }

  ionCode = -1010010021;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "AntiLambda1405Proton", "AntiLambda1405Proton", 2.295, kFALSE, 0.05, 3, "Special", ionCode);
  }

  ionCode = 1020000021;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "Lambda1405Lambda1405", "Lambda1405Lambda1405", 2.693, kFALSE, 0.05, 0, "Special", ionCode);
  }

  ionCode = -1020000021;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("AntiLambda1405Lambda1405",
                       "AntiLambda1405Lambda1405",
                       2.693,
                       kFALSE,
                       0.05,
                       0,
  ...
  ionCode = 9010221;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("f0_980", "f0_980", 0.980, kFALSE, 0.07, 0, "Resonance", ionCode);
  }

  ionCode = 225;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("f2_1270", "f2_1270", 1.275, kFALSE, 0.185, 0, "Resonance", ionCode);
  }
  ...
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle(
      "Xi_Minus_1820", "Xi_Minus_1820", 1.8234, kFALSE, 0.024, -3, "Resonance", ionCode);
  }
  if (!pdgDB->GetParticle(-ionCode)) {
    pdgDB->AddParticle(
      "Xi_Plus_1820", "Xi_Plus_1820", 1.8234, kFALSE, 0.024, 3, "Resonance", -ionCode);
  }
  ...
  if (!pdgDB->GetParticle(-ionCode)) {
    pdgDB->AddParticle(
      "Xi_0_Bar_1820", "Xi_0_Bar_1820", 1.8234, kFALSE, 0.024, 0, "Resonance", -ionCode);
  }
  ...
  ionCode = 9322134;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("ps_2100", "ps_2100", 2.100, kFALSE, 0.040, 3, "Resonance", ionCode);
  }
  if (!pdgDB->GetParticle(-ionCode)) {
    pdgDB->AddParticle(
      "anti-ps_2100", "anti-ps_2100", 2.100, kFALSE, 0.040, -3, "Resonance", -ionCode);
  }

  ionCode = 9322136;
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("ps_2500", "ps_2500", 2.500, kFALSE, 0.040, 3, "Resonance", ionCode);
  }
  if (!pdgDB->GetParticle(-ionCode)) {
    pdgDB->AddParticle(
      "anti-ps_2500", "anti-ps_2500", 2.500, kFALSE, 0.040, -3, "Resonance", -ionCode);
  }
  ...
  if (!pdgDB->GetParticle(ionCode)) {
    pdgDB->AddParticle("d*_2380", "d*_2380", 2.38, kFALSE, 0.070, 3, "Resonance", ionCode);
  }
