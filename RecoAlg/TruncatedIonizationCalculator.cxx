  ...
  namespace rec {
    namespace alg {
      ...
      void TruncatedIonizationCalculator::Configure(fhicl::ParameterSet const& pset)
      {
        ...
        fdEdxScoreParsFileName = pset.get<std::string>(
          "dEdxScoreParsFileName",
          "/pnfs/dune/persistent/users/fmlopez/GAr/MVAData/gar_dEdx_proton_score_v00_01_00.root");
      ...
      //----------------------------------------------------------------------------
      void TruncatedIonizationCalculator::PrepareAlgo(const rec::Track* track,
                                                      const rec::TrackIoniz* ionization)
      {
      ...
      //----------------------------------------------------------------------------
      std::vector<std::pair<float, float>> TruncatedIonizationCalculator::RegroupTrackClusters(
      ...
      //----------------------------------------------------------------------------
      float TruncatedIonizationCalculator::CalculateTruncatedMean(
        std::vector<std::pair<float, float>> IonizationData,
        float percentage)
      {
        ...
        std::transform(
          IonizationData.begin(),
          IonizationData.end(),
          std::back_inserter(dQdXvector),
          [](const std::pair<float, float>& pair) -> float { return pair.first / pair.second; });
        ...
        float TruncatedMean;
        TruncatedMean =
          std::accumulate(dEdXvector.begin(), dEdXvector.end(), 0.0) / dEdXvector.size();
      ...
      {
        return (std::exp(dQdx * fFitB * fIonizationEnergy / (fFitC * fGroupGain)) - fFitA) /
               (fFitB)*1e3;
      }

      std::vector<float> TruncatedIonizationCalculator::CalibrateIonization(
        std::vector<float> dQdxData)
      {

        std::vector<float> dEdxData;
        std::transform(dQdxData.begin(),
                       dQdxData.end(),
                       std::back_inserter(dEdxData),
                       [this](const float& dQdx) -> float { return CalibrationFunction(dQdx); });
        return dEdxData;
      }

      //----------------------------------------------------------------------------
      float TruncatedIonizationCalculator::TotalCaloEnergy(
        std::vector<std::pair<float, float>> IonizationData)
      {
        ...
        for (size_t i = 0; i < IonizationData.size(); ++i) {
          ...
          if (deltadEdx <= fdEdxMax) { TotalEnergy += deltadEdx * IonizationData[i].second; }
          else {
      ...
      float TruncatedIonizationCalculator::GetdEdxProtonScore()
      {
        ...
        for (auto& [key, clf] : fScorerMap) {
          ...
          if ((fTrackMomentum >= p_min) && (fTrackMomentum < p_max)) {
            ...
            ProtonScore = Sigmoid(fTruncatedMean - calibration.dEdx_max_f1,
