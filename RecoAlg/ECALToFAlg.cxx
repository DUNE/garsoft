  ...
  namespace rec {
    namespace alg {
      ...
      void ECALToFAlg::Configure(fhicl::ParameterSet const& pset)
      {
        ...
        fToFScoreParsFileName = pset.get<std::string>(
          "ToFScoreParsFileName",
          "/pnfs/dune/persistent/users/fmlopez/GAr/MVAData/gar_tof_proton_score_v00_01_00.root");
      ...
      bool ECALToFAlg::ComputeEntryPoint()
      {
        ...
        ROOT::Math::WrappedFunction<std::function<float(float)>>
          helix_circle_intersections_wrapped_entry(helix_circle_intersections_to_wrap_entry);

        ROOT::Math::RootFinder rootFinderEntryBarrel(ROOT::Math::RootFinder::kBRENT);
        rootFinderEntryBarrel.SetFunction(
          helix_circle_intersections_wrapped_entry, fTrackPar[3], fPhiMax);
        ...
        try {
          ...
          TrackLengthExtraBarrel = TMath::Abs((fTrackPar[3] - root_entry_barrel) / fTrackPar[2]) *
                                   TMath::Sqrt(1 + TMath::Power(TMath::Tan(fTrackPar[4]), 2));
        }
        ...
        ROOT::Math::WrappedFunction<std::function<float(float)>>
          helix_x_intersections_wrapped_entry(helix_x_intersections_to_wrap_entry);

        ROOT::Math::RootFinder rootFinderEntryEndCap(ROOT::Math::RootFinder::kBRENT);
        rootFinderEntryEndCap.SetFunction(
          helix_x_intersections_wrapped_entry, fTrackPar[3], fPhiMax);
        ...
        try {
          ...
          TrackLengthExtraEndCap = TMath::Abs((fTrackPar[3] - root_entry_endcap) / fTrackPar[2]) *
                                   TMath::Sqrt(1 + TMath::Power(TMath::Tan(fTrackPar[4]), 2));
        }
        catch (const std::exception& excpt) {
        ...
        if ((finderStatusBarrel == false) && (finderStatusEndCap == false)) { return false; }
        ...
        if ((TrackLengthExtraBarrel <= TrackLengthExtraEndCap) && (finderStatusBarrel == true)) {
          ...
          for (int i = 0; i < 3; ++i)
            fEntryPoint[i] = EntryPointBarrel[i];
          fEntryPhi = root_entry_barrel;
        }
        else if ((TrackLengthExtraEndCap < TrackLengthExtraBarrel) &&
                 (finderStatusEndCap == true)) {
          // Using propagation to endcap
          fTrackLengthExtra = TrackLengthExtraEndCap;
          for (int i = 0; i < 3; ++i)
            fEntryPoint[i] = EntryPointEndCap[i];
      ...
      void ECALToFAlg::AddHits(std::vector<const rec::CaloHit*> hit_vec)
      {
        ...
        for (auto const& hit : hit_vec) {
          ...
          if (((DetID == 1) && (Layer < 9)) || ((DetID == 2) && (Layer < 7))) {
            if (fPropagateToEndCap) { PropagateHitEndCap(hit, Layer); }
            else {
      ...
      void ECALToFAlg::PropagateHitEndCap(const rec::CaloHit* hit, int Layer)
      {
        ...
        ROOT::Math::WrappedFunction<std::function<float(float)>> helix_x_intersections_wrapped(
          helix_x_intersections_to_wrap);
        ...
        try {
          ...
          distance_hit_projected =
            std::hypot(xHit - projected[0], yHit - projected[1], zHit - projected[2]);
          distance_projected_entry =
            TMath::Abs((fEntryPhi - root) / fTrackPar[2]) *
            TMath::Sqrt(1 + TMath::Power(TMath::Tan(fTrackPar[4]), 2)); // compute arc length
          ...
          if (fHitMap.find(Layer) == fHitMap.end()) {
            ...
            fHitMap[Layer] =
              std::make_tuple(distance_hit_projected, distance_projected_entry, tHit);
          }
          else {
            ...
            if (distance_hit_projected <= old_distance_hit_projected)
              fHitMap[Layer] =
                std::make_tuple(distance_hit_projected, distance_projected_entry, tHit);
          }
        }
      ...
      void ECALToFAlg::PropagateHitBarrel(const rec::CaloHit* hit, int Layer)
      {
        ...
        ROOT::Math::WrappedFunction<std::function<float(float)>> helix_circle_intersections_wrapped(
          helix_circle_intersections_to_wrap);
      ...
      void ECALToFAlg::ComputeArrivalTime()
      {
        ...
        if (fTimeMethod.compare("Earliest") == 0) {
          std::vector<float>::iterator time_min =
            std::min_element(time_vec.begin(), time_vec.end());
          float time_min_distance = distance_vec[std::distance(time_vec.begin(), time_min)];

          fTime = *time_min - time_min_distance / 300;
        }
        else if (fTimeMethod.compare("Average") == 0) {
          fTime = std::accumulate(time_corrected_vec.begin(), time_corrected_vec.end(), 0.0) /
                  time_corrected_vec.size();
        }
        else if (fTimeMethod.compare("Fit") == 0) {
      ...
      //----------------------------------------------------------------------------
      float ECALToFAlg::helix_circle_intersections(float phi,
                                                   float y0,
                                                   float z0,
                                                   float R,
                                                   float phi0,
                                                   float r)
      {
        return TMath::Power(y0 - R * (TMath::Cos(phi) - TMath::Cos(phi0)) - fTPCCent[1], 2) +
               TMath::Power(z0 + R * (TMath::Sin(phi) - TMath::Sin(phi0)) - fTPCCent[2], 2) -
               TMath::Power(r, 2);
      }

      //----------------------------------------------------------------------------
      float ECALToFAlg::helix_x_intersections(float phi,
                                              float x0,
                                              float t0,
                                              float R,
                                              float lambda0,
                                              ...
                                              float x)
      {
        return x0 + ((x0 > 0.) ? 1. : ((x0 < 0.) ? -1. : 0.)) * fDriftVelocity * t0 +
               R * TMath::Tan(lambda0) * (phi - phi0) - fTPCCent[0] - x;
      }

      //----------------------------------------------------------------------------
      void ECALToFAlg::helix(float phi,
                             float* trackPar,
                             float* trackEnd,
                             float* projected,
                             float t0)
      {
        ...
        projected[0] =
          trackEnd[0] +
          ((trackEnd[0] > 0.) ? 1. : ((trackEnd[0] < 0.) ? -1. : 0.)) * fDriftVelocity * t0 +
          (1 / trackPar[2]) * TMath::Tan(trackPar[4]) * (phi - trackPar[3]);
        projected[1] =
          trackPar[0] - (1 / trackPar[2]) * (TMath::Cos(phi) - TMath::Cos(trackPar[3]));
        projected[2] =
          trackPar[1] + (1 / trackPar[2]) * (TMath::Sin(phi) - TMath::Sin(trackPar[3]));
      }
      ...
      float ECALToFAlg::GetToFProtonScore()
      {
        ...
        for (auto& [key, clf] : fScorerMap) {
          ...
          if ((fTrackMomentum >= p_min) && (fTrackMomentum < p_max)) {
            ...
            ProtonScore = Sigmoid(-(fBeta - calibration.beta_max_f1) * 10,
