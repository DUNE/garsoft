  ...
  namespace rec {
    namespace alg {
      ...
      void ECALMuonBDT::Configure(fhicl::ParameterSet const& pset)
      {
        ...
        fBDTWeightDirectory =
          pset.get<std::string>("BDTWeightDirectory", "$GARSOFT_DIR/MVAData/weights");
        fBDTSummaryFileName = pset.get<std::string>(
          "BDTSummaryFileName",
          "/pnfs/dune/persistent/users/fmlopez/GAr/MVAData/gar_bdt_summary_v00_01_00.root");
      ...
      void ECALMuonBDT::LoadClassifiers()
      {
        ...
        for (const auto& entry :
             std::filesystem::directory_iterator(Wildcard(fBDTWeightDirectory).at(0))) {
          ...
          if (std::stof(p0) >= fMaxMomentumECALOnly) {
            ...
            reader->AddVariable("ClusterTotalEnergy/RecoMomentumFWD",
                                &_ClusterTotalEnergyOverRecoMomentumFWD);
            reader->AddVariable("ClusterMuIDTotalEnergy/RecoMomentumFWD",
                                &_ClusterMuIDTotalEnergyOverRecoMomentumFWD);
          }
          else {
            reader->AddVariable("ClusterTotalEnergy", &_ClusterTotalEnergy);
            reader->AddVariable("HitMeanEnergy", &_HitMeanEnergy);
      ...
      //----------------------------------------------------------------------------
      void ECALMuonBDT::AddECALHits(const rec::Cluster* ecal_cluster,
                                    std::vector<const rec::CaloHit*> ecal_hit_vec)
      {

        fECALTotalEnergy += ecal_cluster->Energy();

        float ClusterX = ecal_cluster->Position()[0];
        float ClusterY = ecal_cluster->Position()[1];
        float ClusterZ = ecal_cluster->Position()[2];
        std::vector<float> ClusterPos{ClusterX, ClusterY, ClusterZ};

        float ClusterMainAxisX = ecal_cluster->EigenVectors()[0];
        float ClusterMainAxisY = ecal_cluster->EigenVectors()[1];
        float ClusterMainAxisZ = ecal_cluster->EigenVectors()[2];
        std::vector<float> ClusterMainAxis{
          ClusterX + ClusterMainAxisX, ClusterY + ClusterMainAxisY, ClusterZ + ClusterMainAxisZ};

        for (auto const& ecal_hit : ecal_hit_vec) {
          fECALHitEnergy.push_back(ecal_hit->Energy());

          float HitX = ecal_hit->Position()[0];
          float HitY = ecal_hit->Position()[1];
          float HitZ = ecal_hit->Position()[2];
          std::vector<float> HitPos{HitX, HitY, HitZ};

          std::vector<float> HitDiff1;
          std::transform(HitPos.begin(),
                         HitPos.end(),
                         ClusterPos.begin(),
                         std::back_inserter(HitDiff1),
                         std::minus<float>());
          std::vector<float> HitDiff2;
          std::transform(HitPos.begin(),
                         HitPos.end(),
                         ClusterMainAxis.begin(),
                         std::back_inserter(HitDiff2),
                         std::minus<float>());

          std::vector<float> HitDistVec = CrossProduct1D(HitDiff1, HitDiff2);
          fECALHitDistCluster.push_back(std::sqrt(
            std::inner_product(HitDistVec.begin(), HitDistVec.end(), HitDistVec.begin(), 0.0)));

          std::vector<float> HitDistCent;
          std::transform(HitPos.begin(),
                         HitPos.end(),
                         fTPCCent.begin(),
                         std::back_inserter(HitDistCent),
                         std::minus<float>());
          fECALHitDistCentre.push_back(std::sqrt(
            std::inner_product(HitDistCent.begin(), HitDistCent.end(), HitDistCent.begin(), 0.0)));
      ...
      //----------------------------------------------------------------------------
      void ECALMuonBDT::AddMuIDHits(const rec::Cluster* muid_cluster,
                                    std::vector<const rec::CaloHit*> muid_hit_vec)
      {
        ...
        for (auto const& muid_hit : muid_hit_vec) {
          ...
          float HitMuIDX = muid_hit->Position()[0];
          float HitMuIDY = muid_hit->Position()[1];
          float HitMuIDZ = muid_hit->Position()[2];
          std::vector<float> HitMuIDPos{HitMuIDX, HitMuIDY, HitMuIDZ};

          fMuIDHitPos.push_back(HitMuIDPos);

          std::vector<float> HitDistMuIDCent;
          std::transform(HitMuIDPos.begin(),
                         HitMuIDPos.end(),
                         fTPCCent.begin(),
                         std::back_inserter(HitDistMuIDCent),
                         std::minus<float>());
          fMuIDHitDistCentre.push_back(std::sqrt(std::inner_product(
            HitDistMuIDCent.begin(), HitDistMuIDCent.end(), HitDistMuIDCent.begin(), 0.0)));
      ...
      void ECALMuonBDT::ComputeFeatures()
      {
        ...
        if (fNECALHits > 0) {
          ...
          fECALHitDistClusterMean =
            std::accumulate(fECALHitDistCluster.begin(), fECALHitDistCluster.end(), 0.0) /
            fECALHitDistCluster.size();
          fECALHitDistClusterRMS = std::sqrt(std::inner_product(fECALHitDistCluster.begin(),
                                                                fECALHitDistCluster.end(),
                                                                fECALHitDistCluster.begin(),
                                                                0.0) /
                                             fECALHitDistCluster.size());

          // Find min and max values of the distance between the hits and the center of the TPC
          fECALHitDistCentreMin =
            *std::min_element(fECALHitDistCentre.begin(), fECALHitDistCentre.end());
          fECALHitDistCentreMax =
            *std::max_element(fECALHitDistCentre.begin(), fECALHitDistCentre.end());
          ...
          if (fNECALHits > 1) {
            TGraph* graph =
              new TGraph(fECALHitTime.size(), &fECALHitTime[0], &fECALHitDistCentre[0]);
            TF1* linearFit =
              new TF1("linearFit", "pol1", fECALHitDistCentreMin, fECALHitDistCentreMax);
            graph->Fit(linearFit, "Q");
            fECALToFVelocity = linearFit->GetParameter(1);
          }

          // Find mean, std and max of the hit energy
          fECALHitEnergyMean = std::accumulate(fECALHitEnergy.begin(), fECALHitEnergy.end(), 0.0) /
                               fECALHitEnergy.size();
          fECALHitEnergyVar =
            std::accumulate(fECALHitEnergy.begin(),
                            fECALHitEnergy.end(),
                            0.0,
                            [&](float accumulator, const float& val) {
                              return accumulator + ((val - fECALHitEnergyMean) *
                                                    (val - fECALHitEnergyMean) / (fNECALHits - 1));
                            });
          ...
          std::sort(indices.begin(), indices.end(), [&](int A, int B) -> bool {
            return fECALHitDistCluster[A] < fECALHitDistCluster[B];
          });
          ...
          fECALHitDistCentroid =
            std::inner_product(
              fECALHitDistCentre.begin(), fECALHitDistCentre.end(), fECALHitEnergy.begin(), 0.0) /
            std::accumulate(fECALHitEnergy.begin(), fECALHitEnergy.end(), 0.0);
          fECALHitTimeCentroid =
            std::inner_product(
              fECALHitTime.begin(), fECALHitTime.end(), fECALHitEnergy.begin(), 0.0) /
            std::accumulate(fECALHitEnergy.begin(), fECALHitEnergy.end(), 0.0);
        }
        ...
        if (fNMuIDHits > 0) {
          ...
          for (size_t m = 0; m < fMuIDHitPos.size(); m++) {
            for (size_t n = 0; n < fMuIDHitPos.size(); n++) {
              ...
              std::transform(fMuIDHitPos[m].begin(),
                             fMuIDHitPos[m].end(),
                             fMuIDHitPos[n].begin(),
                             std::back_inserter(HitDistMuID),
                             std::minus<float>());
              float distHit_muid = std::sqrt(std::inner_product(
                HitDistMuID.begin(), HitDistMuID.end(), HitDistMuID.begin(), 0.0));
          ...
          fMuIDHitDistCentreMax =
            *std::max_element(fMuIDHitDistCentre.begin(), fMuIDHitDistCentre.end());

          // Find mean, std and max of the hit energy
          fMuIDHitEnergyMean = std::accumulate(fMuIDHitEnergy.begin(), fMuIDHitEnergy.end(), 0.0) /
                               fMuIDHitEnergy.size();
          fMuIDHitEnergyVar =
            std::accumulate(fMuIDHitEnergy.begin(),
                            fMuIDHitEnergy.end(),
                            0.0,
                            [&](float accumulator, const float& val) {
                              return accumulator + ((val - fMuIDHitEnergyMean) *
                                                    (val - fMuIDHitEnergyMean) / (fNMuIDHits - 1));
                            });
          ...
          // Compute energy-weighted centroids for distance and time of ECAL hits
          fMuIDHitDistCentroid =
            std::inner_product(
              fMuIDHitDistCentre.begin(), fMuIDHitDistCentre.end(), fMuIDHitEnergy.begin(), 0.0) /
            std::accumulate(fMuIDHitEnergy.begin(), fMuIDHitEnergy.end(), 0.0);
          fMuIDHitTimeCentroid =
            std::inner_product(
              fMuIDHitLayer.begin(), fMuIDHitLayer.end(), fMuIDHitEnergy.begin(), 0.0) /
            std::accumulate(fMuIDHitEnergy.begin(), fMuIDHitEnergy.end(), 0.0);

          // Use centroids as a measure of velocity
          fMuIDToFVelocity = (fMuIDHitDistCentroid - fECALHitDistCentroid) /
                             (fMuIDHitTimeCentroid - fECALHitTimeCentroid);
        }
      ...
      //----------------------------------------------------------------------------
      float ECALMuonBDT::ReEvaluateTMVA(float x,
                                        float learning_rate,
                                        float n_estimators,
                                        float a,
                                        float b)
