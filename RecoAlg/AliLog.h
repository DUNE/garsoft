...
public:
  ...
  static void Message(UInt_t level,
                      const char* message,
                      const char* module,
                      ...
                      const char* file,
                      Int_t line);
  static void Debug(UInt_t level,
                    const char* message,
                    const char* module,
                    const char* className,
                    const char* function,
  ...
  static Int_t RedirectStdoutTo(EType_t type,
                                UInt_t level,
                                const char* module,
                                const char* className,
                                const char* function,
                                ...
                                Int_t line,
                                Bool_t print);
  static Int_t RedirectStderrTo(EType_t type,
                                UInt_t level,
                                const char* module,
                                const char* className,
                                const char* function,
  ...
  static ostream& Stream(EType_t type,
                         UInt_t level,
                         const char* module,
                         const char* className,
                         const char* function,
...
private:
  ...
  static void RootErrorHandler(Int_t level,
                               Bool_t abort,
                               const char* location,
                               const char* message);
  ...
  ostream& GetStream(EType_t type,
                     UInt_t level,
                     const char* module,
                     const char* className,
                     const char* function,
...
#define REDIRECTSTDOUT(type, level, scope, whatever)                                               \
  do {                                                                                             \
    Int_t originalStdout = AliLog::RedirectStdoutTo(                                               \
      type, level, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__, kFALSE);               \
    whatever;                                                                                      \
    AliLog::RestoreStdout(originalStdout);                                                         \
  } while (false)
/**
...
#define REDIRECTSTDERR(type, level, scope, whatever)                                               \
  do {                                                                                             \
    Int_t originalStderr = AliLog::RedirectStderrTo(                                               \
      type, level, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__, kFALSE);               \
    whatever;                                                                                      \
    AliLog::RestoreStderr(originalStderr);                                                         \
  } while (false)
/**
...
#define AliDebugLevel()                                                                            \
  ((AliLog::IsDebugEnabled()) ? AliLog::GetDebugLevel(MODULENAME(), ClassName()) : -1)
/**
 * Get the class (ROOT-enabled) debug level
 */
#define AliDebugLevelClass()                                                                       \
  ((AliLog::IsDebugEnabled()) ? AliLog::GetDebugLevel(MODULENAME(), Class()->GetName()) : -1)
/**
 * Get the debug level associated with scope
 * @param scope Scope
 */
#define AliDebugLevelGeneral(scope)                                                                \
  ((AliLog::IsDebugEnabled()) ? AliLog::GetDebugLevel(MODULENAME(), scope) : -1)
#endif
...
#define AliDebugF(level, message, ...)                                                             \
  do {                                                                                             \
  } while (false)
#define AliDebugClassF(level, message, ...)                                                        \
  do {                                                                                             \
  } while (false)
#define AliDebugGeneralF(scope, level, message, ...)                                               \
  do {                                                                                             \
  } while (false)
#else
...
#define AliDebug(logLevel, message)                                                                \
  do {                                                                                             \
    if (ALIROOT_UNLIKELY(AliLog::IsDebugEnabled() &&                                               \
                         AliLog::GetDebugLevel(MODULENAME(), ClassName()) >= logLevel)) {          \
      AliLog::Debug(                                                                               \
        logLevel, message, MODULENAME(), ClassName(), FUNCTIONNAME(), __FILE__, __LINE__);         \
    }                                                                                              \
  } while (0)
/**
 *
...
#define AliDebugClass(logLevel, message)                                                           \
  do {                                                                                             \
    if (ALIROOT_UNLIKELY(AliLog::IsDebugEnabled() &&                                               \
                         AliLog::GetDebugLevel(MODULENAME(), Class()->GetName()) >= logLevel)) {   \
      AliLog::Debug(                                                                               \
        logLevel, message, MODULENAME(), Class()->GetName(), FUNCTIONNAME(), __FILE__, __LINE__);  \
    }                                                                                              \
  } while (0)
...
#define AliDebugGeneral(scope, logLevel, message)                                                  \
  do {                                                                                             \
    if (ALIROOT_UNLIKELY(AliLog::IsDebugEnabled() &&                                               \
                         AliLog::GetDebugLevel(MODULENAME(), scope) >= logLevel)) {                \
      AliLog::Debug(logLevel, message, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__);   \
    }                                                                                              \
  } while (0)
/**
...
#define AliDebugF(logLevel, format, ...)                                                           \
  do {                                                                                             \
    if (ALIROOT_UNLIKELY(AliLog::IsDebugEnabled() &&                                               \
                         AliLog::GetDebugLevel(MODULENAME(), ClassName()) >= logLevel)) {          \
      TString m;                                                                                   \
      m.Form(format, __VA_ARGS__);                                                                 \
      AliLog::Debug(logLevel, m, MODULENAME(), ClassName(), FUNCTIONNAME(), __FILE__, __LINE__);   \
    }                                                                                              \
  } while (0)
/**
...
#define AliDebugClassF(logLevel, format, ...)                                                      \
  do {                                                                                             \
    if (ALIROOT_UNLIKELY(AliLog::IsDebugEnabled() &&                                               \
      ...
      m.Form(format, __VA_ARGS__);                                                                 \
      AliLog::Debug(                                                                               \
        logLevel, m, MODULENAME(), Class()->GetName(), FUNCTIONNAME(), __FILE__, __LINE__);        \
    }                                                                                              \
  } while (0)
/**
 * Outut debug information, filtered on debug level.  For use in
...
#define AliDebugGeneralF(scope, logLevel, format, ...)                                             \
  do {                                                                                             \
    if (ALIROOT_UNLIKELY(AliLog::IsDebugEnabled() &&                                               \
      ...
      TString m;                                                                                   \
      m.Form(format, __VA_ARGS__);                                                                 \
      AliLog::Debug(logLevel, m, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__);         \
    }                                                                                              \
  } while (0)
...
#define StderrToAliDebug(level, whatever)                                                          \
...
#define StdoutToAliDebugClass(level, whatever)                                                     \
  REDIRECTSTDOUT(AliLog::kDebug, level, Class()->GetName(), whatever)
#define StderrToAliDebugClass(level, whatever)                                                     \
  REDIRECTSTDERR(AliLog::kDebug, level, Class()->GetName(), whatever)
#define ToAliDebugClass(level, whatever)                                                           \
  REDIRECTSTDOUTANDSTDERR(AliLog::kDebug, level, Class()->GetName(), whatever)
#define StdoutToAliDebugGeneral(scope, level, whatever)                                            \
  REDIRECTSTDOUT(AliLog::kDebug, level, scope, whatever)
#define StderrToAliDebugGeneral(scope, level, whatever)                                            \
  REDIRECTSTDERR(AliLog::kDebug, level, scope, whatever)
#define ToAliDebugGeneral(scope, level, whatever)                                                  \
  REDIRECTSTDOUTANDSTDERR(AliLog::kDebug, level, scope, whatever)

// debug stream objects
#define AliDebugStream(level)                                                                      \
  AliLog::Stream(                                                                                  \
    AliLog::kDebug, level, MODULENAME(), ClassName(), FUNCTIONNAME(), __FILE__, __LINE__)
#define AliDebugClassStream(level)                                                                 \
  AliLog::Stream(                                                                                  \
    AliLog::kDebug, level, MODULENAME(), Class()->GetName(), FUNCTIONNAME(), __FILE__, __LINE__)
#define AliDebugGeneralStream(scope, level)                                                        \
...
#define AliMessage(lvl, message)                                                                   \
  do {                                                                                             \
    AliLog::Message(lvl, message, MODULENAME(), ClassName(), FUNCTIONNAME(), __FILE__, __LINE__);  \
  } while (false)
/**
 * Macro that will output stuff using the logging facilities.
...
#define AliMessageClass(lvl, message)                                                              \
  do {                                                                                             \
    AliLog::Message(                                                                               \
      lvl, message, MODULENAME(), Class()->GetName(), FUNCTIONNAME(), __FILE__, __LINE__);         \
  } while (false)
/**
 * Macro that will output stuff using the logging facilities.
...
#define AliMessageGeneral(scope, lvl, message)                                                     \
  do {                                                                                             \
    AliLog::Message(lvl, message, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__);        \
  } while (false)
/**
...
#define AliMessageF(lvl, format, ...)                                                              \
  do {                                                                                             \
    TString m;                                                                                     \
    m.Form(format, __VA_ARGS__);                                                                   \
    AliLog::Message(lvl, m, MODULENAME(), ClassName(), FUNCTIONNAME(), __FILE__, __LINE__);        \
  } while (false)
/**
 * Print a message using the AliLog logging facility. This macro
 * accepts printf-like format arguments.  Note, at least 3 arguments
...
#define AliMessageClassF(lvl, format, ...)                                                         \
  do {                                                                                             \
    TString m;                                                                                     \
    m.Form(format, __VA_ARGS__);                                                                   \
    AliLog::Message(lvl, m, MODULENAME(), Class()->GetName(), FUNCTIONNAME(), __FILE__, __LINE__); \
  } while (false)
/**
 * Print a message using the AliLog logging facility. This macro
 * accepts printf-like format arguments.  Note, at least 3 arguments
...
#define AliMessageGeneralF(scope, lvl, format, ...)                                                \
  do {                                                                                             \
    TString m;                                                                                     \
    m.Form(format, __VA_ARGS__);                                                                   \
    AliLog::Message(lvl, m, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__);              \
  } while (false)
...
#define AliInfoGeneralF(scope, message, ...)                                                       \
  do {                                                                                             \
  } while (false)
#else
/**
...
#define StdoutToAliInfoClass(whatever)                                                             \
  REDIRECTSTDOUT(AliLog::kInfo, 0, Class()->GetName(), whatever)
#define StderrToAliInfoClass(whatever)                                                             \
  REDIRECTSTDERR(AliLog::kInfo, 0, Class()->GetName(), whatever)
#define ToAliInfoClass(whatever)                                                                   \
  REDIRECTSTDOUTANDSTDERR(AliLog::kInfo, 0, Class()->GetName(), whatever)
...
// info stream objects
#define AliInfoStream()                                                                            \
  AliLog::Stream(AliLog::kInfo, 0, MODULENAME(), ClassName(), FUNCTIONNAME(), __FILE__, __LINE__)
#define AliInfoClassStream()                                                                       \
  AliLog::Stream(                                                                                  \
    AliLog::kInfo, 0, MODULENAME(), Class()->GetName(), FUNCTIONNAME(), __FILE__, __LINE__)
#define AliInfoGeneralStream(scope)                                                                \
  AliLog::Stream(AliLog::kInfo, 0, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__)
...
#define AliWarningGeneralF(scope, message, ...)                                                    \
  do {                                                                                             \
  } while (false)
#else
/**
...
#define StdoutToAliWarningClass(whatever)                                                          \
  REDIRECTSTDOUT(AliLog::kWarning, 0, Class()->GetName(), whatever)
#define StderrToAliWarningClass(whatever)                                                          \
  REDIRECTSTDERR(AliLog::kWarning, 0, Class()->GetName(), whatever)
#define ToAliWarningClass(whatever)                                                                \
  REDIRECTSTDOUTANDSTDERR(AliLog::kWarning, 0, Class()->GetName(), whatever)
#define StdoutToAliWarningGeneral(scope, whatever)                                                 \
  REDIRECTSTDOUT(AliLog::kWarning, 0, scope, whatever)
#define StderrToAliWarningGeneral(scope, whatever)                                                 \
  REDIRECTSTDERR(AliLog::kWarning, 0, scope, whatever)
#define ToAliWarningGeneral(scope, whatever)                                                       \
  REDIRECTSTDOUTANDSTDERR(AliLog::kWarning, 0, scope, whatever)

// warning stream objects
#define AliWarningStream()                                                                         \
  AliLog::Stream(AliLog::kWarning, 0, MODULENAME(), ClassName(), FUNCTIONNAME(), __FILE__, __LINE__)
#define AliWarningClassStream()                                                                    \
  AliLog::Stream(                                                                                  \
    AliLog::kWarning, 0, MODULENAME(), Class()->GetName(), FUNCTIONNAME(), __FILE__, __LINE__)
#define AliWarningGeneralStream(scope)                                                             \
  AliLog::Stream(AliLog::kWarning, 0, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__)
 ...
 * @see AliMessageGeneralF
 */
#define AliErrorGeneralF(scope, message, ...)                                                      \
...
#define StdoutToAliErrorClass(whatever)                                                            \
  REDIRECTSTDOUT(AliLog::kError, 0, Class()->GetName(), whatever)
#define StderrToAliErrorClass(whatever)                                                            \
  REDIRECTSTDERR(AliLog::kError, 0, Class()->GetName(), whatever)
#define ToAliErrorClass(whatever)                                                                  \
  REDIRECTSTDOUTANDSTDERR(AliLog::kError, 0, Class()->GetName(), whatever)
...
#define ToAliErrorGeneral(scope, whatever)                                                         \
  REDIRECTSTDOUTANDSTDERR(AliLog::kError, 0, scope, whatever)

// error stream objects
#define AliErrorStream()                                                                           \
  AliLog::Stream(AliLog::kError, 0, MODULENAME(), ClassName(), FUNCTIONNAME(), __FILE__, __LINE__)
#define AliErrorClassStream()                                                                      \
  AliLog::Stream(                                                                                  \
    AliLog::kError, 0, MODULENAME(), Class()->GetName(), FUNCTIONNAME(), __FILE__, __LINE__)
#define AliErrorGeneralStream(scope)                                                               \
  AliLog::Stream(AliLog::kError, 0, MODULENAME(), scope, FUNCTIONNAME(), __FILE__, __LINE__)
 ...
 * @see AliMessageGeneralF
 */
#define AliFatalGeneralF(scope, message, ...)                                                      \
