...
AliExternalTrackParam::AliExternalTrackParam() : AliVTrack(), fX(0), fAlpha(0)
{
  //
  // default constructor
  //
...
AliExternalTrackParam::AliExternalTrackParam(const AliExternalTrackParam& track)
  : AliVTrack(track), fX(track.fX), fAlpha(track.fAlpha)
{
  //
  // copy constructor
  //
...
//_____________________________________________________________________________
void AliExternalTrackParam::Set(Double_t xyz[3], Double_t pxpypz[3], Double_t cv[21], Short_t sign)
{
  //
  // create external track parameters from the global parameters
  // x,y,z,px,py,pz and their 6x6 covariance matrix
  ...
  if (radPos2 < radMax * radMax) { // inside the ITS
    ...
    Float_t phiPos = TMath::Pi() + TMath::ATan2(-xyz[1], -xyz[0]);
    fAlpha = TMath::DegToRad() * (20 * ((((Int_t)(phiPos * TMath::RadToDeg())) / 20)) + 10);
  }
  //
  Double_t cs = TMath::Cos(fAlpha), sn = TMath::Sin(fAlpha);
  // protection:  avoid alpha being too close to 0 or +-pi/2
  if (TMath::Abs(sn) < 2 * kSafe) {
    if (fAlpha > 0)
      fAlpha += fAlpha < TMath::Pi() / 2. ? 2 * kSafe : -2 * kSafe;
    else
      fAlpha += fAlpha > -TMath::Pi() / 2. ? -2 * kSafe : 2 * kSafe;
    cs = TMath::Cos(fAlpha);
    sn = TMath::Sin(fAlpha);
  ...
  fP[4] = TMath::Sign(1 / mom.Pt(), charge);
  //
  if (TMath::Abs(1 - fP[2]) < kSafe)
    fP[2] = 1. - kSafe; //Protection
  else if (TMath::Abs(-1 - fP[2]) < kSafe)
    fP[2] = -1. + kSafe; //Protection
  //
  // Covariance matrix (formulas to be simplified)
  Double_t pt = 1. / TMath::Abs(fP[4]);
...
Double_t AliExternalTrackParam::GetD(Double_t x, Double_t y, Double_t b) const
{
  ...
  sn = rp4 * xt - fP[2];
  cs = rp4 * yt + TMath::Sqrt((1. - fP[2]) * (1. + fP[2]));
...
Bool_t AliExternalTrackParam::Invert()
{
  ...
  //
  fC[1] =
    -fC[1]; // since the fP1 and fP2 are not inverted, their covariances with others change sign
  fC[3] = -fC[3];
  fC[7] = -fC[7];
  fC[8] = -fC[8];
  fC[11] = -fC[11];
...
Bool_t AliExternalTrackParam::PropagateTo(Double_t xk, Double_t b)
{
  ...
  Double_t &fC00 = fC[0], &fC10 = fC[1], &fC11 = fC[2], &fC20 = fC[3], &fC21 = fC[4], &fC22 = fC[5],
           &fC30 = fC[6], &fC31 = fC[7], &fC32 = fC[8], &fC33 = fC[9], &fC40 = fC[10],
           &fC41 = fC[11], &fC42 = fC[12], &fC43 = fC[13], &fC44 = fC[14];
  ...
  if (TMath::Abs(x2r) < 0.05)
    ...
    if (f1 * f1 + f2 * f2 > 1 &&
        f1 * f2 < 0) { // special cases of large rotations or large abs angles
      if (f2 > 0)
        rot = TMath::Pi() - rot; //
      else
        rot = -TMath::Pi() - rot;
...
Bool_t AliExternalTrackParam::PropagateBxByBz(Double_t alpha, Double_t x, Double_t b[3])
{
  //------------------------------------------------------------------
  // Transform this track to the local coord. system rotated
  // by angle "alpha" (rad) with respect to the global coord. system,
...
void AliExternalTrackParam::Propagate(Double_t len, Double_t x[3], Double_t p[3], Double_t bz) const
{
  //+++++++++++++++++++++++++++++++++++++++++
  ...
  if (OneOverPt() < kAlmost0 || TMath::Abs(bz) < kAlmost0Field ||
      GetC(bz) < kAlmost0) { //straight-line tracks
    Double_t unit[3];
    GetDirection(unit);
    x[0] += unit[0] * len;
    x[1] += unit[1] * len;
...
Bool_t AliExternalTrackParam::Intersect(Double_t pnt[3], Double_t norm[3], Double_t bz) const
{
  //+++++++++++++++++++++++++++++++++++++++++
  // Origin: K. Shileev (Kirill.Shileev@cern.ch)
  ...
  //+++++++++++++++++++++++++++++++++++++++++
  Double_t x0[3];
  GetXYZ(x0); //get track position in MARS
...
Double_t AliExternalTrackParam::GetPredictedChi2(const Double_t p[2], const Double_t cov[3]) const
{
  //----------------------------------------------------------------
                                                 ...
                                                 const Double_t covxyz[3]) const
{
  ...
  Double_t res[3] = {GetX() - p[0], GetY() - p[1], GetZ() - p[2]};
  ...
  ;
  v(1, 0) = a * s2;
  v(1, 1) = a * a * s2 + GetSigmaY2();
  v(1, 2) = a * b * s2 + GetSigmaZY();
  v(2, 0) = b * s2;
  v(2, 1) = a * b * s2 + GetSigmaZY();
  v(2, 2) = b * b * s2 + GetSigmaZ2();
...
Double_t AliExternalTrackParam::GetPredictedChi2(const AliExternalTrackParam* t) const
{
  ...
  c(3, 0) = GetSigmaTglY();
  c(3, 1) = GetSigmaTglZ();
  c(3, 2) = GetSigmaTglSnp();
  c(3, 3) = GetSigmaTgl2();
  c(4, 0) = GetSigma1PtY();
  c(4, 1) = GetSigma1PtZ();
  c(4, 2) = GetSigma1PtSnp();
  c(4, 3) = GetSigma1PtTgl();
  c(4, 4) = GetSigma1Pt2();

  c(0, 0) += t->GetSigmaY2();
  c(1, 0) += t->GetSigmaZY();
  c(1, 1) += t->GetSigmaZ2();
  c(2, 0) += t->GetSigmaSnpY();
  c(2, 1) += t->GetSigmaSnpZ();
  c(2, 2) += t->GetSigmaSnp2();
  c(3, 0) += t->GetSigmaTglY();
  c(3, 1) += t->GetSigmaTglZ();
  c(3, 2) += t->GetSigmaTglSnp();
  c(3, 3) += t->GetSigmaTgl2();
  c(4, 0) += t->GetSigma1PtY();
  c(4, 1) += t->GetSigma1PtZ();
  c(4, 2) += t->GetSigma1PtSnp();
  c(4, 3) += t->GetSigma1PtTgl();
  c(4, 4) += t->GetSigma1Pt2();
  c(0, 1) = c(1, 0);
  c(0, 2) = c(2, 0);
  c(1, 2) = c(2, 1);
  c(0, 3) = c(3, 0);
  ...
  for (Int_t i = 0; i < 5; i++)
    for (Int_t j = 0; j < 5; j++)
      chi2 += res[i] * res[j] * c(i, j);

  return chi2;
}

Bool_t AliExternalTrackParam::PropagateTo(Double_t p[3],
                                          Double_t covyz[3],
                                          Double_t covxyz[3],
                                          Double_t bz)
{
...
Double_t* AliExternalTrackParam::GetResiduals(Double_t* p, Double_t* cov, Bool_t updated) const
{
  //------------------------------------------------------------------
...
void AliExternalTrackParam::GetHelixParameters(Double_t hlx[6], Double_t b) const
{
  //--------------------------------------------------------------------
...
static void Evaluate(const Double_t* h,
                     Double_t t,
                                       ...
                                       Double_t& xp) const
{
  ...
  while (max--) {
    ...
    Double_t h11 = (g1[0] * g1[0] - dx * gg1[0]) / dx2 + (g1[1] * g1[1] - dy * gg1[1]) / dy2 +
                   (g1[2] * g1[2] - dz * gg1[2]) / dz2;
    Double_t h22 = (g2[0] * g2[0] + dx * gg2[0]) / dx2 + (g2[1] * g2[1] + dy * gg2[1]) / dy2 +
    ...
    if (TMath::Abs(dt1) / (TMath::Abs(t1) + 1.e-3) < 1.e-4)
      if (TMath::Abs(dt2) / (TMath::Abs(t2) + 1.e-3) < 1.e-4) {
        ...
        if (lmb < 0.) AliDebug(1, " stopped at not a minimum !");
        break;
      }
...
Bool_t AliExternalTrackParam::GetYZAt(Double_t x, Double_t b, Double_t* yz) const
{
  ...
  if (TMath::Abs(f1) >= kAlmost1 || TMath::Abs(f2) >= kAlmost1 || TMath::Abs(fP[4]) < kAlmost0)
    return kFALSE;
  double r1 = TMath::Sqrt((1. - f1) * (1. + f1)), r2 = TMath::Sqrt((1. - f2) * (1. + f2));
  double dy2dx = (f1 + f2) / (r1 + r2);
  yz[0] = fP[0] + dx * dy2dx;
  if (TMath::Abs(x2r) < 0.05)
    yz[1] = fP[1] + dx * (r2 + f2 * dy2dx) * fP[3];
  else {
    double chord = dx * TMath::Sqrt(1 + dy2dx * dy2dx); // distance from old position to new one
    double rot =
...
void AliExternalTrackParam::g3helx3(Double_t qfield, Double_t step, Double_t vect[7])
{
  /******************************************************************
 *                                                                *
...
Bool_t AliExternalTrackParam::PropagateToBxByBz(Double_t xk, const Double_t b[3])
{
  ...
  if (TMath::Abs(dx) > 1e5 || TMath::Abs(GetY()) > 1e5 || TMath::Abs(GetZ()) > 1e5) {
  ...
  */
  ...
  double dy2dx = (f1 + f2) / (r1 + r2);
  Double_t step = (TMath::Abs(x2r) < 0.05) ?
                    dx * TMath::Abs(r2 + f2 * dy2dx) // chord
                    :
  ...
  Double_t t = cosalp * r[0] - sinalp * r[1];
...
Bool_t AliExternalTrackParam::ConstrainToVertex(const AliVVertex* vtx, Double_t b[3])
{
  ...
  Double_t dz[2], cov[3];
  if (!PropagateToDCABxByBz(vtx, b, 3, dz, cov)) return kFALSE;
  ...
  if (TMath::Abs(crv) > kAlmost0) { // helix
    ...
    if (TMath::Abs(y0) > kAlmost0) { // when y0==0 the x,y is unique
      ...
      if (dir == 0) { // chose the one which corresponds to smallest step
        double delta = (x - fX) * dfx -
                       (y - fy) * dfy; // the choice of + in C will lead to smaller step if delta<0
        if (delta < 0)
          x += dfx;
        else
          x -= dfx;
        ...
        if (TMath::Abs(dfeps) < kEps && // are we already in right r?
            TMath::Abs(fX * fX + fy * fy - r * r) < kEps)
          return fX;
        x += dfx + dfx;
    ...
    if (TMath::Abs(sn) >= kAlmost1) { // || to Y axis
      ...
      if (dir > 0) { // along the track direction
        if (sn > 0) {
          if (fy > det) return kFALSE;
        } // track is along Y axis and above the circle
        else {
          if (fy < -det) return kFALSE;
        ...
        if (sn > 0) {
          if (fy < -det) return kFALSE;
        } // track is along Y axis
        else if (fy > det)
          return kFALSE; // track is against Y axis
      }
    }
      ...
      if (!dir) {
        ...
        if (fX > det)
          return kFALSE; // current point is in on the right from the circle
        else if (fX < -det)
          x = -det; // on the left
        else
          x = det; // within the circle
      }
      ...
      if (dir == 0)
        ...
        if (t >= -det)
          ...
          return kFALSE; // both solutions have negative t
      }
      else { // go in increasing fX direction. (t+-det < 0)
        if (t < det)
          t -= det; // take minimal step giving t<0
        else
          return kFALSE; // both solutions have positive t
      }
      x = fX + cs * t;
    }
  }
                                        ...
                                        Double_t* alpSect) const
{
  ...
  if (alpSect) {
    ...
    while (1) {
      ...
      if ((cs * ca + sn * sa) < 0) {
        AliDebug(1,
                 Form("Rotation to target sector impossible: local cos(phi) would become %.2f",
                      cs * ca + sn * sa));
        return kFALSE;
      }
      //
      ...
      if (TMath::Abs(f1) >= kAlmost1) {
        AliDebug(
          1, Form("Rotation to target sector impossible: local sin(phi) would become %.2f", f1));
        return kFALSE;
      }
      //
...
Double_t AliExternalTrackParam::GetParameterAtRadius(Double_t r, Double_t bz, Int_t parType) const
{
  ...
  if (parType < 3) { return xyz[parType]; }
  ...
  if (parType >= 3 && parType < 6) { return pxyz[parType % 3]; }
  return 0;
}
