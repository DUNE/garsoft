                                          ...
                                          const char* branchName)
{
  ...
  //Bool_t keysOK=kTRUE;
  for (Int_t i = 0; i < keys->GetEntries();
       i++) { // check TProcesIDs as they are always used in TTree merging (sometimes failed)
    TKey* key = (TKey*)keys->At(i);
    if (TString(key->GetClassName()) == "TProcessID") {
      TObject* o = file->Get(key->GetName());
      if (TString(o->GetName()).Length() == 0) {
        ::Error("AliXRDPROOFtoolkit::CheckTreeInFile()",
                "Wring process ID:%s\t%s\n",
                fileName,
                key->GetName());
        //keysOK=kFALSE;
        delete file;
...
Bool_t AliXRDPROOFtoolkit::FilterList(const char* inputList, const char* fileList, Int_t checkLevel)
{
  ...
  while (finput.good()) {
    ...
    if (status == 0) { focGood << currentFile << endl; }
    else {
...

  {
    for (Int_t itree = 0; itree < nTrees; itree++) {
      ...
      if (treeName.Length() > 0) {
        joinTree->Branch(
          Form("%s.status", treeName.Data()), &status[itree], Form("%s.status/I", treeName.Data()));
      }
      else {
  ...
  Int_t nbranches = 0;
  {
    while (fin.good()) {
      ...
      if (nbranches != nbranchesCurrent) {
        foutLog << "Error  tree layout\t" << currentFile << "   \t" << treeIn << "  \t" << nbranches
                << "  \t" << nbranchesCurrent << "\n";
        cout << "Error tree layout\t" << currentFile << "   \t" << treeIn << "  \t" << nbranches
             << "  \t" << nbranchesCurrent << "\n";
      }
      counter++;
    }
...
Int_t AliXRDPROOFtoolkit::TestFile(const char* fileName, const char* keyNames)
{
  ...
  for (Int_t ikey = 0; ikey < list->GetEntries(); ikey++) {
    ...
    for (Int_t jkey = 0; jkey < keyMaskArray->GetEntriesFast(); jkey++) {
      ...
      if (reg.Match(list->At(ikey)->GetName())) { isSelected = kTRUE; }
    }
    ...
    try {
      ...
      if ((TString(key->GetClassName()) == "TTree")) {
        ...
        for (Int_t j = 0; j < tentries; j++) {
          ...
          if (val < 0) {
            ::Error("AliXRDPROOFtoolkit::TestFile.TestKey",
                    "%s/%s/ TestStatus_%d",
                    fileName,
                    oname.Data(),
                    8);
      ...
      ::Error("AliXRDPROOFtoolkit::TestFile.TestAllFailed",
              "%s/%s/TestStatusKey_%d",
              fileName,
              oname.Data(),
              test);
      status = test;
    ...
    if (status == 0) {
      ::Info("AliXRDPROOFtoolkit::TestFile.TestKey",
             "%s/%s/ TestStatus_%d",
             fileName,
             oname.Data(),
             status);
