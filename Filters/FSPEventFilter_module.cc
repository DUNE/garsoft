...
namespace gar {
  namespace filt {
    ...
    //-------------------------------------------------
    FSPEventFilter::FSPEventFilter(fhicl::ParameterSet const& pset) : EDFilter{pset}
    {
      ...
      bool usegeniegenlabels = pset.get_if_present<std::vector<std::string>>("GENIEGeneratorLabels",
                                                                             fGENIEGeneratorLabels);
    ...
    bool FSPEventFilter::filter(art::Event& evt)
    {
      ...
      if (fGeneratorLabels.size() < 1) {
        mcthandlelist =
          evt.getMany<std::vector<simb::MCTruth>>(); // get them all (even if there are none)
      }
      else {
      ...
      for (size_t imchl = 0; imchl < mcthandlelist.size(); ++imchl) {
        for (simb::MCTruth const& mct : (*mcthandlelist.at(imchl))) {
          ...
          if (!inVolume) {
            ...
            if (mct.NeutrinoSet()) {
              ...
              for (auto vol : fVolumes) {
                ...
                if (vol == "ECAL" &&
                    (fGeo->PointInECALBarrel(place) || fGeo->PointInECALEndcap(place)))
                  inVolume = true;
                if (vol == "MuID" &&
                    (fGeo->PointInMuIDBarrel(place) || fGeo->PointInMuIDEndcap(place)))
                  inVolume = true;
              }
            }
          }
          ...
          for (int i = 0; i < mct.NParticles(); ++i) {
            ...
            if (part.StatusCode() == 1 && part.P() > fPminCut) FSP.push_back(part.PdgCode());
          }

          if (isMatched(fPDG, FSP)) { return true; }
