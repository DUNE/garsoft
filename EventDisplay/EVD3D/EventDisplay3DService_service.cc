...
namespace gar {
  namespace evd3d {
    ...
    {
      if (gEve) { gEve->Redraw3D(kFALSE, kTRUE); }
      else {
      ...
      case evdb::kGOTO_EVENT: {
        ...
        if (targRun < 0 || targEvt < 0) {
          mf::LogWarning("EvtDisplayService")
          ...
          if (!rootInput->seekToEvent(id)) { // Couldn't find event
            mf::LogWarning("EvtDisplayService")
              << "Unable to find " << id << " -- reloading current event.";
            // Reload current event.
            rootInput->seekToEvent(evt.id());
          }
        }
      ...
      default: {
        throw art::Exception(art::errors::LogicError)
          << "EvtDisplayService in unhandled state " << evdb::NavState::Which() << ".\n";
      }
      }
    }
  }
