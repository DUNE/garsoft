...
namespace {
  ...
  {
    mf::LogWarning("EventDisplay3D") << "EventDisplay3DToad::" << fcn << " failed with message:\n"
                                     << e;
  }
}
...
namespace gar {
  namespace evd3d {
    class EventDisplay3DToad : public art::EDAnalyzer {
    public:
      explicit EventDisplay3DToad(fhicl::ParameterSet const& pset);
      virtual ~EventDisplay3DToad();
    ...
    private:
      ...
      const gar::detinfo::DetectorClocks* fTime =
        gar::providerFrom<detinfo::DetectorClocksServiceGAr>();
      ...
      std::vector<std::string>
        fCaloClusterLabels;                   ///< module labels that produced Calorimeter Clusters
      ...
      void DrawTrack(const gar::rec::Track* trk,
                     TEveElementList*& eve_list,
                     int counter,
                     bool forward);
      ...
      void GetRawCaloHits(std::vector<const raw::CaloRawDigit*>& digitCalo,
    ...
    //......................................................................
    EventDisplay3DToad::EventDisplay3DToad(fhicl::ParameterSet const& pset)
      : art::EDAnalyzer(pset)
      , fEve(0)
      , fTeRun(0)
      , fTeEvt(0)
      ...
      , fScalingfactor(1.0)
    {
      ...
      fGeoManager->SetVisLevel(
        5); //By default, TGeoVolume::Draw() paints the content of a given volume three levels down.
      fGeoManager->DefaultColors();
    }

    //......................................................................
    void EventDisplay3DToad::reconfigure(fhicl::ParameterSet const& pset)
    {
    ...
    //----------------------------------------------------
    EventDisplay3DToad::~EventDisplay3DToad() {}

    //----------------------------------------------------
    void EventDisplay3DToad::makeNavPanel()
    {
      ...
      navFrame->AddFrame(b);
      b->Connect(
        "Clicked()", "gar::evd3d::EventDisplay3DUtils", fEvtDisplayUtil.get(), "PrevEvent()");
      ...
      navFrame->AddFrame(b);
      b->Connect(
        "Clicked()", "gar::evd3d::EventDisplay3DUtils", fEvtDisplayUtil.get(), "NextEvent()");
      ...
      fTeRun->Connect(
        "ReturnPressed()", "gar::evd3d::EventDisplay3DUtils", fEvtDisplayUtil.get(), "GotoEvent()");
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::beginJob()
    {
      ...
      std::cout << "Drawing at reference (" << origin[0] << ", " << origin[1] << ", " << origin[2]
                << ")" << std::endl;
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::beginRun(const art::Run& /* run */)
    {
      return;
    }

    //----------------------------------------------------
    void EventDisplay3DToad::analyze(const art::Event& event)
    {
      this->cleanEvt();
      ...
      //Draw MCTruth
      if (fDrawMCTruth) this->DrawMCTruth(event);

      if (fDrawMCTPC) this->DrawMCTPCTruth(event);
      ...
      return;
    } // end EventDisplay3DToad::EventDisplay3DToad::analyze

    //----------------------------------------------------
    void EventDisplay3DToad::cleanEvt()
    {
      //check if any element is present in the event and remove it
      if (fEve->GetCurrentEvent() != nullptr) fEve->GetCurrentEvent()->DestroyElements();
    }

    //----------------------------------------------------
    void EventDisplay3DToad::endJob() {}

    //----------------------------------------------------
    void EventDisplay3DToad::PickVolumes(TEveElementList*& list)
    {
      ...
      for (auto volname : fVolumesToShow) {
        ...
        for (unsigned int i = 0; i < nodevec.size(); i++) {
          ...
          if (nodename.find("volGArTPC") !=
              std::string::npos) //|| nodename.find("EndcapECal") != std::string::npos)
          {
            ...
            for (int idaugh = 0; idaugh < node->GetNdaughters(); idaugh++) {
              ...
              TGeoShape* daugh_clonedShape =
                dynamic_cast<TGeoShape*>(daugh_shape->Clone("fakeShape"));
              ...
              if (daugh_nodename.find("volTPCChamber") != std::string::npos) {
                ...
                for (int idaugh2 = 0; idaugh2 < daugh_node->GetNdaughters(); idaugh2++) {
                  ...
                  TGeoShape* daugh_clonedShape2 =
                    dynamic_cast<TGeoShape*>(daugh_shape2->Clone("fakeShape2"));
                  ...
                  if (daugh_nodename2.find("OROC_alu_frame_OROC_frame_vol_0") !=
                      std::string::npos) {
                    daugh_fakeShape2->SetMainColor(kGray);
                    daugh_fakeShape2->SetMainTransparency(50);
                  }
                  if (daugh_nodename2.find("Terminator_holderterminator_holder_vol_0") !=
                      std::string::npos) {
                    daugh_fakeShape2->SetMainColor(kOrange);
                    daugh_fakeShape2->SetMainTransparency(70);
                  }
                  if (daugh_nodename2.find("Terminator_bottom_terminator_vol_1_0") !=
                      std::string::npos) {
                    daugh_fakeShape2->SetMainColor(kGray);
                    daugh_fakeShape2->SetMainTransparency(70);
                  }
                  if (daugh_nodename2.find("Terminator_left_terminato_vol_2_0") !=
                      std::string::npos) {
                    daugh_fakeShape2->SetMainColor(kGray);
                    daugh_fakeShape2->SetMainTransparency(70);
                  }
                  if (daugh_nodename2.find("Terminator_right_terminator_vol_3_0") !=
                      std::string::npos) {
                    daugh_fakeShape2->SetMainColor(kGray);
                    daugh_fakeShape2->SetMainTransparency(70);
                  }
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawMCTruth(const art::Event& event)
    {
      ...
      for (unsigned int idx = 0; idx < mctruth.size(); idx++) {
        ...
        for (int particleIdx = 0; particleIdx < mctruth[idx]->NParticles(); particleIdx++) {
          ...
          if (mcPart.Mother() == -1 && mcPart.StatusCode() == 0) {
            ...
            motherPart->SetPoint(
              0, particlePosition.X(), particlePosition.Y(), particlePosition.Z());
            particlePosition += std::min(arcLenToDraw + 10., 1000.) * oppPartDir;
            motherPart->SetPoint(
              1, particlePosition.X(), particlePosition.Y(), particlePosition.Z());
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawMCTPCTruth(const art::Event& event)
    {
      ...
      for (unsigned int p = 0; p < simlist.size(); ++p) {
        ...
        label << "Position (" << simHit->X() << ", " << simHit->Y() << ", " << simHit->Z()
              << " ) cm\n";
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawMCCaloTruth(const art::Event& event)
    {
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawRawHits(const art::Event& event)
    {
      ...
      for (unsigned int p = 0; p < rawlist.size(); ++p) {
        ...
        label << "Position (" << rawHit->X() << ", " << rawHit->Y() << ", " << rawHit->Z()
              << " ) cm\n";
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawRecoHits(const art::Event& event)
    {
      ...
      for (unsigned int p = 0; p < recolist.size(); ++p) {
        ...
        const std::array<double, 3> point = {
          recoHit->Position()[0], recoHit->Position()[1], recoHit->Position()[2]};

        std::ostringstream label;
        label << "ECAL Reco Hit " << p << "\n";
        label << "Energy: " << recoHit->Energy() * 1000 << " MeV\n";
        label << "Position (" << recoHit->Position()[0] << ", " << recoHit->Position()[1] << ", "
              << recoHit->Position()[2] << " ) cm\n";
        ...
        evehit->SetPoint(
          0, recoHit->Position()[0], recoHit->Position()[1], recoHit->Position()[2]); //cm
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawHighLevelReco(const art::Event& event)
    {
      if (fDrawECALClusters) {
        ...
        for (unsigned int p = 0; p < clusters.size(); ++p) {
          ...
          label << "Position (" << clus->Position()[0] << ", " << clus->Position()[1] << ", "
                << clus->Position()[2] << " ) cm\n";
          label << "Shape: r_forw " << shape[0] << ", r_bck " << shape[1] << ", r2 " << shape[2]
                << ", r3 " << shape[3] << ", vol " << shape[4] << ", width " << shape[5];
      ...
      if (fDrawTracks) {
        ...
        for (auto tv = trackView.begin(); tv != trackView.end(); ++tv) {
          ...
          if (fDrawIntersection) this->DrawIntersections((*tv), fTrackList, icounter);
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawTrack(const gar::rec::Track* trk,
                                       TEveElementList*& eve_list,
                                       int counter,
                                       bool forward)
    {
      ...
      TString trackname = (forward == true) ? TString::Format("Track Fit forward %i", counter) :
                                              TString::Format("Track Fit backward %i", counter);
      ...
      std::ostringstream label;
      label << "Track " << counter << "\n";
      label << "Vertex (" << trk->Vertex()[0] << ", " << trk->Vertex()[1] << ", "
            << trk->Vertex()[2] << ") cm\n";
      label << "End (" << trk->End()[0] << ", " << trk->End()[1] << ", " << trk->End()[2]
            << ") cm\n";
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawTrackPolyLine3D(
      std::vector<const gar::rec::TrackTrajectory*> const& trajectories,
      TEveElementList*& eve_list,
      int counter)
    {
      int color = evd::kColor[counter % evd::kNCOLS];
      ...
      for (auto itr : trajectories) // there should only be one
      {
        std::vector<TVector3> ftraj =
          itr->getFWDTrajectory(); // later -- decide if we need the backward one too
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawTPCCluster3D(
      std::vector<const gar::rec::TPCCluster*> const& TPCClusters,
      TEveElementList*& eve_list)
    {
      unsigned int idx = 0;
      for (auto itr : TPCClusters) {
        ...
        label << "Position (" << itr->Position()[0] << ", " << itr->Position()[1] << ", "
              << itr->Position()[2] << " ) cm";
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawIntersections(const gar::rec::Track* trk,
                                               TEveElementList*& fTrackList_local,
                                               int counter)
    {
      int color = evd::kColor[counter % evd::kNCOLS];
      ...
      int result = util::TrackPropagator::PropagateToCylinder(trk->TrackParEnd(),
                                                              trk->End(),
                                                              fGeometry->GetECALInnerBarrelRadius(),
                                                              fGeometry->TPCYCent(),
                                                              fGeometry->TPCZCent(),
                                                              xyz_intersection,
                                                              xyz_otherintersect,
      ...
      if (result == 0) {
        ...
        intersection1->SetName(
          TString::Format("Track forward Calo Intersection Barrel %i", counter).Data());
        label << "Barrel Intersection forward - Track " << counter << "\n";
        label << "Position (" << xyz_intersection[0] << ", " << xyz_intersection[1] << ", "
              << xyz_intersection[2] << ") cm";
        ...
        intersection1->SetPoint(
          0, xyz_intersection[0], xyz_intersection[1], xyz_intersection[2]); //cm
        fTrackList_local->AddElement(intersection1);
      }
      else {
        //Try Endcap
        result = util::TrackPropagator::PropagateToX(trk->TrackParEnd(),
                                                     trk->End(),
                                                     (trk->End()[0] > 0) ?
                                                       fGeometry->GetECALEndcapStartX() :
                                                       -fGeometry->GetECALEndcapStartX(),
                                                     xyz_intersection,
                                                     fGeometry->GetECALOuterBarrelRadius());

        if (result == 0) {
          TEvePointSet* intersection3 = new TEvePointSet(1);
          intersection3->SetName(
            TString::Format("Track forward Calo Intersection Endcap %i", counter).Data());
          label.str("");
          label.clear();
      ...
      //Backward case
      result = util::TrackPropagator::PropagateToCylinder(trk->TrackParBeg(),
                                                          trk->Vertex(),
                                                          fGeometry->GetECALInnerBarrelRadius(),
                                                          fGeometry->TPCYCent(),
                                                          fGeometry->TPCZCent(),
      ...
      if (result == 0) {
        ...
        intersection2->SetName(
          TString::Format("Track backward Calo Intersection Barrel %i", counter).Data());
        label.str("");
        label.clear();
        ...
        intersection2->SetPoint(
          0, xyz_intersection[0], xyz_intersection[1], xyz_intersection[2]); //cm
        fTrackList_local->AddElement(intersection2);
      }
      else {
        //Try Endcap
        result = util::TrackPropagator::PropagateToX(trk->TrackParBeg(),
                                                     trk->Vertex(),
                                                     (trk->Vertex()[0] > 0) ?
                                                       fGeometry->GetECALEndcapStartX() :
                                                       -fGeometry->GetECALEndcapStartX(),
        ...
        if (result == 0) {
          ...
          intersection4->SetName(
            TString::Format("Track backward Calo Intersection Endcap %i", counter).Data());
          label.str("");
          label.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawHelix3D(const float* trackpar,
                                         const float xpar,
                                         const float xother,
                                         TEveLine*& eve_track,
                                         int color)
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::DrawArrow3D(const float* fVertex,
                                         const float* fDir,
                                         int color,
                                         TEveArrow*& arrow)
    {
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetSimTPCHits(std::vector<const sdp::EnergyDeposit*>& simTPC,
                                           const art::Event& event)
    {
      simTPC.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetSimCaloHits(std::vector<const sdp::CaloDeposit*>& simCalo,
                                            const art::Event& event)
    {
      simCalo.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetRawCaloHits(std::vector<const raw::CaloRawDigit*>& digitCalo,
                                            const art::Event& event)
    {
      digitCalo.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetRecoCaloHits(std::vector<const rec::CaloHit*>& recoCalo,
                                             const art::Event& event)
    {
      recoCalo.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetRecoMuIDHits(std::vector<const rec::CaloHit*>& recoMuID,
                                             const art::Event& event)
    {
      recoMuID.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetCaloClusters(std::vector<const rec::Cluster*>& caloCluster,
                                             const art::Event& event)
    {
      caloCluster.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetTracks(art::View<gar::rec::Track>& track, const art::Event& event)
    {
      try {
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetVertices(std::vector<const rec::Vertex*>& vertex,
                                         const art::Event& event)
    {
      vertex.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetMCTruth(std::vector<const simb::MCTruth*>& mcvec,
                                        const art::Event& event)
    {
      mcvec.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::GetParticle(std::vector<const simb::MCParticle*>& plist,
                                         const art::Event& event)
    {
      plist.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3DToad::UpdateHeader(const art::Event& event)
    {
    ...
    DEFINE_ART_MODULE(EventDisplay3DToad)
