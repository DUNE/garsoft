...
namespace {
  ...
  {
    mf::LogWarning("EventDisplay3D") << "EventDisplay3D::" << fcn << " failed with message:\n" << e;
  }
}
...
namespace gar {
  namespace evd3d {
    ...
    void EventDisplay3D::analyze(const art::Event& event)
    {
      ...
      //Draw MCTruth
      if (fDrawMCCaloTruth) this->DrawMCCaloTruth(event);

      //Draw raw hits
      if (fDrawECALRawHits) this->DrawRawHits(event);

      //Draw reco hits
      if (fDrawECALRecoHits) this->DrawRecoHits(event);
      ...
      for (auto volname : fVolumesToShow) {
        ...
        for (unsigned int i = 0; i < nodevec.size(); i++) {
          ...
          if (nodename.find("BarrelECal") != std::string::npos ||
              nodename.find("EndcapECal") != std::string::npos) {
    ...
    //----------------------------------------------------
    void EventDisplay3D::DrawTrack(const gar::rec::Track* trk,
                                   TEveElementList*& eve_list,
                                   int counter,
                                   bool forward)
    {
    ...
    //----------------------------------------------------
    void EventDisplay3D::DrawTrackPolyLine3D(
    ...
    //----------------------------------------------------
    void EventDisplay3D::DrawTPCCluster3D(
    ...
    //----------------------------------------------------
    void EventDisplay3D::DrawIntersections(const gar::rec::Track* trk,
    ...
    //----------------------------------------------------
    void EventDisplay3D::DrawHelix3D(const float* trackpar,
                                     const float xpar,
                                     const float xother,
                                     TEveLine*& eve_track,
                                     int color)
    ...
    //----------------------------------------------------
    void EventDisplay3D::DrawArrow3D(const float* fVertex,
                                     const float* fDir,
                                     int color,
                                     TEveArrow*& arrow)
    {
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetSimTPCHits(std::vector<const sdp::EnergyDeposit*>& simTPC,
                                       const art::Event& event)
    {
      simTPC.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetSimCaloHits(std::vector<const sdp::CaloDeposit*>& simCalo,
                                        const art::Event& event)
    {
      simCalo.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetRawCaloHits(std::vector<const raw::CaloRawDigit*>& digitCalo,
                                        const art::Event& event)
    {
      digitCalo.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetRecoCaloHits(std::vector<const rec::CaloHit*>& recoCalo,
                                         const art::Event& event)
    {
      recoCalo.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetRecoMuIDHits(std::vector<const rec::CaloHit*>& recoMuID,
                                         const art::Event& event)
    {
      recoMuID.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetCaloClusters(std::vector<const rec::Cluster*>& caloCluster,
                                         const art::Event& event)
    {
      caloCluster.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetVertices(std::vector<const rec::Vertex*>& vertex,
                                     const art::Event& event)
    {
      vertex.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetMCTruth(std::vector<const simb::MCTruth*>& mcvec,
                                    const art::Event& event)
    {
      mcvec.clear();
    ...
    //----------------------------------------------------
    void EventDisplay3D::GetParticle(std::vector<const simb::MCParticle*>& plist,
                                     const art::Event& event)
    {
      plist.clear();
