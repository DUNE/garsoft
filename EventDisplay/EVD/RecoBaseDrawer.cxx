...
namespace gar {
  namespace evd {
    ...
    ///
    void RecoBaseDrawer::Hit3D(const art::Event& evt, evdb::View3D* view)
    {
      art::ServiceHandle<evd::RecoDrawingOptions> recoOpt;
      art::ServiceHandle<evd::RawDrawingOptions> rawOpt;

      if (recoOpt->fDrawHits == 0 || rawOpt->fDrawRawOrReco < 1) return;
    ...
    /// @param plane  : plane number of view
    ///
    void RecoBaseDrawer::TPCCluster3D(const art::Event& evt, evdb::View3D* view)
    {
      art::ServiceHandle<evd::RecoDrawingOptions> recoOpt;
      art::ServiceHandle<evd::RawDrawingOptions> rawOpt;

      if (recoOpt->fDrawTPCClusters == 0 || rawOpt->fDrawRawOrReco < 1) return;
      ...
      for (auto const& which : recoOpt->fTPCClusterLabels) {
        ...
        this->DrawTPCCluster3D(TPCClusters,
                               view,
                               h % evd::kNCOLS,
    ...
    /// @param view   : Pointer to view to draw on
    ///
    void RecoBaseDrawer::CaloCluster3D(const art::Event& evt, evdb::View3D* view)
    {
      art::ServiceHandle<evd::RecoDrawingOptions> recoOpt;
      art::ServiceHandle<evd::RawDrawingOptions> rawOpt;

      if (recoOpt->fDrawCaloClusters == 0 || rawOpt->fDrawRawOrReco < 1) return;
    ...
    /// @param view   : Pointer to view to draw on
    ///
    void RecoBaseDrawer::CaloHit3D(const art::Event& evt, evdb::View3D* view)
    {
      art::ServiceHandle<evd::RecoDrawingOptions> recoOpt;
      art::ServiceHandle<evd::RawDrawingOptions> rawOpt;

      if (recoOpt->fDrawCaloHits == 0 || rawOpt->fDrawRawOrReco < 1) return;
                                     ...
                                     float lengthscale)
    {
      ...
      if (perp1.Mag() == 0) { perp1 = av.Cross(yhat); }
      if (perp1.Mag() == 0) { perp1 = av.Cross(zhat); }
      if (perp1.Mag() == 0) {
        // nb. coordinates are so Y is up, but ROOT thinks Z is up, so report (z,x,y)
    ...
    //......................................................................
    void RecoBaseDrawer::DrawTPCCluster3D(
    ...
    //......................................................................
    void RecoBaseDrawer::DrawTrackPolyLine3D(
      ...
      int width)
    {
      ...
      for (auto itr : trajectories) // there should only be one
      {
        ...
        for (size_t i = 0; i < ftraj.size(); ++i) {
          tpoly.SetPoint(
            i, zcent + ftraj.at(i).Z(), xcent + ftraj.at(i).X(), ycent + ftraj.at(i).Y());
        }
        tpoly.SetLineWidth(width);
      }
                                       ...
                                       evdb::View3D* view)
    {
      ...
      const int colorheatindex[16] = {kBlue + 2,
                                      kBlue + 1,
                                      kBlue,
                                      kCyan + 2,
                                      kCyan + 1,
      ...
      for (auto itr : CaloHits) {
        ...
        for (size_t j = 0; j < pointlist.size(); ++j) {
          // nb. coordinates are so Y is up, but ROOT thinks Z is up, so report (z,x,y)
          pl.SetPoint(
            j, poslist[pointlist[j]][2], poslist[pointlist[j]][0], poslist[pointlist[j]][1]);
        }
      }
                                      ...
                                      int /*size*/)
    {
      ...
      for (unsigned int idx = 0; idx < vechits.size(); ++idx) {
        ...
        int colorloc = evd::kColor[idx % evd::kNCOLS];

        // Make and fill a polyline: the vechit extent in both directions
        TPolyLine3D& pl = view->AddPolyLine3D(2, colorloc, 1, 1);
        ...
        pl.SetPoint(
          0, pos[2] - dir[2] * (len / 2), pos[0] - dir[0] * (len / 2), pos[1] - dir[1] * (len / 2));

        // This would be the center, but don't need to draw that
        //pl.SetPoint(1, pos[0], pos[1], pos[2]);

        pl.SetPoint(
          1, pos[2] + dir[2] * (len / 2), pos[0] + dir[0] * (len / 2), pos[1] + dir[1] * (len / 2));
        ++p;
      }
    }

    //----------------------------------------------------------------------------
    void RecoBaseDrawer::Track3D(const art::Event& evt, evdb::View3D* view)
    {
      art::ServiceHandle<evd::RecoDrawingOptions> recoOpt;
      art::ServiceHandle<evd::RawDrawingOptions> rawOpt;
      ...
      if (drawTracks) {
        for (auto const& which : recoOpt->fTrackLabels) {
          ...
          for (size_t t = 0; t < trackView.size(); ++t) {
            ...
            DrawTPCCluster3D(
              TPCClusters, view, color, recoOpt->fTPCClusterMarker, recoOpt->fTPCClusterMarkerSize);
          }
          ...
          for (auto tv = trackView.begin(); tv != trackView.end(); ++tv) {
            ...
            if (recoOpt->fDrawTracks == 1 || !fmt.isValid()) {
              DrawHelix3D(
                (*tv)->TrackParBeg(), (*tv)->Vertex()[0], (*tv)->End()[0], view, color, width);
              DrawHelix3D(
                (*tv)->TrackParEnd(), (*tv)->End()[0], (*tv)->Vertex()[0], view, color, width);
            }
    ...
    //----------------------------------------------------------------------------
    void RecoBaseDrawer::VecHit3D(const art::Event& evt, evdb::View3D* view)
    {
      art::ServiceHandle<evd::RecoDrawingOptions> recoOpt;
      art::ServiceHandle<evd::RawDrawingOptions> rawOpt;
