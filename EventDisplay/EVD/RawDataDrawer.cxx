...
namespace gar {
  namespace evd {
    ...
    {
      fEventTQHist = (TH1F*)new TH1F(
        "eventTQHist", "ADC Sum vs Time, All Channels;time (ticks);ADC Sum", 100, 0, 10000);
      fDigitTQHist = (TH1F*)new TH1F(
        "digitTQHist", "ADC Sum vs Time, One Channel;time (ticks);ADC Sum", 100, 0, 10000);
    }
    ...
    //......................................................................
    void RawDataDrawer::FillQHisto(gar::raw::RawDigit const& dig, TH1F* histo)
    {
    ...
    //......................................................................
    void RawDataDrawer::RawDigit3D(const art::Event& evt, evdb::View3D* view)
    {
      ...
      double driftVelocity = detProp->DriftVelocity(detProp->Efield(), detProp->Temperature());
      ...
      for (auto dig : rawhits) {
        ...
        if (dig->Channel() == rawopt->fChannel) this->FillQHisto(*dig, fDigitTQHist);

        // draw the digit as time, y, z with the latter coordinates
        // given by the channel map.
        float xyz[3];

        geom->ChannelToPosition((unsigned int)dig->Channel(), xyz);
        ...
        for (size_t t = 0; t < dig->Samples(); ++t) {
          ...
          if (chanposx < 0) { xyz[0] = chanposx + driftdistance; }
          else {
