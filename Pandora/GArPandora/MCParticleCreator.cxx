  ...
  namespace gar_pandora {
    ...
    pandora::StatusCode MCParticleCreator::CreateMCParticles(const art::Event& pEvent)
    {
      PANDORA_RETURN_RESULT_IF(
        ...
        !=,
        this->CollectMCParticles(pEvent, m_settings.m_geantModuleLabel, artMCParticleVector));
      if (!m_settings.m_generatorModuleLabel.empty())
        PANDORA_RETURN_RESULT_IF(
          pandora::STATUS_CODE_SUCCESS,
          !=,
          this->CollectGeneratorMCParticles(
            pEvent, m_settings.m_generatorModuleLabel, generatorArtMCParticleVector));
      PANDORA_RETURN_RESULT_IF(
        pandora::STATUS_CODE_SUCCESS,
        !=,
        this->CollectMCParticles(
          pEvent, m_settings.m_geantModuleLabel, artMCTruthToMCParticles, artMCParticlesToMCTruth));
    ...
    pandora::StatusCode MCParticleCreator::CreateMCParticles() const
    {
      ...
      for (MCParticlesToMCTruth::const_iterator iter = artMCParticlesToMCTruth.begin(),
                                                iterEnd = artMCParticlesToMCTruth.end();
           iter != iterEnd;
           ++iter) {
      ...
      for (MCTruthToMCParticles::const_iterator iter1 = artMCTruthToMCParticles.begin(),
                                                iterEnd1 = artMCTruthToMCParticles.end();
           iter1 != iterEnd1;
           ++iter1) {
        ...
        if (truth->NeutrinoSet()) {
          ...
          const pandora::CartesianVector momentum(
            neutrino.Nu().Px(), neutrino.Nu().Py(), neutrino.Nu().Pz());
          const pandora::CartesianVector vertex((neutrino.Nu().Vx() - m_origin[0]) * CLHEP::cm,
                                                (neutrino.Nu().Vy() - m_origin[1]) * CLHEP::cm,
                                                (neutrino.Nu().Vz() - m_origin[2]) * CLHEP::cm);
          const pandora::CartesianVector endpoint((neutrino.Nu().EndX() - m_origin[0]) * CLHEP::cm,
                                                  (neutrino.Nu().EndY() - m_origin[1]) * CLHEP::cm,
                                                  (neutrino.Nu().EndZ() - m_origin[2]) * CLHEP::cm);
              ...
              try {
                PANDORA_THROW_RESULT_IF(pandora::STATUS_CODE_SUCCESS,
                                        ...
                                        PandoraApi::SetMCParentDaughterRelationship(
                                          m_pandora, &neutrino, particle.get()));
              }
              catch (const pandora::StatusCodeException&) {
                MF_LOG_WARNING("MCParticleCreator")
                  << "CreatePandoraMCParticles - unable to create mc particle relationship, "
      ...
      MF_LOG_DEBUG("MCParticleCreator")
      ...
      for (MCParticleMap::const_iterator iterI = particleMap.begin(), iterEndI = particleMap.end();
        ...
        const pandora::CartesianVector momentum(
          pMcParticle->Px(), pMcParticle->Py(), pMcParticle->Pz());
        const pandora::CartesianVector vertex((pMcParticle->Vx() - m_origin[0]) * CLHEP::cm,
                                              (pMcParticle->Vy() - m_origin[1]) * CLHEP::cm,
                                              (pMcParticle->Vz() - m_origin[2]) * CLHEP::cm);
        const pandora::CartesianVector endpoint((pMcParticle->EndX() - m_origin[0]) * CLHEP::cm,
                                                (pMcParticle->EndY() - m_origin[1]) * CLHEP::cm,
                                                (pMcParticle->EndZ() - m_origin[2]) * CLHEP::cm);
        ...
        MF_LOG_DEBUG("MCParticleCreator")
          << " Adding MC Particle with parameters "
          ...
          << " mcParticleParameters.m_mcParticleType = "
          << mcParticleParameters.m_mcParticleType.Get()
          << " mcParticleParameters.m_pParentAddress = "
          << mcParticleParameters.m_pParentAddress.Get()
        ...
        try {
          MF_LOG_DEBUG("MCParticleCreator::CreateMCParticles")
            << " Creating mc particle " << pMcParticle.get() << " of pdg " << pMcParticle->PdgCode()
            << " with TrackID " << pMcParticle->TrackId() << " and energy " << pMcParticle->E()
            << " GeV";
          ...
          if (iterJ != particleMap.end()) {
            try {
              MF_LOG_DEBUG("MCParticleCreator::CreateMCParticles")
                << " Adding daughter relation " << iterJ->second.get() << " to mc particle "
                << pMcParticle.get();

              PANDORA_THROW_RESULT_IF(pandora::STATUS_CODE_SUCCESS,
                                      !=,
                                      PandoraApi::SetMCParentDaughterRelationship(
                                        m_pandora, iterJ->second.get(), pMcParticle.get()));
            }
            catch (const pandora::StatusCodeException&) {
              MF_LOG_WARNING("MCParticleCreator")
                << "CreatePandoraMCParticles - Unable to create mc particle relationship, invalid "
          ...
          MF_LOG_WARNING("MCParticleCreator")
            << "CreatePandoraMCParticles - Unable to create MCParticle " << std::endl;
          continue;
        }
      }
    ...
    pandora::StatusCode MCParticleCreator::CreateTrackToMCParticleRelationships(
      const TrackVector& trackVector) const
    {
      ...
      for (auto const& itr : trackVector) {
        try {
          ...
          const float d0 =
            std::sqrt(trackParams[0] * trackParams[0] + trackParams[1] * trackParams[1]) *
            CLHEP::cm;
          const float z0 = pTrack->End()[0] * CLHEP::cm;

          const pandora::Helix helixFit(
            trackParams[3], d0, z0, omega, std::tan(trackParams[4]), m_bField);
          ...
          for (MCParticleMap::const_iterator iterI = particleMap.begin(),
                                             iterEndI = particleMap.end();
               iterI != iterEndI;
               ++iterI) {
            simb::MCParticle* pMCParticle = const_cast<simb::MCParticle*>(iterI->second.get());

            if (nullptr == pMCParticle) continue;
          ...
          if (nullptr == pBestMCParticle) continue;

          MF_LOG_DEBUG("MCParticleCreator::CreateTrackToMCParticleRelationships")
            << "Found MCParticle " << pBestMCParticle << " associated to track " << pTrack
          ...
          MF_LOG_ERROR("MCParticleCreator::CreateTrackToMCParticleRelationships")
            << "Failed to extract track to mc particle relationship: "
    ...
    pandora::StatusCode MCParticleCreator::CreateCaloHitToMCParticleRelationships(
      const CalorimeterHitVector& calorimeterHitVector) const
    {
      ...
      for (auto const& itr : calorimeterHitVector) {
        ...
        for (size_t ieve = 0; ieve < eveides.size(); ieve++) {
          MF_LOG_DEBUG("MCParticleCreator::CreateCaloHitToMCParticleRelationships")
            << " Found eveID " << eveides[ieve].trackID << " associated to art hit " << itr;
      ...
      for (auto const& hitMapItr : eveCaloHitMap) {
        ...
        for (MCParticleMap::const_iterator iterI = particleMap.begin(),
          ...
          if (nullptr == pMCParticleCalo) continue;
          if (trackID != pMCParticleCalo->TrackId()) continue;

          //Need to get the original monte carlo particle that created this one (mother <- daughters)
          const simb::MCParticle* pMCPrimary =
            MCParticleCreator::GetFinalStateMCParticle(particleMap, pMCParticleCalo);
          if (nullptr == pMCPrimary) continue;
            ...
            try {
              PANDORA_THROW_RESULT_IF(pandora::STATUS_CODE_SUCCESS,
                                      ...
                                      PandoraApi::SetCaloHitToMCParticleRelationship(
                                        m_pandora, hit, pMCPrimary, hit->Energy()));
            }
            catch (const pandora::StatusCodeException&) {
              MF_LOG_WARNING("MCParticleCreator")
                << "CreateCaloHitToMCParticleRelationships - unable to create calo hit to mc "
                                                              ...
                                                              MCParticleVector& particleVector)
    {
      ...
      if (!theParticles) {
        MF_LOG_WARNING("MCParticleCreator")
          << "  Failed to find MC particles for label " << label << std::endl;
    ...
    pandora::StatusCode MCParticleCreator::CollectGeneratorMCParticles(
      const art::Event& pEvent,
      const std::string& label,
      RawMCParticleVector& particleVector)
    {
      ...
      if (!mcTruthBlocks) {
        MF_LOG_WARNING("MCParticleCreator")
          << "  Failed to find MC Truth for generator " << label << std::endl;
        return pandora::STATUS_CODE_NOT_FOUND;
      }

      MF_LOG_DEBUG("MCParticleCreator")
        << "  Found: " << mcTruthBlocks->size() << " MC truth blocks " << std::endl;

      if (mcTruthBlocks->size() != 1)
        throw cet::exception("MCParticleCreator")
          << " PandoraCollector::CollectGeneratorMCParticles --- Unexpected number of MC truth "
             "blocks ";
    ...
    pandora::StatusCode MCParticleCreator::CollectMCParticles(
      const art::Event& pEvent,
      const std::string& label,
    ...
    const simb::MCParticle* MCParticleCreator::GetFinalStateMCParticle(
      ...
      const simb::MCParticle* inputParticle)
    {
      ...
      for (MCParticleVector::const_reverse_iterator iter = mcVector.rbegin(),
        ...
        if (MCParticleCreator::IsVisible(nextParticle)) return nextParticle.get();
      }
    ...
    MCParticleCreator::Settings::Settings() : m_geantModuleLabel(""), m_generatorModuleLabel("") {}
  }
}
