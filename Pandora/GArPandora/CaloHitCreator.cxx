...
namespace gar {
  namespace gar_pandora {
      ...
      , artCalorimeterHitVector(0)
    {
      ...
      const std::vector<gar::geo::LayeredCalorimeterStruct::Layer>& barrelLayers =
        (fGeo->GetECALLayeredCalorimeterData()[gar::geo::LayeredCalorimeterData::BarrelLayout]
           .get())
          ->layers;

      const std::vector<gar::geo::LayeredCalorimeterStruct::Layer>& endcapLayers =
        (fGeo->GetECALLayeredCalorimeterData()[gar::geo::LayeredCalorimeterData::EndcapLayout]
           .get())
          ->layers;

      ///Take thicknesses from last layer (was like that before with gear)
      m_eCalEndCapLayerThickness =
        (endcapLayers.back().inner_thickness + endcapLayers.back().outer_thickness) * CLHEP::cm;
      m_eCalBarrelLayerThickness =
        (barrelLayers.back().inner_thickness + barrelLayers.back().outer_thickness) * CLHEP::cm;

      if ((m_eCalEndCapLayerThickness < std::numeric_limits<float>::epsilon()) ||
          (m_eCalBarrelLayerThickness < std::numeric_limits<float>::epsilon()))
    ...
    pandora::StatusCode CaloHitCreator::CreateCaloHits(const art::Event& pEvent)
    {
      PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS,
                               !=,
                               this->CollectECALCaloHits(pEvent,
                                                         m_settings.m_CaloHitCollection,
                                                         m_settings.m_CaloHitInstanceName,
                                                         artCalorimeterHitVector));
    ...
    pandora::StatusCode CaloHitCreator::CreateECalCaloHits() const
    {
      ...
      for (CalorimeterHitVector::const_iterator iter = artCalorimeterHitVector.begin(),
                                                iterEnd = artCalorimeterHitVector.end();
           iter != iterEnd;
           ++iter) {
        try {
          ...
          if (std::fabs((pCaloHit->Position()[0] - m_origin[0]) * CLHEP::cm) <
              m_settings.m_eCalBarrelOuterZ) {
            this->GetBarrelCaloHitProperties(pCaloHit,
                                             barrelLayers,
                                             m_settings.m_eCalBarrelInnerSymmetry,
                                             caloHitParameters,
                                             m_settings.m_eCalBarrelNormalVector,
                                             absorberCorrection);
            caloHitParameters.m_hadronicEnergy =
              m_settings.m_eCalToHadGeVBarrel * pCaloHit->Energy();
          }
          else {
            this->GetEndCapCaloHitProperties(
              pCaloHit, endcapLayers, caloHitParameters, absorberCorrection);
            caloHitParameters.m_hadronicEnergy =
              m_settings.m_eCalToHadGeVEndCap * pCaloHit->Energy();
          }

          caloHitParameters.m_mipEquivalentEnergy =
            pCaloHit->Energy() * m_settings.m_eCalToMip * absorberCorrection;
          ...
          const float rCoordinate(
            std::sqrt(
              (pCaloHit->Position()[1] - m_origin[1]) * (pCaloHit->Position()[1] - m_origin[1]) +
              (pCaloHit->Position()[2] - m_origin[2]) * (pCaloHit->Position()[2] - m_origin[2])) *
            CLHEP::cm);
          if (rCoordinate > m_settings.m_eCalBarrelOuterR) {
            MF_LOG_WARNING("CaloHitCreator::CreateECalCaloHits")
              ...
              << " z: " << pCaloHit->Position()[2] * CLHEP::cm << " rCoordinate " << rCoordinate
              << " > "
              << " m_settings.m_eCalBarrelOuterR " << m_settings.m_eCalBarrelOuterR
              ...
              << " caloHitParameters.m_nCellRadiationLengths = "
              << caloHitParameters.m_nCellRadiationLengths.Get()
              << " caloHitParameters.m_nCellInteractionLengths = "
              << caloHitParameters.m_nCellInteractionLengths.Get()
              << " caloHitParameters.m_hadronicEnergy = "
              << caloHitParameters.m_hadronicEnergy.Get()
              << " caloHitParameters.m_mipEquivalentEnergy = "
              << caloHitParameters.m_mipEquivalentEnergy.Get()
              << " caloHitParameters.m_electromagneticEnergy = "
              << caloHitParameters.m_electromagneticEnergy.Get();
            continue;
          }
    ...
    void CaloHitCreator::GetCommonCaloHitProperties(
      const gar::rec::CaloHit* const pCaloHit,
      PandoraApi::CaloHit::Parameters& caloHitParameters) const
    {
      ...
      const pandora::CartesianVector positionVector((pCaloHitPosition[0] - m_origin[0]) * CLHEP::cm,
                                                    (pCaloHitPosition[1] - m_origin[1]) * CLHEP::cm,
                                                    (pCaloHitPosition[2] - m_origin[2]) *
                                                      CLHEP::cm);
    ...
    void CaloHitCreator::GetEndCapCaloHitProperties(
      const gar::rec::CaloHit* const pCaloHit,
      ...
      float& absorberCorrection) const
    {
      ...
      const int physicalLayer(std::min(static_cast<int>(caloHitParameters.m_layer.Get()),
      ...
      double thickness =
        (layers[physicalLayer].inner_thickness + layers[physicalLayer].sensitive_thickness / 2.0) *
        CLHEP::cm;
      ...
      if (physicalLayer > 0) {
        thickness += (layers[physicalLayer - 1].outer_thickness -
                      layers[physicalLayer].sensitive_thickness / 2.0) *
                     CLHEP::cm;
        nRadLengths += layers[physicalLayer - 1].outer_nRadiationLengths;
        nIntLengths += layers[physicalLayer - 1].outer_nInteractionLengths;
        layerAbsorberThickness += (layers[physicalLayer - 1].outer_thickness -
                                   layers[physicalLayer].sensitive_thickness / 2.0) *
                                  CLHEP::cm;
      }
      ...
      if (caloHitParameters.m_nCellRadiationLengths.Get() < std::numeric_limits<float>::epsilon() ||
        ...
        MF_LOG_ERROR("CaloHitCreator::GetEndcapCaloHitProperties")
          << "Calo hit has 0 radiation length or interaction length: not creating a Pandora calo "
             "hit.";
        throw pandora::StatusCodeException(pandora::STATUS_CODE_INVALID_PARAMETER);
      }
      ...
      for (unsigned int i = 0, iMax = layers.size(); i < iMax; ++i) {
        ...
        if (i > 0)
          absorberThickness +=
            (layers[i - 1].outer_thickness - layers[i - 1].sensitive_thickness / 2.0) * CLHEP::cm;

        if (absorberThickness < std::numeric_limits<float>::epsilon()) continue;
      ...
      caloHitParameters.m_cellNormalVector = (pCaloHit->Position()[0] * CLHEP::cm > 0) ?
    ...
    void CaloHitCreator::GetBarrelCaloHitProperties(
      const gar::rec::CaloHit* const pCaloHit,
      const std::vector<gar::geo::LayeredCalorimeterStruct::Layer>& layers,
      ...
      float& absorberCorrection) const
    {
      ...
      if (barrelSymmetryOrder > 2) {
        ...
        caloHitParameters.m_cellNormalVector =
          pandora::CartesianVector(0, std::cos(phi), std::sin(phi));
      }
      else {
        const float* pCaloHitPosition(pCaloHit->Position());
    ...
    int CaloHitCreator::GetNLayersFromEdge(const gar::rec::CaloHit* const pCaloHit) const
    {
      ...
      const float barrelMaximumRadius(this->GetMaximumRadius(
        pCaloHit, m_settings.m_eCalBarrelOuterSymmetry, m_settings.m_eCalBarrelOuterPhi0));
      const float endCapMaximumRadius(
        this->GetMaximumRadius(pCaloHit,
                               m_settings.m_eCalEndCapInnerSymmetryOrder,
                               m_settings.m_eCalEndCapInnerPhiCoordinate));
      ...
      if (caloHitAbsZ < m_settings.m_eCalBarrelOuterZ) {
        radialDistanceToEdge =
          (m_settings.m_eCalBarrelOuterR - barrelMaximumRadius) / m_eCalBarrelLayerThickness;
      }
      else {
        radialDistanceToEdge =
          (m_settings.m_eCalEndCapOuterR - endCapMaximumRadius) / m_eCalEndCapLayerThickness;
      }
      ...
      if (caloHitAbsZ >= m_settings.m_eCalBarrelOuterZ) {
        rearDistanceToEdge =
          (m_settings.m_eCalEndCapOuterZ - caloHitAbsZ) / m_eCalEndCapLayerThickness;
      }
      else {
        const float rearDistance((m_settings.m_eCalBarrelOuterZ - caloHitAbsZ) /
                                 m_eCalBarrelLayerThickness);

        if (rearDistance < m_settings.m_layersFromEdgeMaxRearDistance) {
          const float overlapDistance((m_settings.m_eCalEndCapOuterR - endCapMaximumRadius) /
                                      m_eCalEndCapLayerThickness);
    ...
    float CaloHitCreator::GetMaximumRadius(const gar::rec::CaloHit* const pCaloHit,
                                           ...
                                           const float phi0) const
    {
      ...
      if (symmetryOrder <= 2)
        return std::sqrt((pCaloHitPosition[1] * pCaloHitPosition[1]) +
                         (pCaloHitPosition[2] * pCaloHitPosition[2]) * CLHEP::cm);
    ...
    pandora::StatusCode CaloHitCreator::CollectECALCaloHits(const art::Event& pEvent,
                                                            ...
                                                            CalorimeterHitVector& ecalCaloHitVector)
    {
      ...
      if (!theHits) {
        MF_LOG_WARNING("CaloHitCreator::CreateECalCaloHits")
          << "  Failed to find ecal hits for label " << label << std::endl;
        return pandora::STATUS_CODE_NOT_FOUND;
      }

      MF_LOG_DEBUG("CaloHitCreator::CreateECalCaloHits")
        << "  Found: " << theHits->size() << " Hits " << std::endl;
