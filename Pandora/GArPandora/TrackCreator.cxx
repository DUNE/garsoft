  ...
  namespace gar_pandora {
    ...
    pandora::StatusCode TrackCreator::CreateTracks(const art::Event& pEvent)
    {
      PANDORA_RETURN_RESULT_IF(
        ...
        !=,
        this->CollectTracks(pEvent, m_settings.m_trackCollection, artTrkVector));
    ...
    pandora::StatusCode TrackCreator::CreateTrackAssociations(const art::Event& pEvent)
    {
      ...
      PANDORA_RETURN_RESULT_IF(
        pandora::STATUS_CODE_SUCCESS, !=, this->ExtractProngsAndSplits(pEvent));
    ...
    {
      for (TrackVector::const_iterator iter = artTrkVector.begin(), iterEnd = artTrkVector.end();
           iter != iterEnd;
           ++iter) {
        ...
        const float d0 =
          std::sqrt(trackParams[0] * trackParams[0] + trackParams[1] * trackParams[1]) * CLHEP::cm;
    ...
    void TrackCreator::GetTrackStates(const gar::rec::Track* const pTrack,
                                      PandoraApi::Track::Parameters& trackParameters) const
    {
      ...
      const pandora::CartesianVector momentum = pandora::CartesianVector(std::cos(trackParams[3]),
                                                                         std::sin(trackParams[3]),
                                                                         std::tan(trackParams[4])) *
                                                pt;
      ...
      const pandora::CartesianVector startPosition(pTrack->Vertex()[0] * CLHEP::cm,
                                                   pTrack->Vertex()[1] * CLHEP::cm,
                                                   pTrack->Vertex()[2] * CLHEP::cm);
      const pandora::CartesianVector endPosition(
        pTrack->End()[0] * CLHEP::cm, pTrack->End()[1] * CLHEP::cm, pTrack->End()[2] * CLHEP::cm);
      ...
      const pandora::CartesianVector newcalorimeterPosition =
        m_rotation.MakeRotation(calorimeterPosition);
      trackParameters.m_trackStateAtCalorimeter =
        pandora::TrackState(newcalorimeterPosition, momentum);
      trackParameters.m_isProjectedToEndCap =
        ((std::fabs(trackParameters.m_trackStateAtCalorimeter.Get().GetPosition().GetX()) <
          m_settings.m_eCalEndCapInnerZ) ?
           false :
           true);
      ...
      const float particleEnergy(std::sqrt(
        particleMass * particleMass + trackParameters.m_momentumAtDca.Get().GetMagnitudeSquared()));
      trackParameters.m_timeAtCalorimeter = minGenericTime * particleEnergy / 299.792f;
    }

    //------------------------------------------------------------------------------------------------------------------------------------------

    bool TrackCreator::PassesQualityCuts(const gar::rec::Track* const pTrack,
                                         const PandoraApi::Track::Parameters& trackParameters) const
    {
      ...
      if (std::fabs(pTrack->TrackParEnd()[2] / CLHEP::cm) < std::numeric_limits<float>::epsilon()) {
        MF_LOG_ERROR("TrackCreator::PassesQualityCuts") << "Track has Omega = 0 ";
        return false;
      }
      ...
      const float sigmaPOverP(std::sqrt(pTrack->CovMatEndPacked()[5]) /
                              std::fabs(pTrack->TrackParEnd()[2] / CLHEP::cm));
      ...
      if (sigmaPOverP > m_settings.m_maxTrackSigmaPOverP) {
        MF_LOG_WARNING("TrackCreator::PassesQualityCuts")
          ...
          << sigmaPOverP * (momentumAtDca.GetMagnitude()) << " chi2 = " << pTrack->ChisqForward();
        return false;
      }
    ...
    void TrackCreator::TrackReachesECAL(const gar::rec::Track* const /* pTrack */,
                                        PandoraApi::Track::Parameters& trackParameters) const
    {
    ...
    void TrackCreator::DefineTrackPfoUsage(const gar::rec::Track* const pTrack,
                                           PandoraApi::Track::Parameters& trackParameters) const
    {
      ...
      if (trackParameters.m_reachesCalorimeter.Get() && !this->IsParent(pTrack)) {
        ...
        if (this->PassesQualityCuts(pTrack, trackParameters)) {
          ...
          const float trackEnergy(
            std::sqrt(momentumAtDca.GetMagnitudeSquared() + particleMass * particleMass));

          if (trackEnergy < m_settings.m_unmatchedVertexTrackMaxEnergy) {
            if ((d0 < m_settings.m_d0UnmatchedVertexTrackCut) &&
                (z0 < m_settings.m_z0UnmatchedVertexTrackCut)) {
    ...
    //------------------------------------------------------------------------------------------------------------------------------------------
    void TrackCreator::CalculateTrackStateAtCalo(const gar::rec::Track* const pTrack,
                                                 pandora::CartesianVector& posAtCalo) const
    {
      ...
      int result =
        util::TrackPropagator::PropagateToCylinder(pTrack->TrackParEnd(),
                                                   pTrack->End(),
                                                   m_settings.m_eCalBarrelInnerR / CLHEP::cm,
                                                   m_settings.m_GArCenterY / CLHEP::cm,
                                                   m_settings.m_GArCenterZ / CLHEP::cm,
                                                   fbestECalProjection,
                                                   otherECALProjection,
                                                   m_settings.m_eCalEndCapInnerZ / CLHEP::cm);
      bestECalProjection.SetValues(fbestECalProjection[0] * CLHEP::cm,
                                   fbestECalProjection[1] * CLHEP::cm,
                                   fbestECalProjection[2] * CLHEP::cm);

      if (result != 0) {
        //Propagate to the Endcap
        float endcapProjection[3] = {0., 0., 0.};
        int result_local = util::TrackPropagator::PropagateToX(
          pTrack->TrackParEnd(),
          pTrack->End(),
          (pTrack->End()[0] > 0) ? m_settings.m_eCalEndCapInnerZ / CLHEP::cm :
                                   -m_settings.m_eCalEndCapInnerZ / CLHEP::cm,
          endcapProjection,
          m_settings.m_eCalBarrelInnerR / CLHEP::cm);

        if (result_local == 0) {
          bestECalProjection.SetValues(endcapProjection[0] * CLHEP::cm,
                                       endcapProjection[1] * CLHEP::cm,
                                       endcapProjection[2] * CLHEP::cm);
        }
      }
    ...
    //------------------------------------------------------------------------------------------------------------------------------------------
    void TrackCreator::CalculateTimeAtCalo(const gar::rec::Track* const pTrack,
                                           float& timeAtCalo) const
    {
      const float* trackParams = pTrack->TrackParEnd(); //y, z, omega, phi, lambda
      const float omega = trackParams[2] / CLHEP::cm;
      const float tanl = std::tan(trackParams[4]);
      const float d0(std::sqrt(trackParams[0] * trackParams[0] + trackParams[1] + trackParams[1]) *
                     CLHEP::cm);
      const float z0(std::fabs(pTrack->Vertex()[0]) * CLHEP::cm);
      ...
      (void)helix.GetPointInX(static_cast<float>(signPx) * m_settings.m_eCalEndCapInnerZ,
                              referencePoint,
                              bestECalProjection,
                              minGenericTime);
      ...
      if (m_settings.m_eCalBarrelInnerSymmetry > 0) {
        ...
        for (int i = 0; i < m_settings.m_eCalBarrelInnerSymmetry; ++i) {
          ...
          const pandora::StatusCode statusCode(
            helix.GetPointInZY(m_settings.m_eCalBarrelInnerR * std::cos(phi),
                               m_settings.m_eCalBarrelInnerR * std::sin(phi),
                               std::cos(phi + 0.5 * M_PI),
                               std::sin(phi + 0.5 * M_PI),
                               referencePoint,
                               barrelProjection,
                               genericTime));
        ...
        // Cylinder
        float genericTime(std::numeric_limits<float>::max());
        const pandora::StatusCode statusCode(helix.GetPointOnCircle(
                                                    ...
                                                    TrackVector& trkVector)
    {
      ...
      MF_LOG_DEBUG("TrackCreator::CreateTracks")
        << "  Found: " << theTrk->size() << " tracks " << std::endl;
