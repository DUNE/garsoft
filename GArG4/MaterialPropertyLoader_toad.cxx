...
namespace gar {
  namespace garg4 {
    ...
    //----------------------------------------------
    void MaterialPropertyLoaderToad::SetMaterialProperty(
      ...
      double Unit)
    {
      ...
      MF_LOG_INFO("MaterialPropertyLoader") << "Added property " << Material << "  " << Property;
    }

    //----------------------------------------------
    void MaterialPropertyLoaderToad::SetMaterialConstProperty(std::string const& Material,
                                                              std::string const& Property,
    ...
    //----------------------------------------------
    void MaterialPropertyLoaderToad::SetBirksConstant(std::string const& Material,
                                                      double PropertyValue,
                                                      double Unit)
    {
      fBirksConstants[Material] = PropertyValue * Unit;
      MF_LOG_INFO("MaterialPropertyLoader") << "Set Birks constant " << Material;
    }

    //----------------------------------------------
    void MaterialPropertyLoaderToad::UpdateGeometry(G4LogicalVolumeStore* lvs)
    {
      std::map<std::string, G4MaterialPropertiesTable*> MaterialTables;
      std::map<std::string, bool> MaterialsSet;

      MF_LOG_INFO("MaterialPropertyLoader") << "UPDATING GEOMETRY";
    ...
    //--------------------------------------------------------------------------
    void MaterialPropertyLoaderToad::SetReflectances(
      std::string const& /*Material*/,
      ...
      std::map<std::string, std::map<double, double>> const& DiffuseFractions)
    {
      ...
      for (auto const& itMat : DiffuseFractions) {
        ...
        SetMaterialProperty("GAr", DiffusePropName, DiffuseToStore, 1);
      }
    }

    //--------------------------------------------------------------------------
    void MaterialPropertyLoaderToad::GetPropertiesFromServices()
    {
