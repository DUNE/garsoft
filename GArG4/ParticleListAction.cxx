  ...
  namespace garg4 {
    ...
    void ParticleListAction::PreTrackingAction(const G4Track* track)
    {
      ...
      // If this is not a primary particle...
        ...
        std::string matname = "";
        auto geonode = geomanager->FindNode(
          trackpos[0] / CLHEP::cm, trackpos[1] / CLHEP::cm, trackpos[2] / CLHEP::cm);
        if (geonode) { matname = geonode->GetMedium()->GetMaterial()->GetName(); }

        std::regex const re_material(fEMShowerDaughterMatRegex);

        if (is_em_shower_daughter &&
            (!fKeepEMShowerDaughters || !std::regex_match(matname, re_material))) {
          ...
          if (!fParticleList->KnownParticle(fCurrentTrackID)) fCurrentTrackID = sdp::NoParticleId;
        ...
        if (fTrackIDToMCTruthIndex.count(fCurrentTrackID) > 0)
          ...
          throw cet::exception("ParticleListAction")
            << "Cannot find MCTruth index for track id " << fCurrentTrackID << " or " << parentID;
      ...
      MF_LOG_DEBUG("ParticleListAction")
        << "There are now " << fParticleList->size() << " particles in the list";

      if (fTrackIDToMCTruthIndex.count(fCurrentTrackID) > 0)
        MF_LOG_WARNING("ParticleListAction")
          << "attempting to put " << fCurrentTrackID << " into fTrackIDToMCTruthIndex map "
          << " particle is\n"
          << *(fCurrentParticle.particle);
    ...
    void ParticleListAction::PostTrackingAction(const G4Track* aTrack)
    {
      ...
      if (!fCurrentParticle.keep) {
        ...
        MF_LOG_VERBATIM("ParticleListAction")
          << "dropping particle with track id " << fCurrentParticle.particle->TrackId();
    ...
    void ParticleListAction::SteppingAction(const G4Step* step)
    {
      ...
      if (fCurrentParticle.particle->NumberTrajectoryPoints() == 0) {
        ...
        TLorentzVector fourPos(
          position.x() / CLHEP::cm, position.y() / CLHEP::cm, position.z() / CLHEP::cm, time);
