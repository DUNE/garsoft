  ...
  namespace garg4 {
                                                                       ...
                                                                       const G4Step& step)
    {
      ...
      for (unsigned int ii = 0; ii < (unsigned int)fNSplit; ii++) {
        ...
        for (j = 0; j < numSec; j++) {
          ...
          if (abs(pdg) == 310 || abs(pdg) == 311 || abs(pdg) == 3122 || abs(pdg) == 2112) {
            ...
            if (pdg == G4KaonZeroShort::KaonZeroShort()->GetPDGEncoding() &&
                G4UniformRand() < 0.50) {
      ...
      while (iter != secondaries.end()) {
        ...
        if (abs(pdg) == 130 || abs(pdg) == 310 || abs(pdg) == 311 || abs(pdg) == 3122 ||
            abs(pdg) == 2112) {
          if (pdg != 2112)
            std::cout << "MuNuclSplXSB(): Adding " << pdgstr << " of Kinetic Energy and weight "
                      << ke << " GeV and " << secWeight << "." << std::endl;
      ...
      G4GPILSelection* selection)
    {
      if (xBiasMode == 2) {
        ...
        G4double intLen;
        intLen = (1. / eFactor) *
                 pRegProcess->AlongStepGetPhysicalInteractionLength(
