...
namespace gar {
                                                                        ...
                                                                        G4Step const& aStep)
  {
    ...
    for (i = 0; i < j; i++) { //loop over all saved interaction sites
      ...
      if (sqrt(pow(x1[0] - pos[0], 2.) + pow(x1[1] - pos[1], 2.) + pow(x1[2] - pos[2], 2.)) <
          delta) {
        exists = true;
        ...
        for (k = 0; k < NumQuanta; k++) {
          ...
          if (k < NumPhotons) {
            ...
            if (ElectricField) {
              ...
              aQuantum =
                new G4DynamicParticle(G4ThermalElectron::ThermalElectron(), electronMomentum);
              if (Phase == kStateGas) {
              ...
              aQuantum =
                new G4DynamicParticle(G4ThermalElectron::ThermalElectron(), photonMomentum);
                                                ...
                                                G4int Z)
  {
    ...
    f3 = thra * exp(-thrb * efieldinput) +
         thrc * exp(-(pow(efieldinput - thrd, 2)) / (thrf * thrf)) +
