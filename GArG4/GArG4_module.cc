...
namespace gar {
  namespace garg4 {
    ...
    private:
      ...
      garg4::EnergyDepositAction*
        fEDepAction;                      ///< Geant4 user action to handle GAr energy depositions
      garg4::AuxDetAction* fAuxDetAction; ///< Geant4 user action to handle GAr energy depositions
      garg4::ParticleListAction*
        fParticleListAction;                    ///< Geant4 user action to particle information.
      ...
      float
        fProductionCut; ///< G4 will check if a produced particle should travel this far and drop it if not
  ...
  namespace garg4 {
                                                                                   ...
                                                                                   "GEANTSeed"))
    {
      ...
      if (geo->HasECALDetector()) produces<std::vector<sdp::CaloDeposit>>("ECAL"); //ECAL
      if (geo->HasTrackerScDetector())
        produces<std::vector<sdp::CaloDeposit>>("TrackerSc");                      //TrackerSc
      if (geo->HasMuonDetector()) produces<std::vector<sdp::CaloDeposit>>("MuID"); //MuID
    ...
    void GArG4::SetLimitsAndCuts()
    {
      ...
      if (fMaxStepSize.size() != fDetRegionNames.size()) {
        MF_LOG_INFO("GArG4") << "Step size vector has " << fMaxStepSize.size()
                             << " elements != DetRegionName has " << fDetRegionNames.size()
                             << " elements"
      ...
      for (auto& name : fDetRegionNames) {
        ...
        MF_LOG_INFO("GArG4") << "Setting production cuts for region " << reg->GetName()
                             << " production cut set to " << fProductionCut * CLHEP::mm << " mm";
        ...
        if (fMaxStepSize.size() != fDetRegionNames.size())
          ...
          MF_LOG_INFO("GArG4") << "Setting user limits for region " << reg->GetName()
                               << " max step length set to " << fMaxStepSize.at(index) * CLHEP::mm
                               << " mm";
    ...
    void GArG4::beginJob()
    {
      ...
      fG4Help->ConstructDetector(
        geo
          ->GDMLFile()); //TO DO!!! This create a problem when we want to use a custom magnetic field!!! (it calls under the service mag::Magnetic field)

      G4LogicalVolumeStore* store = G4LogicalVolumeStore::GetInstance();
      if (store == nullptr) {
        MF_LOG_INFO("GArG4") << "G4LogicalVolumeStore::GetInstance() not available";
      }
      ...
      fAuxDetAction = new garg4::AuxDetAction(&(fEngine), fAuxDetActionPSet);
    ...
    //--------------------------------------------------------------------------
    std::unique_ptr<PositionInVolumeFilter> GArG4::CreateParticleVolumeFilter(
      std::set<std::string> const& vol_names) const
    {
    ...
    void GArG4::produce(::art::Event& evt)
    {
      ...
      std::unique_ptr<::art::Assns<simb::MCTruth, simb::MCParticle>> tpassn(
        new ::art::Assns<simb::MCTruth, simb::MCParticle>);
      ...
      if (fInputLabels.size() < 1) { mclists = evt.getMany<std::vector<simb::MCTruth>>(); }
      else {
      ...
      if (fdumpParticleList) {
        MF_LOG_INFO("GArG4") << "Dump sim::ParticleList; size() = " << particleList.size() << "\n"
                             << particleList;
      }
      ...
      while (iPartPair != particleList.end()) {
        ...
        if (trackIDToMCT.count(trackID) > 0) {
          ...
          util::CreateAssn(*this, evt, *partCol, mctPtrs[mctidx], *tpassn, nGeneratedParticles);
        }
        else
          throw cet::exception("GArG4") << "Cannot find MCTruth for Track Id: " << trackID
      ...
      for (auto const& tpc : fEDepAction->EnergyDeposits()) {
        MF_LOG_DEBUG("GArG4") << "adding TPC deposits for track id: " << tpc.TrackID();
      ...
      if (geo->HasTrackerScDetector()) deposits = fAuxDetAction->TrackerScDeposits();

      for (auto const& hit : deposits) {
        MF_LOG_DEBUG("GArG4") << "adding calo deposits for track id: " << hit.TrackID();
      ...
      for (auto const& hit : fAuxDetAction->MuIDDeposits()) {
        MF_LOG_DEBUG("GArG4") << "adding muid deposits for track id: " << hit.TrackID();
      ...
      for (auto const& hit : fAuxDetAction->LArDeposits()) {
        MF_LOG_DEBUG("GArG4") << "adding LAr deposits for track id: " << hit.TrackID();
