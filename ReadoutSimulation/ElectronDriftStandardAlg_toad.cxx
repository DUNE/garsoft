...
namespace gar {
  namespace rosim {
    ...
    //--------------------------------------------------------------------------
    ElectronDriftStandardAlgToad::ElectronDriftStandardAlgToad(CLHEP::HepRandomEngine& engine,
    ...
    //------------------------------------------------------------------------
    ElectronDriftStandardAlgToad::~ElectronDriftStandardAlgToad() {}

    //--------------------------------------------------------------------------
    void ElectronDriftStandardAlgToad::DriftElectronsToReadout(
      gar::sdp::EnergyDeposit const& dep,
      gar::rosim::ElectronDriftInfo& driftInfo)
    {
      ...
      const int electrons =
        gar::rosim::IonizationAndScintillation::Instance()->NumberIonizationElectrons();
      ...
      float xyz[3] = {dep.X(), dep.Y(), dep.Z()};

      MF_LOG_DEBUG("ElectronDriftStandardAlgToad::DriftElectronsToReadout")
        << "energy deposition: " << dep.X() << " " << dep.Y() << " " << dep.Z() << std::endl;
      ...
      for (size_t c = 0; c < nClusters; ++c) {
        ...
        MF_LOG_DEBUG("ElectronDriftStandardAlgToad::DriftElectronsToReadout")
          << "g4 time: " << g4time << " drift time " << driftT << " diffusion  x:" << XDiff[c]
          << " y " << YDiff[c] << " z " << ZDiff[c] << " t " << TDiff[c] << " tick period "
