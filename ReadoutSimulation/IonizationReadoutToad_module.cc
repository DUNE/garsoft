...
namespace gar {
  ...
  namespace rosim {
    ...
    //----------------------------------------------------------------------
    // Constructor
    IonizationReadoutToad::IonizationReadoutToad(fhicl::ParameterSet const& pset) : art::EDProducer{pset},
    ...
    void IonizationReadoutToad::reconfigure(fhicl::ParameterSet const& pset) {
      ...
      if (driftAlgName.compare("Standard") == 0) {
        fDriftAlg = std::make_unique<gar::rosim::ElectronDriftStandardAlgToad>(fEngine,
                                                                           driftAlgPars);
      } else {
    ...
    //--------------------------------------------------------------------------
    void IonizationReadoutToad::DriftElectronsToReadout(std::vector<sdp::EnergyDeposit> const& edepCol,
                                                    std::vector<edepIDE>                 & edepIDEs)
    {
      ...
      for (size_t e = 0; e < edepCol.size(); ++e) {
        ...
        for (size_t c = 0; c < clusterXPos.size(); ++c) {
          ...
          MF_LOG_DEBUG("IonizationReadoutToad::DriftElectronsToReadout") << "pproj: " << pproj.Y() << " " << pproj.Z() << std::endl;
          for (size_t icd=0; icd<ncdistrib; ++icd) {
            TH2F *prfhist=0;
            if (cwn.at(icd).roctype == gar::geo::HFILLER) {
              prfhist = fHFILLPRF;
              MF_LOG_DEBUG("IonizationReadoutToad::DriftElectronsToReadout") << " hole filler" << std::endl;
            } else if (cwn.at(icd).roctype == gar::geo::IROC) {
              prfhist = fIROCPRF;
              MF_LOG_DEBUG("IonizationReadoutToad::DriftElectronsToReadout") << " iroc " << std::endl;
            } else if (cwn.at(icd).roctype == gar::geo::IOROC) {
              prfhist = fIOROCPRF;
              MF_LOG_DEBUG("IonizationReadoutToad::DriftElectronsToReadout") << " ioroc " << std::endl;
            } else if (cwn.at(icd).roctype == gar::geo::OOROC) {
              prfhist = fOOROCPRF;
              MF_LOG_DEBUG("IonizationReadoutToad::DriftElectronsToReadout") << " ooroc " << std::endl;
            } else {
              throw cet::exception("IonizationReadoutToad::DriftElectronsToReadout") << "Ununderstood readout chamber type "
                                << cwn.at(icd).roctype << "\n";
            }
            TVector3 dproj = pproj - cwn.at(icd).pos;
            dproj.SetX(0);
            MF_LOG_DEBUG("IonizationReadoutToad::DriftElectronsToReadout") << " Pad loc: " << cwn.at(icd).pos.Y() << 
            ...
            MF_LOG_DEBUG("IonizationReadoutToad::DriftElectronsToReadout") << "along, perp: " << dist_along_padrow << 
    ...
    //--------------------------------------------------------------------------
    void IonizationReadoutToad::CreateSignalDigit(unsigned int                                      const& channel,
                                              std::vector<float>                                     & electrons,
                                              std::set<size_t>                                       & eDepLocs,
                                              std::deque<float>                                      & eDepWeights,
                                              std::vector<raw::RawDigit>                             & digCol,
