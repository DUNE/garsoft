...
namespace gar {
  ...
  namespace rosim {
    ...
    //-----------------------------------------------------------------------
    // Constructor
    RawDigitAna::RawDigitAna(fhicl::ParameterSet const& pset) : EDAnalyzer(pset)
    {
    ...
    void RawDigitAna::analyze(const ::art::Event& evt)
    {
      ...
      if (digCol.failedToGet()) {
        MF_LOG_VERBATIM("RawDigitAna") << "failed to get raw digits, bail";
        return;
      }
      ...
      if (!fmEnergyDep.isValid()) {
        MF_LOG_WARNING("RawDigitAna") << "Unable to find valid association between RawDigits and "
      ...
      for (size_t d = 0; d < digCol->size(); ++d) {
        ...
        MF_LOG_DEBUG("RawDigitAna") << "There are " << edepsCol.size()
                                    << " energy depositions for channel " << (*digCol)[d].Channel();
        ...
        for (auto edep : edepsCol) {
          ...
          MF_LOG_DEBUG("RawDigitAna")
            << "pos: (" << fEDep.x << ", " << fEDep.y << ", " << fEDep.z << ") e: " << fEDep.e
            << " t: " << fEDep.t << " dX: " << fEDep.dX;
