...
namespace gar {
  namespace cheat {
    ...
    struct CalIDE {
      ...
      CalIDE(int id, float ef, float e) : trackID(id), energyFrac(ef), energyTot(e) {}
    };
    ...
    public:
      ...
      sim::ParticleList* GetParticleList() { return &fParticleList; }
      ...
      std::vector<art::Ptr<rec::Hit>> ParticleToHits(simb::MCParticle* const p,
      ...
      std::vector<art::Ptr<rec::CaloHit>> ParticleToCaloHits(
      ...
      // method to return the fraction of all hits in an event from a specific set of
      // Geant4 track IDs that arerepresented in a collection of hits
      std::pair<double, double> CaloHitEfficiency(
        simb::MCParticle* const p,
        std::vector<art::Ptr<rec::CaloHit>> const& hits,
      ...
      std::vector<art::Ptr<rec::Track>> MCParticleToTracks(
        simb::MCParticle* const p,
      ...
      std::vector<art::Ptr<rec::Cluster>> MCParticleToClusters(
        simb::MCParticle* const p,
    ...
    private:
      ...
      std::vector<CalIDE> CellIDToCalIDEs(rec::CaloHit hit) const;

      std::vector<simb::MCParticle*> MCPartsInCluster(rec::Cluster* const c);

      TLorentzVector EnergyDepositToMomentum(const int& trackID,
    ...
    protected:
      ...
      std::unordered_map<int, int>
        fTrackIDToMCTruthIndex; ///< map of track ids to MCTruthList entry.  Track Ids from MCParticle
      ...
      bool
        fSplitEDeps; ///< use weights from Pad Response Functions to break true energy deposits into channel specific contributions
      bool
        fDontChaseNeutrons; ///< stop chasing backtracked shower particles up the particle tree if it finds a neutron.

      // vector gives a fast lookup and is only 677864*24 = 16Mbytes.
      std::vector<std::vector<std::pair<const sdp::EnergyDeposit*, float const>>>
        fChannelToEDepCol; ///< convenience collections of EnergyDeposits for each channel
