...
namespace gar {
  namespace cheat {
    ...
    //----------------------------------------------------------------------
    BackTracker::BackTracker(fhicl::ParameterSet const& pset, art::ActivityRegistry& reg)
    ...
    void BackTracker::Rebuild(art::Event const& evt, art::ScheduleContext)
    {
      ...
      if (!fo.isValid()) {
        ...
        for (size_t iPart = 0; iPart < partCol->size(); ++iPart) {
          ...
          try {
            ...
            if (fSTFU <= 12) { // Ye who comprehend messagelogger, doeth ye better.
              MF_LOG_WARNING("BackTracker_service::Rebuild")
                << "unable to find MCTruth from ParticleList created in " << fG4ModuleLabel
                << "; all attempts to backtrack will fail\n"
      ...
      fInverseVelocity = 1.0 / detProp->DriftVelocity(detProp->Efield(), detProp->Temperature());
      ...
      for (auto vec : fChannelToEDepCol)
        vec.clear();
        ...
        if (!fmEnergyDep.isValid()) {
          ...
          if (fSTFU <= 12) { // Ye who comprehend messagelogger, doeth ye better.
            MF_LOG_DEBUG("BackTracker_service::Rebuild")
              << "There are " << fChannelToEDepCol.size() << " channels in the geometry";
          }
          ...
          for (size_t iDig = 0; iDig < digCol->size(); ++iDig) {
            ...
            for (size_t iDep = 0; iDep < eDeps.size(); iDep++) {
              ...
              fChannelToEDepCol[(*digCol)[iDig].Channel()].emplace_back(
                std::make_pair(eDeps[iDep], w));

              MF_LOG_DEBUG("BackTracker_service::Rebuild")
                << "eDep\t" << iDep << " from RawDigit \t" << iDig
                << "\t TrkID, energy, dl, position:\t " << eDeps[iDep]->TrackID() << "\t "
                << eDeps[iDep]->Energy() << "\t " << eDeps[iDep]->dX() << "\t " << eDeps[iDep]->X()
                << "\t " << eDeps[iDep]->Y() << "\t " << eDeps[iDep]->Z() << std::endl;
            }
          }
          fHasHits = true;
        }
      ...
      if (!fTrackIDToTPCClusters.empty() && !fTPCClusterIDToHits.empty()) {
        for (auto aTrack : *recoTrackCol) {
          ...
          for (const rec::TPCCluster* aTPClus : tpclusThisTrack) {
            ...
            hitsThisTrack.insert(hitsThisTrack.end(), hitsThisClus.begin(), hitsThisClus.end());
          }
