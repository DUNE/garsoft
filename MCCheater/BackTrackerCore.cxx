...
namespace gar {
  namespace cheat {
    ...
    //----------------------------------------------------------------------
    art::Ptr<simb::MCTruth> const BackTrackerCore::ParticleToMCTruth(
      simb::MCParticle* const p) const
    {
      if (!fHasMC) {
      ...
      if (mct > fMCTruthList.size()) {
        throw cet::exception("BackTrackerCore::ParticleToMCTruth")
          << "attempting to find MCTruth index for out-of-range value: " << mct << "/"
          << fMCTruthList.size();
      }
    ...
    //----------------------------------------------------------------------
    std::vector<HitIDE> BackTrackerCore::HitToHitIDEs(art::Ptr<rec::Hit> const& hit) const
    {
      if (!fHasMC || !fHasHits) {
    ...
    //----------------------------------------------------------------------
    std::vector<HitIDE> BackTrackerCore::HitToHitIDEs(rec::Hit const& hit) const
    {
      if (!fHasMC || !fHasHits) {
        throw cet::exception("BackTrackerCore::HitToHitIDEs")
          << "Attempting to backtrack without MC truth or gar::rec::Hit information";
    ...
    //----------------------------------------------------------------------
    std::vector<art::Ptr<rec::Hit>> BackTrackerCore::ParticleToHits(
      simb::MCParticle* const p,
      ...
      bool checkNeutrals) const
    {
      ...
      bool obviousNeutral =
        PDGpid == 22 || PDGpid == 2112 || PDGpid == 111 || abs(PDGpid) == 14 || abs(PDGpid) == 12;
    ...
    //----------------------------------------------------------------------
    std::vector<HitIDE> BackTrackerCore::ChannelToHitIDEs(raw::Channel_t const& channel,
                                                          double const start,
                                                          double const stop) const
    {
      ...
      for (auto const& edepfrac : chanEDeps) {
        ...
        tdc = fClocks->TPCG4Time2TDC(edep->Time() +
                                     TMath::Abs(edep->X() - chanpos[0]) * fInverseVelocity);
        if ((unsigned int)start <= tdc && tdc <= (unsigned int)stop) {
          float energy = edep->Energy() * weight;
          MF_LOG_DEBUG("BackTrackerCore::ChannelToHitIDEs")
            << "edep energy: " << edep->Energy() << ", weight: " << weight
            << ", contributed energy: " << energy;
      ...
      if (nbefore != nafter)
        MF_LOG_WARNING("BackTrackerCore::ChannelToHitIDEs")
          << "found and removed " << nbefore - nafter
    ...
    //----------------------------------------------------------------------
    std::pair<double, double> BackTrackerCore::HitPurity(
      simb::MCParticle* const p,
      ...
      bool weightByCharge) const
    {
      ...
      for (auto hit : hits) {
        ...
        for (auto iHitIDE : hitIDEs) {
          ...
          if (iHitIDE.trackID == trackIDin && iHitIDE.energyFrac >= fMinHitEnergyFraction) {
            desired += weight;
            break;
          }
        }
    ...
    //----------------------------------------------------------------------
    std::pair<double, double> BackTrackerCore::HitEfficiency(
      simb::MCParticle* const p,
      std::vector<art::Ptr<rec::Hit>> const& hits,
      std::vector<art::Ptr<rec::Hit>> const& allhits,
      bool weightByCharge) const
    ...
    //----------------------------------------------------------------------
    std::vector<CalIDE> BackTrackerCore::CaloHitToCalIDEs(art::Ptr<rec::CaloHit> const& hit) const
    {
    ...
    //----------------------------------------------------------------------
    std::vector<CalIDE> BackTrackerCore::CaloHitToCalIDEs(rec::CaloHit const& hit) const
    {
      if (!fHasMC || !fHasCalHits) {
        throw cet::exception("BackTrackerCore::CaloHitToCalIDEs")
          << "Attempting to backtrack without MC truth or gar::rec::CaloHit information";
    ...
    //----------------------------------------------------------------------
    std::vector<art::Ptr<rec::CaloHit>> BackTrackerCore::ParticleToCaloHits(
      simb::MCParticle* const p,
    ...
    //----------------------------------------------------------------------
    std::vector<CalIDE> BackTrackerCore::CellIDToCalIDEs(rec::CaloHit hit) const
    {
      ...
      float time;
      if (hitTime.second == 0) { time = hitTime.first; }
      else {
        ...
        TVector3 whereBe(
          hit.Position()[0], hit.Position()[1], hit.Position()[2]); // C++ & ROOT at their finest
      ...
      bool inYoke = (fGeo->PointInMuIDBarrel(verty) || fGeo->PointInMuIDEndcap(verty));
      ...
      for (auto const& edep : cellEDeps) {
        ...
        if (start <= edep->Time() && edep->Time() <= stop) {
          ...
          int tidDET;
          if (DETEve != nullptr) { tidDET = DETEve->TrackId(); }
          else {
            tidDET = tid;
          }
    ...
    //----------------------------------------------------------------------
    std::pair<double, double> BackTrackerCore::CaloHitPurity(
      simb::MCParticle* const p,
      ...
      bool weightByCharge) const
    {
      ...
      for (auto hit : hits) {
        ...
        for (auto iCalIDE : calIDEs) {
          // Don't double count if this hit has > 1 of the desired GEANT trackIDs
          if (iCalIDE.trackID == trackIDin && iCalIDE.energyFrac >= fMinCaloHitEnergyFrac) {
            desired += weight;
            break;
          }
        }
    ...
    //----------------------------------------------------------------------
    std::pair<double, double> BackTrackerCore::CaloHitEfficiency(
      simb::MCParticle* const p,
      std::vector<art::Ptr<rec::CaloHit>> const& hits,
    ...
    //----------------------------------------------------------------------
    std::vector<art::Ptr<rec::Hit>> const BackTrackerCore::TrackToHits(rec::Track* const t)
    {

      MF_LOG_DEBUG("BackTrackerCore::TrackToHits")
        << "BT:TrackToHits called, read map with " << fTrackIDToHits.size() << " entries"
        << std::endl;

      // Evidently, use of std::unordered_map keeps this method from being const
      std::vector<art::Ptr<gar::rec::Hit>> retval;
      if (!fTrackIDToHits.empty()) { retval = fTrackIDToHits[t->getIDNumber()]; }
      return retval;
    }
    //-----------------------------------------------------------------------
    std::vector<art::Ptr<rec::Hit>> const BackTrackerCore::TPCClusterToHits(
      rec::TPCCluster* const clust)
    {

      // Evidently, use of std::unordered_map keeps this method from being const
      std::vector<art::Ptr<gar::rec::Hit>> retval;
      if (!fTPCClusterIDToHits.empty()) { retval = fTPCClusterIDToHits[clust->getIDNumber()]; }
      return retval;
    }

    //-----------------------------------------------------------------------
    //std::vector<art::Ptr<rec::TPCCluster>> const
    std::vector<const rec::TPCCluster*> const BackTrackerCore::TrackToTPCClusters(
      rec::Track* const t)
    {
      ...
      if (!fTrackIDToTPCClusters.empty()) { retval = fTrackIDToTPCClusters[t->getIDNumber()]; }
      return retval;
    }
    ...
    //----------------------------------------------------------------------
    std::vector<std::pair<simb::MCParticle*, float>> BackTrackerCore::TrackToMCParticles(
      rec::Track* const t)
    {
      ...
      std::set<std::pair<int, float>, Comparator> setOfTracks(
        lTrackIdToEnergy.begin(),
        lTrackIdToEnergy.end(),
    ...
    //----------------------------------------------------------------------
    std::vector<art::Ptr<rec::Track>> BackTrackerCore::MCParticleToTracks(
      simb::MCParticle* const p,
      std::vector<art::Ptr<rec::Track>> const& tracks)
    {
      ...
      for (art::Ptr<rec::Track> iTrack : tracks) {
        ...
        for (std::pair<simb::MCParticle*, float> matches : fromMatch) {
          if (matches.first->TrackId() == desiredTrackId && matches.second > fTrackFracMCP) {
    ...
    //----------------------------------------------------------------------
    std::vector<art::Ptr<rec::CaloHit>> const BackTrackerCore::ClusterToCaloHits(
      rec::Cluster* const c)
    {
      ...
      if (!fClusterIDToCaloHits.empty()) { retval = fClusterIDToCaloHits[c->getIDNumber()]; }
      return retval;
    }

    //----------------------------------------------------------------------
    std::vector<std::pair<simb::MCParticle*, float>> BackTrackerCore::ClusterToMCParticles(
      rec::Cluster* const c)
    {
      ...
      // Declaring a set that will store the pairs using above comparison logic
      std::set<std::pair<int, float>, Comparator> setOfTracks(
        lClusterIdToEnergy.begin(),
        lClusterIdToEnergy.end(),
        [](std::pair<int, float> a, std::pair<int, float> b) { return a.second > b.second; });
    ...
    //----------------------------------------------------------------------
    std::vector<art::Ptr<rec::Cluster>> BackTrackerCore::MCParticleToClusters(
      simb::MCParticle* const p,
      std::vector<art::Ptr<rec::Cluster>> const& clusters)
    {
      ...
      for (art::Ptr<rec::Cluster> iCluster : clusters) {
        ...
        for (std::pair<simb::MCParticle*, float> matches : fromMatch) {
          if (matches.first->TrackId() == desiredTrackId && matches.second > fClusterFracMCP) {
    ...
    //----------------------------------------------------------------------
    bool BackTrackerCore::ClusterCreatedMCParticle(simb::MCParticle* const p, rec::Cluster* const c)
    {
    ...
    //----------------------------------------------------------------------
    bool BackTrackerCore::MCParticleCreatedCluster(simb::MCParticle* const p, rec::Cluster* const c)
    {
      std::vector<simb::MCParticle*> partsIsParts = MCPartsInCluster(c);
    ...
    //----------------------------------------------------------------------
    std::vector<simb::MCParticle*> BackTrackerCore::MCPartsInCluster(rec::Cluster* const c)
    {
      ...
      for (art::Ptr iCaloHit : caloHits) {
        ...
        for (CalIDE iIDE : IDEsThisHit)
          trackIdsInCluster.push_back(iIDE.trackID);
      }
