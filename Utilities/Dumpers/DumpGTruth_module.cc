/**
 * @file   garsoft/Utilities/DumpGTruth_module.cc
...
void sim::DumpGTruth::analyze(art::Event const& event)
{
  ...
  if (bAllTruth) {
    ...
    for (auto const& inputTag : fInputTruth) {
      AllTruths.emplace_back(event.getValidHandle<std::vector<simb::GTruth>>(inputTag));
    } // for
  }
  ...
  if (AllTruths.empty() && !bAllowNoTruth) {
    throw art::Exception(art::errors::ProductNotFound) << "No GENIE truth found to be dumped!\n";
  }
  ...
  if (bAllTruth) {
    mf::LogVerbatim(fOutputCategory)
      << "Event " << event.id() << " contains " << nTruths << " GENIE truth blocks in "
    ...
    mf::LogVerbatim(fOutputCategory) << "Dumping " << nTruths << " GENIE truth blocks from "
