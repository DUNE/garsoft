/**
 * @file   garsoft/Utilities/MCDumpers.h
...
#ifndef GARSOFT_MCDUMPERS_MCDUMPERS_H
#define GARSOFT_MCDUMPERS_MCDUMPERS_H
                               ...
                               std::string firstIndent)
{
  ...
  if (particle.Rescatter()) {
    out << " rescattered (" << particle.Rescatter() << ") at vertex " << particle.GetGvtx();
  }
  out << "\n"
  ...
  if (particle.Polarization().Mag2() != 0.) {
    out << " with polarization " << gar::dump::vector3D(particle.Polarization());
  }
...
#endif // GARSOFT_MCDUMPERS_MCDUMPERS_H
