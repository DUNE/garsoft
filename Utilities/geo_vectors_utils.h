/**
 * @file   garsoft/Utilities/geo_vectors_utils.h
 * @brief  Utilities to extend the interface of geometry vectors.
 * @author Gianluca Petrillo (petrillo@fnal.gov)
 * @date   November 27, 2017
 * @ingroup Geometry
 *
 * This library provides facilities that can be used for both GArSoft geometry
...
#ifndef GARSOFT_UTILITIES_GEO_VECTORS_UTILS_H
#define GARSOFT_UTILITIES_GEO_VECTORS_UTILS_H

#include "geo_vectors.h"
...
namespace geo {
  ...
  namespace vect {
    ...
    // --- BEGIN Support for GArSoft geometry vectors --------------------------
    /// @{
    /// @name Support for GArSoft geometry vectors
    ...
    /// @}
    // --- END Support for GArSoft geometry vectors ----------------------------
...
namespace geo {
  namespace vect {
    namespace details {
      ...
      template <typename Dest, typename Source>
      struct ConvertToImpl<Dest, Source, 2U> : private ConvertToImplBase<Dest, Source> {
        static Dest convert(Source const& v) { return {Xcoord(v)(), Ycoord(v)()}; }
      }; // struct ConvertToImpl<2U>

      template <typename Dest, typename Source>
      struct ConvertToImpl<Dest, Source, 3U> : private ConvertToImplBase<Dest, Source> {
        static Dest convert(Source const& v) { return {Xcoord(v)(), Ycoord(v)(), Zcoord(v)()}; }
      }; // struct ConvertToImpl<3U>

      template <typename Dest, typename Source>
      struct ConvertToImpl<Dest, Source, 4U> : private ConvertToImplBase<Dest, Source> {
      ...
      template <typename Dest, typename Source, unsigned int Dim = dimension<Source>()>
      ...
      struct ConvertToDispatcher<Vector, Vector, Dim> {
        static_assert(Dim == dimension<Vector>(), "Inconsistent vector dimension");
...
#endif // GARSOFT_UTILITIES_GEO_VECTORS_UTILS_H
