...
//-----------------------------------------------------------------------------------------
gar::detinfo::DetectorClocksServiceStandardGAr::DetectorClocksServiceStandardGAr(
...
//------------------------------------------------------------
void gar::detinfo::DetectorClocksServiceStandardGAr::preProcessEvent(::art::Event const& evt,
                                                                     art::ScheduleContext)
//------------------------------------------------------------
{
  ...
  fClocks->SetTriggerTime(trig_ptr->TriggerTime(), trig_ptr->BeamGateTime());
    ...
    if (file != 0 && !file->IsZombie() && file->IsOpen()) {
      ...
      for (size_t i = 0; i < detinfo::kInheritConfigTypeMax; ++i)
        ...
        if (config_count.at(i) && cfgValue.at(i) != config_value.at(i)) {
          ...
          MF_LOG_INFO("DetectorClocksServiceStandardGAr") << Form(
...
DEFINE_ART_SERVICE_INTERFACE_IMPL(gar::detinfo::DetectorClocksServiceStandardGAr,
