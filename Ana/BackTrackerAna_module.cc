...
BackTrackerAna::BackTrackerAna(fhicl::ParameterSet const& p) : EDAnalyzer{p} // ,
...
void BackTrackerAna::beginJob()
{
  ...
  fHistGenDiff = tfs->make<TH1F>("hGenDiff",
                                 "gen stage;E_{final} - E_{initial} [MeV]",
                                 101,
                                 -500,
                                 500); //genie conserves energy?
  fHistG4Diff = tfs->make<TH1F>("hG4Diff",
                                "G4 stage;E_{final} - E_{initial} [GeV]",
                                301,
                                -1000,
                                2000); //geant conserves energy?
  fHistEdeps = tfs->make<TH1F>(
    "hEdeps", "G4 stage - sdp::EnergyDeposit; remaining MCParticle KE [GeV]", 200, -10, 10);
  fHistHitCompleteE = tfs->make<TH1F>(
    "hHitCompleteE", "reco stage: TPC hit;remaining MCParticle energy [GeV]", 200, -10, 10);
  fHistHitCompleteKE = tfs->make<TH1F>(
    "hHitCompleteKE", "reco stage: TPC hit;remaining MCParticle energy [GeV]", 200, -10, 10);
  fHistHitEnergy =
    tfs->make<TH1F>("hHitEnergy", "TPC Hit -> deposited energy; energy per hit [MeV]", 1000, 0, 10);
  fHistTrackHitDiff = tfs->make<TH1F>(
    "hTrackHitDiff", "reco stage: TPC track vs. hit;track - sum hit energy [GeV]", 51, -2, 2);
  //fHistTrackTotHitSum = tfs->make<TH1F>("hTrackTotHitSum","reco stage: TPC track vs hit;
  fHistTrackHitMult = tfs->make<TH1F>(
    "hTrackHitMult", "reco stage: TPC Track #rightarrow hit;hit duplication", 30, -0.5, 29.5);
  fHistClustHitMult = tfs->make<TH1F>(
    "hClustHitMult", "reco stage: TPC cluster #rightarrow hit;hit duplication", 30, -0.5, 29.5);
  fHistClustMult = tfs->make<TH1F>(
    "hClustMult", "reco stage: TPC track #rightarrow cluster;cluster duplication", 30, -0.5, 29.5);
  fHistNHitAssocTrackNHitTotDiff = tfs->make<TH1F>(
    "hNHitAssocTrackNHitTotDiff",
    "TPC Track #rightarrow hit;total N^{0} hits - total N^{0} hits assoc'd to tracks",
    11001,
    -1000,
    10000);

  fHistEdepRelKE = tfs->make<TH1F>("hEdepRelKE",
                                   "Track -> MCParticles;E_{dep}/KE_{initial}",
                                   29,
                                   0,
                                   29.001); //want 1 in the first bin
  fHistG4Diff2d = tfs->make<TH2F>(
    "hG4Diff2d", "G4 stage;E_{initial} [GeV]; E_{final} [GeV]", 100, 0, 1000, 100, 0, 1000);
...
void BackTrackerAna::analyze(art::Event const& e)
{
  ...
  for (auto const& gth : gthandlelist) {
    ...
    for (auto const& gt : *gth) {
      ...
      fHistGenDiff->Fill(
        (gt.fFShadSystP4.E() + gt.fFSleptonP4.E() - gt.fProbeP4.E() - gt.fTgtP4.E()));
  ...
  map<int, double> mcpIdToKE, trkIdToERemain, trkIdToKERemain,
    trkIdToEdep; //maps for bookkeeping MCParticle energy
  ...
  for (auto mcp : *MCPHandle) {
    ...
    // is it primary?
    if (mcp.Process() == "primary") { eprim += mcp.E(); }

    if (fGeo->PointInGArTPC(mcp.Position().Vect())) {
      trkIdToKERemain[mcp.TrackId()] =
        mcp.E() - mcp.Momentum().M();          // fill with MCParticles initial KE
    ...
    if (mcp.E() < mcp.EndE())
      std::cout << "MCParticle with final energy > intial energy ??? Ef-Ei = "
                << mcp.EndE() - mcp.E() << std::endl;

    double deltaEDep = mcp.E() - mcp.Momentum().M() - trkIdToEdep[mcp.TrackId()];
    if (deltaEDep < 0 && deltaEDep > 1e-11) deltaEDep = 0.;
    fHistEdeps->Fill(deltaEDep);
    if (deltaEDep < -1e-11)
      cout << "negative remaining energy from EnergyDeposit subtraction"
           << "\n\ttrackID = " << mcp.TrackId() << "\n\tPDG = " << mcp.PdgCode()
           << "\n\tinitial KE = " << mcp.E() - mcp.Momentum().M() << "\n\tMCP remaining energy = "
           << mcp.E() - mcp.Momentum().M() - trkIdToEdep[mcp.TrackId()] << endl;
  ...
  map<rec::Hit, int> track_hitcounts,
    cluster_hitcounts; //count instances of hits found from track/cluster associations
  //int nprint = 0;
  for (rec::Hit const hit : *TPCHitHandle) {
    ...
    for (cheat::HitIDE const& ide : fBt->HitToHitIDEs(hit)) {
      ...
      trkIdToKERemain[ide.trackID] -=
        ide.energyTot; //subtract energy from MCParticle's available (kinetic) energy
  ...
  map<const rec::IDNumber, int>
    tpcclustcounts; //map of all TPCClusters to num instances in associations
    ...
    for (std::pair<simb::MCParticle*, float> const& mcp : trkmcps) {
      ...
      if (eratio > 1)
        cout << "bad true contributed energy to track:MCParticle initial KE ratio!"
             ...
             << "\n\tAvg. dE/dx = "
             << (mcp.first->E() - mcp.first->EndE()) * 1e3 / mcp.first->Trajectory().TotalLength()
             << " MeV/cm"
