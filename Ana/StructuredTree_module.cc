...
namespace gar {
  ...
  private:
    ...
    //config
    string
      fAnaMode; ///< analysis configs: (gen)eral, (reco) R&D or (readout) sim R&D, default="gen"
    ...
    // genTree
    vector<Int_t>
      fGIndex; ///< index of GTruth object associated with MCTruth in this row (-1 if not associated, e.g. CRY event)
    ...
    vector<const simb::MCTruth*>
      fMCTruths; //needed for G4Particle -> MCTruth (FS particles) matching
...
void gar::StructuredTree::beginJob()
{
  ...
  if (fWriteDisplay) {
    fDisplayTree =
      tfs->make<TTree>("displayTree", "truth/reco level traj points for event display");
...
void gar::StructuredTree::endRun(art::Run const& run)
{
  ...
  MF_LOG_INFO("anatree") << "POT for this file is " << fPOT << " The number of spills is "
                         << fNSpills;
}
...
void gar::StructuredTree::ClearVectors()
{
  ...
  //ECal/MuID clusters
  fCalClusters.clear();
  if (fGeo->HasMuonDetector()) { fMuClusters.clear(); }
    ...
    for (size_t imct = 0; imct < mcthandle->size(); imct++) {
      ...
      if (mct.GeneratorInfo().generator == simb::_ev_generator::kGENIE) {
        ...
        for (auto const& gth : gthandlelist) {
          ...
          if ((*gth).size() != 1)
            throw cet::exception("StructuredTree") << "more than 1 GTruth in MCTruth! ("
          ...
          garana::GTruth gtout =
            MakeAnaGTruth(gt, fRegionNameToID.at(PointToRegion(100 * gt.fVertex.Vect())));
          gtout.fVertex -= fTpcCenter;
          fGTruth.push_back(
            gtout); //MakeAnaGTruth( gt, fRegionNameToID.at( PointToRegion(100*gt.fVertex.Vect()) ) ) );
      ...
      for (int imcp = 0; imcp < mct.NParticles(); imcp++) {
        ...
        if (mcp.StatusCode() == 1) { //GENIE specific???
          ...
          mf::LogDebug("StructuredTree::FillGenTree")
            << "FS particle with trackID " << (int)mcp.TrackId() << ", PDG " << mcp.PdgCode()
            << ", "
            << "4-position (" << mcp.Position(0).X() << ", " << mcp.Position(0).Y() << ", "
            << mcp.Position(0).Z() << ", " << mcp.Position(0).T() << "), and "
...
void gar::StructuredTree::FillG4Tree(Event const& e)
{
  ...
  for (auto mcp : *MCPHandle) {
    ...
    if (mcp.Mother() > 0) {
      ...
      //std::cout
      mf::LogDebug("StructuredTree")
        << "G4 primary with trackID " << mcp.TrackId() << ", PDG " << mcp.PdgCode()
        << ", initial 4-position (" << mcp.Position(0).X() << "," << mcp.Position(0).Y() << ","
    ...
    for (size_t ipt = 0; ipt < mcp.NumberTrajectoryPoints(); ipt++) {
      ...
      std::string regionName =
        PointToRegion(mcp.Position(ipt).Vect()); //fGeo->VolumeName(mcp.Position(ipt).Vect());
    ...
    // sanity check
    if (momenta.size() != positions.size())
      std::cout << "G4Particle: positions-momenta size mismatch" << std::endl;
    if (momenta.size() != regions.size())
      std::cout << "G4Particle: positions/momenta - regions size mismatch" << std::endl;

    // make a G4Particle
    fG4Particles.push_back(MakeG4Particle(
      mcp, parentPdg, progenitorPdg, progenitorId, positions, momenta, regions, npointsPerRegion));
    ...
    for (size_t i = 0; i < fMCTruths.size(); i++) {
      if (fMCTruths[i] == mcmatch.get()) { //compare pointers to check MCTruth equality
        ...
        if (!foundfs) {
          ...
          //"WARNING in StructuredTree_module::FillG4Tree: FSParticle association not found!"
          //<< std::endl;
          fG4FSIndex.push_back(
            UINT_MAX); //need to push back something or association ordering is broken
        }
    ...
    if (!found) {
      mf::LogWarning("StructuredTree::FillG4Tree") << "no MCTruth found for MCParticle";
  ...
  if (fAnaMode == "reco") {
    ...
    if (fGeo->HasMuonDetector()) fSimMuHits.resize(fG4Particles.size());
...
void gar::StructuredTree::FillHighLevelRecoInfo(Event const& e)
{
  ...
  findManyCalTrackEnd =
    new art::FindManyP<rec::Track /*, rec::TrackEnd*/>(CalClusterHandle, e, fECALAssnLabel);
  ...
  for (auto /*const&*/ track : *TrackHandle) { //backtracker takes ptr to non-const obj
    ...
    vector<pair<MCParticle*, float>> trackmcps =
      fBt->TrackToMCParticles(&track); //MCParticle w/fraction energy contributed to track
    ...
    for (auto const& mcpe : trackmcps) {
      ...
      for (size_t i = 0; i < fG4Particles.size(); i++) {
        ...
        if (mcpe.first->TrackId() ==
            fG4Particles[i].TrackID()) //fMCParticles and fG4Particles have same mapping
          indices.push_back(i);
      }
    }
    ...
    fTracks.push_back(MakeAnaTrack(track,
                                   pidF,
                                   pidB,
                                   avgIonF,
                                   avgIonB,
  ...
  for (auto cluster : *CalClusterHandle) {
    ...
    std::string regionName =
      PointToRegion(TVector3(cluster.Position())); //fGeo->VolumeName(mcp.Position(ipt).Vect());
    ...
    if (hits.size() != 0 && n0ide > 0)
      mf::LogDebug("FillHighLevelReco")
        << "     hits with no IDEs: " << n0ide << " of " << hits.size() << " hits affected ("
        << 100.0 * (hits.size() - n0ide) / hits.size() << " %)"; // << std::endl;
    ...
    size_t nMatch = 0;
    if (findManyCalTrackEnd->isValid()) { nMatch = findManyCalTrackEnd->at(iCluster).size(); }
...
std::string gar::StructuredTree::PointToRegion(const TVector3& point)
{
  ...
  if (fGeo->PointInMPD(point)) {
    ...
    else if (fGeo->PointInECALBarrel(point))
      region = "BarrelECal";
    else if (fGeo->PointInECALEndcap(point))
      region = "EndcapECal";
...
void gar::StructuredTree::processIonizationInfo(rec::TrackIoniz& ion,
                                                float ionizeTruncate,
                                                float& forwardIonVal,
                                                float& backwardIonVal)
{
