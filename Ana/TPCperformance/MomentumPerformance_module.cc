...
void gar::MomentumPerformance::FillVectors(art::Event const& event)
{
  ...
  if (mctruthHandles.size() != 1) {
    throw cet::exception("MomentumPerformance")
  ...
  for (simb::MCParticle mcp : *MCPHandle) {
    ...
    if (momTrkId > 0) {
      if (TrackIdToIndex.find(momTrkId) != TrackIdToIndex.end()) {
        ...
        throw cet::exception("MomentumPerformance")
          << " mcp trackid " << mcp.TrackId() << " pdg code " << mcp.PdgCode()
          << " could not find mother trk id " << momTrkId
          << " in the TrackIdToIndex map; creating process is " << mcp.Process() << "\nLine "
    ...
    TVector3* trackInPhase;
    TVector3* trackInSpace;
    ...
    float cosT = (*trackInPhase)[0] * momentumMCP.Px() + (*trackInPhase)[1] * momentumMCP.Py() +
    ...
    if (iPickedTrack >= 0 && findIonization->isValid()) {
      ...
      if (kate == rec::TrackEndBeg) { fTrackAvgIon.push_back(avgIonF); }
      else {
