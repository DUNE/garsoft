...
void gar::MatchingPerformance::beginJob()
{
  ...
  if (fVerbosity > 0) {
    chargeFrac =
      tfs->make<TH1F>("chargeFrac", "Fraction of MCPart's ionization in track", 101, 0.00, 1.01);
  }
...
void gar::MatchingPerformance::analyze(art::Event const& event)
{
  ...
  fMaxXdisplacement =
    fDetProp->DriftVelocity(fDetProp->Efield(), fDetProp->Temperature()) * fClocks->SpillLength();
  ...
  if (!fBack->HasTracks() || !fBack->HasClusters()) {
    throw cet::exception("MatchingPerformance")
      << " Not enough backtracker info"
      << " Line " << __LINE__ << " in file " << __FILE__ << std::endl;
  }

  ClearVectors();
  if (FillVectors(event)) { fTree->Fill(); }
...
bool gar::MatchingPerformance::FillVectors(art::Event const& event)
{
  ...
  findManyTrackEndCAL =
    new art::FindMany<rec::Cluster, rec::TrackEnd>(TrackHandle, event, ecalassntag);
  if (!findManyTrackEndCAL->isValid()) {
    throw cet::exception("MatchingPerformance")
      << " Bad TrackEnd-ECAL Assn."
      << " Line " << __LINE__ << " in file " << __FILE__ << std::endl;
  }
  ...
  for (; iNiceTrk != niceTracks.end(); ++iNiceTrk) {
    ...
    std::vector<float> ionzFromSameMCP;
    ionzFromSameMCP.resize(tracksFromSameMCP.size());
  ...
  for (size_t iNiceTrk_local = 0; iNiceTrk_local < niceTracks.size(); ++iNiceTrk_local) {
    ...
    if (fGeo->PointInGArTPC(positionMCP)) {
      ...
      float distStart =
        std::hypot(track.Vertex()[1] - positionMCP[1], track.Vertex()[2] - positionMCP[2]);
      float distEnd = std::hypot(track.End()[1] - positionMCP[1], track.End()[2] - positionMCP[2]);
      if (distStart < distEnd) { endList.pop_front(); }
      else {
        endList.pop_back();
      }
    }
    ...
    if ((*iEnd) == rec::TrackEndBeg) {
      for (size_t i = 0; i < 5; ++i)
        trackPar[i] = track.TrackParBeg()[i];
      for (size_t i = 0; i < 3; ++i)
        trackEnd[i] = track.Vertex()[i];
      ...
      for (size_t i = 0; i < 5; ++i)
        trackPar[i] = track.TrackParEnd()[i];
      for (size_t i = 0; i < 3; ++i)
        trackEnd[i] = track.End()[i];
    ...
    trackPar[0] -= ItsInTulsa[1];
    trackPar[1] -= ItsInTulsa[2];
    for (int i = 0; i < 3; ++i)
      trackEnd[i] -= ItsInTulsa[i];
    ...
    // lambda function, etc.
    std::sort(clustersOnMCP.begin(), clustersOnMCP.end(), [](rec::Cluster a, rec::Cluster b) {
    ...
    float eAssocNoInTruth = 0.0;
    int nAssocNoInTruth = 0;
    float eUnassocInTruth = 0.0;
    int nUnassocInTruth = 0;
    float eIsassocInTruth = 0.0;
    ...
    for (auto const& cluster : (*ClusterHandle)) {
      ...
      if (inECALBarrel) {
        ...
        if (errcode == 0) {
          ...
          bool looneyExtrap;
          if (transDist1 < transDist2) { trackXYZ.SetXYZ(retXYZ1[0], retXYZ1[1], retXYZ1[2]); }
          else {
            trackXYZ.SetXYZ(retXYZ2[0], retXYZ2[1], retXYZ2[2]);
          }
          trackXYZ += TVector3(ItsInTulsa);
          looneyExtrap = !fGeo->PointInECALBarrel(trackXYZ) && !fGeo->PointInECALEndcap(trackXYZ);
          trackXYZ -= TVector3(ItsInTulsa);
          if (!looneyExtrap) {
        ...
        float radiansInDrift = trackPar[2] * fMaxXdisplacement / tan(trackPar[4]);
        if (abs(radiansInDrift) >= 2.0 * M_PI) goto angleCut;
        int errcode = util::TrackPropagator::PropagateToX(trackPar, trackEnd, xClus, retXYZ1);
        if (errcode == 0) {
          trackXYZ.SetXYZ(retXYZ1[0], retXYZ1[1], retXYZ1[2]);
          trackXYZ += TVector3(ItsInTulsa);
          bool looneyExtrap =
            !fGeo->PointInECALBarrel(trackXYZ) && !fGeo->PointInECALEndcap(trackXYZ);
          trackXYZ -= TVector3(ItsInTulsa);
          if (!looneyExtrap) {
    ...
    angleCut:
      ...
      if (nCells >= fClusterDirNhitCut) {
        ...
        int errcode = util::TrackPropagator::DirectionX(trackPar, trackEnd, xEval, trackDir);
        if (errcode == 0) {
          TVector3 clusterDir;
          clusterDir.SetXYZ(
            cluster.EigenVectors()[0], cluster.EigenVectors()[1], cluster.EigenVectors()[2]);
      ...
      if (!clusterOnMCP) {
        ...
        if (inECALBarrel) {
          if (over25) { fXClusTrackOver25Fal.push_back(cutQuantity); }
          else {
        ...
        if (inECALBarrel) {
          if (over25) { fXClusTrackOver25Tru.push_back(cutQuantity); }
          else {
