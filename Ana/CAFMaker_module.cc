...
namespace gar {
  ...
  private:
    ...
    std::string
      fTrackTrajectoryLabel;  ///< module label for TPC Track Trajectories rec:TrackTrajectory
  ...
  for (size_t imchl = 0; imchl < mcthandlelist.size(); ++imchl) {
    for (auto const& mct : (*mcthandlelist.at(imchl))) {
      if (mct.NeutrinoSet()) {
        ...
        if (fWriteCohInfo) { srnu.T = computeT(mct); }
...
void gar::CAFMaker::FillHighLevelRecoInfo(art::Event const& e, caf::StandardRecord& rec)
{
  ...
  if (fWriteCaloClusters) {
    ...
    if (fWriteTracks)
      findManyCALTrackEnd =
        new art::FindManyP<rec::Track, rec::TrackEnd>(RecoClusterHandle, e, fECALAssnLabel);
  }
  ...
  if (fWriteTracks) {
    ...
    for (auto const& track : (*TrackHandle)) {
      ...
      srtrk.start = caf::SRVector3D(track.Vertex()[0], track.Vertex()[1], track.Vertex()[2]);
      ...
      srtrk.end = caf::SRVector3D(track.End()[0], track.End()[1], track.End()[2]);
      ...
      if (srtrk.mcidx > -1) { srtrk.mcfrac = trakt[0].second; }
      else {
        srtrk.mcfrac = 0;
      }
  ...
  if (fWriteTrackTrajectories) {
    ...
    for (auto const& tracktraj : (*TrackTrajHandle)) {
      ...
      for (size_t i = 0; i < temp.size(); i++) {
        srtrk.fwd.traj.emplace_back(temp.at(i).X(), temp.at(i).Y(), temp.at(i).Z());
      }

      temp = tracktraj.getBAKTrajectory();
      for (size_t i = 0; i < temp.size(); i++) {
        srtrk.bak.traj.emplace_back(temp.at(i).X(), temp.at(i).Y(), temp.at(i).Z());
      }
      iTrackTraj++;
    }
  }
  ...
  if (fWriteVertices) {
    ...
    for (auto const& vertex : (*VertexHandle)) {
      ...
      srvtx.ntrks = 0;
      if (findManyTrackEnd->isValid()) { srvtx.ntrks = findManyTrackEnd->at(iVertex).size(); }
  ...
  if (fWriteCaloClusters) {
    for (auto const& cluster : (*RecoClusterHandle)) {
      ...
      if (srclust.mcidx > -1) { srclust.mcfrac = trakt[0].second; }
      else {
        srclust.mcfrac = 0;
      }
...
void gar::CAFMaker::processIonizationInfo(rec::TrackIoniz& ion,
                                          float ionizeTruncate,
                                          float& forwardIonVal,
                                          float& backwardIonVal)
{
...
float gar::CAFMaker::processOneDirection(std::vector<std::pair<float, float>> SigData,
                                         float ionizeTruncate)
{
