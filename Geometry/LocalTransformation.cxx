...
namespace gar {
  namespace geo {
    ...
    //----------------------------------------------------------------------------
    template <>
    HepGeom::Transform3D LocalTransformation<HepGeom::Transform3D>::transformationFromPath(
      std::vector<TGeoNode const*> const& path,
      size_t depth)
    {

      auto const mat = LocalTransformation<TGeoHMatrix>::transformationFromPath(path, depth);
