...
namespace gar {
  namespace geo {
    ...
    GeometryCore::GeometryCore(fhicl::ParameterSet const& pset)
      ...
      , fIndsOfRefract(pset.get<fhicl::ParameterSet>("IndsOfRefract", fhicl::ParameterSet()))
    {
    ...
    //......................................................................
    void GeometryCore::ApplyECALSegmentationAlg(
      std::shared_ptr<geo::seg::SegmentationAlg> pECALSegmentationAlg)
    {
    ...
    //......................................................................
    void GeometryCore::ApplyMinervaSegmentationAlg(
      std::shared_ptr<geo::seg::SegmentationAlg> pMinervaSegmentationAlg)
    {
    ...
    //......................................................................
    void GeometryCore::ApplyMuIDSegmentationAlg(
      std::shared_ptr<geo::seg::SegmentationAlg> pMuIDSegmentationAlg)
    {
                                        ...
                                        bool bForceReload /* = false*/)
    {
      ...
      MF_LOG_INFO("GeometryCore") << "New detector geometry loaded from "
                                  << "\n\t" << fROOTfile << "\n\t" << fGDMLfile;
    ...
    float GeometryCore::GetSensVolumeThickness(const TVector3& point) const
    {
      ...
      if (node) { return this->FindShapeSize(node)[2] * 2; }
      else {
        return 0.;
      }
    }
    ...
    const std::array<double, 3> GeometryCore::FindShapeSize(const TGeoNode* node) const
    {
      ...
      if (volname.find("PV") != std::string::npos) { isBarrel = false; }
      ...
      if (vol) {
        ...
        throw cet::exception("GeometryCore::FindShapeSize")
          << "Could not find the volume associated to node " << node->GetName() << "\n";
      }
    }
                                       ...
                                       std::vector<const TGeoNode*>& path) const
    {
      ...
      if (strncmp(name.c_str(), pCurrentVolume->GetName(), name.length()) == 0) return true;
    ...
    //......................................................................
    void GeometryCore::StoreECALNodes(
      std::map<std::string, std::vector<const TGeoNode*>>& map) const
    {
        ...
        for (unsigned int istave = 0; istave < nstave; istave++) {
          for (unsigned int imodule = 0; imodule < nmodule; imodule++) {
            for (unsigned int ilayer = 0; ilayer < nlayer; ilayer++) {
              boost::format bname =
                boost::format("%sECal_stave%02i_module%02i_layer_%02i_slice2_vol") %
                det[idet].c_str() % istave % imodule % ilayer;
    ...
    //......................................................................
    template <>
    TGeoNode* GeometryCore::FindNode<double>(double const& x,
                                             double const& y,
    ...
    //......................................................................
    bool GeometryCore::WorldToLocal(std::array<double, 3> const& world,
                                    std::array<double, 3>& local,
                                    gar::geo::LocalTransformation<TGeoHMatrix>& trans) const
    {
      ...
      if (path.empty()) {
        throw cet::exception("GeometryCore::WorldToLocal")
    ...
    //......................................................................
    bool GeometryCore::LocalToWorld(std::array<double, 3> const& local,
                                    std::array<double, 3>& world,
                                    gar::geo::LocalTransformation<TGeoHMatrix> const& trans) const
    {
    ...
    //
    void GeometryCore::WorldBox(float* xlo,
                                float* xhi,
                                float* ylo,
                                float* yhi,
                                float* zlo,
                                float* zhi) const
    {
      ...
      if (!s) throw cet::exception("GeometryCore") << "no pointer to world volume TGeoShape\n";
    ...
    bool GeometryCore::PointInDetEnclosure(TVector3 const& point) const
    {
      ...
      if (std::abs(point.x()) > fEnclosureHalfWidth || std::abs(point.y()) > fEnclosureHalfHeight ||
        ...
        if (fPointInWarnings) {
          MF_LOG_WARNING("GeometryCoreBadInputPoint")
            ...
            << " half width = " << fEnclosureHalfWidth << " half height = " << fEnclosureHalfHeight
    ...
    bool GeometryCore::PointInMPD(TVector3 const& point) const
    {
      ...
      // check that the given point is in the enclosure volume at least
      if (std::abs(new_point.x()) > fMPDHalfWidth || std::abs(new_point.y()) > fMPDHalfHeight ||
        ...
        if (fPointInWarnings) {
          MF_LOG_WARNING("GeometryCoreBadInputPoint")
            ...
            << " half width = " << fMPDHalfWidth << " half height = " << fMPDHalfHeight
    ...
    bool GeometryCore::PointInGArTPC(TVector3 const& point) const
    {
      ...
      if (std::abs(point.x() - GetOriginX()) > fTPCLength / 2.0 || std::hypot(z, y) > fTPCRadius) {
        if (fPointInWarnings) {
          MF_LOG_WARNING("GeometryCoreBadInputPoint")
            << "point (" << point.x() << "," << point.y() << "," << point.z() << ") "
            << "is not inside the GArTPC volume "
            << " radius = " << fTPCRadius << " length = " << fTPCLength;
        }
        return false;
      }
    ...
    bool GeometryCore::PointInECALBarrel(TVector3 const& point) const
    {
      ...
      if (vol_name.find("ECal") != std::string::npos ||
          vol_name.find("ECAL") != std::string::npos || vol_name.find("ecal") != std::string::npos)
        return true;
    ...
    bool GeometryCore::PointInMuIDBarrel(TVector3 const& point) const
    {
      ...
      if (vol_name.find("Yoke") != std::string::npos || vol_name.find("yoke") != std::string::npos)
        return true;
    ...
    double GeometryCore::MassBetweenPoints(double* p1, double* p2) const
    {
      ...
      double length = std::sqrt(sqr(p2[0] - p1[0]) + sqr(p2[1] - p1[1]) + sqr(p2[2] - p1[2]));
      double dxyz[3] = {
        (p2[0] - p1[0]) / length, (p2[1] - p1[1]) / length, (p2[2] - p1[2]) / length};
      ...
      length =
        std::sqrt(sqr(p2[0] - current[0]) + sqr(p2[1] - current[1]) + sqr(p2[2] - current[2]));
    ...
    //----------------------------------------------------------------------------
    gar::raw::CellID_t GeometryCore::GetCellID(const TGeoNode* node,
                                               const unsigned int& det_id,
                                               const unsigned int& stave,
                                               const unsigned int& module,
                                               const unsigned int& layer,
                                               const unsigned int& slice,
                                               const std::array<double, 3>& localPosition) const
    {
      ...
      if (node_name.find("ECal") != std::string::npos ||
        ...
        cellID = fECALSegmentationAlg->GetCellID(
          *this, det_id, stave, module, layer, slice, localPosition);
      }
      else if (node_name.find("TrackerSc") != std::string::npos ||
               node_name.find("trackersc") != std::string::npos ||
               node_name.find("Tracker") != std::string::npos ||
               node_name.find("tracker") != std::string::npos) {

        const std::array<double, 3> shape = this->FindShapeSize(node);
        fMinervaSegmentationAlg->setLayerDimXY(shape[0] * 2, shape[1] * 2);
        cellID =
          fMinervaSegmentationAlg->GetCellID(*this, det_id, 0, 0, layer, slice, localPosition);
      }
      else if (node_name.find("Yoke") != std::string::npos ||
               node_name.find("yoke") != std::string::npos) {

        const std::array<double, 3> shape = this->FindShapeSize(node);
        fMuIDSegmentationAlg->setLayerDimXY(shape[0] * 2, shape[1] * 2);
        cellID = fMuIDSegmentationAlg->GetCellID(
          *this, det_id, stave, module, layer, slice, localPosition);
      }
      else {
        MF_LOG_WARNING("GeometryCore::GetCellID") << "Detector id " << det_id << " unknown!"
                                                  << " Node name " << node_name;
      }
    ...
    //----------------------------------------------------------------------------
    std::array<double, 3> GeometryCore::GetPosition(const TGeoNode* node,
                                                    const gar::raw::CellID_t& cID) const
    {
      std::string node_name = node->GetName();
    ...
    //----------------------------------------------------------------------------
    bool GeometryCore::isTile(const std::array<double, 3>& point,
                              const gar::raw::CellID_t& cID) const
    {
      bool isTile = false;
    ...
    //----------------------------------------------------------------------------
    double GeometryCore::getStripLength(const std::array<double, 3>& point,
                                        const gar::raw::CellID_t& cID) const
    {
    ...
    //----------------------------------------------------------------------------
    std::pair<TVector3, TVector3> GeometryCore::GetStripEnds(const std::array<double, 3>& point,
                                                             const gar::raw::CellID_t& cID) const
    {
      std::string node_name = this->FindNode(point)->GetName();
      ...
      std::array<double, 3> stripEnd1local = {
        localStripEnds.first.X(), localStripEnds.first.Y(), localStripEnds.first.Z()};
      std::array<double, 3> stripEnd2local = {
        localStripEnds.second.X(), localStripEnds.second.Y(), localStripEnds.second.Z()};
      ...
      return std::make_pair(TVector3(stripEnd1[0], stripEnd1[1], stripEnd1[2]),
                            TVector3(stripEnd2[0], stripEnd2[1], stripEnd2[2]));
    }

    //----------------------------------------------------------------------------
    std::pair<float, float> GeometryCore::CalculateLightPropagation(
    ...
    //----------------------------------------------------------------------------
    std::array<double, 3> GeometryCore::ReconstructStripHitPosition(
      const std::array<double, 3>& point,
      const std::array<double, 3>& local,
      const float& xlocal,
      const gar::raw::CellID_t& cID) const
    ...
    void GeometryCore::GetGeometryParameters()
    {
      ...
      if (fHasGasTPCDetector) StoreTPCParameters();
      if (fHasECALDetector) StoreECALParameters();
      if (fHasTrackerScDetector) { /* no op */ }
      if (fHasMuonDetector) StoreMuIDParameters();
    ...
    void GeometryCore::FinalizeGeometryParameters()
    {
      ...
      if (fHasMuonDetector) { FindMuIDnLayers(); }

      if (fHasTrackerScDetector) { FindTrackerScnPlanes(); }
    }
    ...
    void GeometryCore::PrintGeometry() const
    {
      ...
      std::cout << "------------------------------" << std::endl;
      std::cout << "ND-GAr detector origin set to " << GetOriginX() << " cm " << GetOriginY()
                << " cm " << GetOriginZ() << " cm" << std::endl;

      //Prints geometry parameters
      std::cout << "------------------------------" << std::endl;
      std::cout << "World Geometry" << std::endl;
      std::cout << "World Origin (x, y, z) " << GetWorldX() << " cm " << GetWorldY() << " cm "
                << GetWorldZ() << " cm" << std::endl;
      std::cout << "World Size (H, W, L) " << GetWorldHalfWidth() << " cm " << GetWorldHalfHeight()
                << " cm " << GetWorldLength() << " cm" << std::endl;

      if (this->HasRock()) {
        std::cout << "------------------------------" << std::endl;
        std::cout << "Rock Geometry" << std::endl;
        std::cout << "Rock Origin (x, y, z) " << GetRockX() << " cm " << GetRockY() << " cm "
                  << GetRockZ() << " cm" << std::endl;
        std::cout << "Rock Size (H, W, L) " << GetRockHalfWidth() << " cm " << GetRockHalfHeight()
                  << " cm " << GetRockLength() << " cm" << std::endl;
      }

      if (this->HasEnclosure()) {
        std::cout << "------------------------------" << std::endl;
        std::cout << "Enclosure Geometry" << std::endl;
        std::cout << "Enclosure Origin (x, y, z) " << GetEnclosureX() << " cm " << GetEnclosureY()
                  << " cm " << GetEnclosureZ() << " cm" << std::endl;
        std::cout << "Enclosure Size (H, W, L) " << GetEnclosureHalfWidth() << " cm "
                  << GetEnclosureHalfHeight() << " cm " << GetEnclosureLength() << " cm"
                  << std::endl;
      }
      ...
      if (this->HasLArTPCDetector()) {
        ...
        std::cout << "LArTPC Origin (x, y, z) " << GetLArTPCX() << " cm " << GetLArTPCY() << " cm "
                  << GetLArTPCZ() << " cm" << std::endl;
        std::cout << "LArTPC Size (H, W, L) " << GetLArTPCHalfWidth() << " cm "
                  << GetLArTPCHalfHeight() << " cm " << GetLArTPCLength() << " cm" << std::endl;
        std::cout << "------------------------------" << std::endl;
        std::cout << "LArActiveArgonCube Geometry" << std::endl;
        std::cout << "LArTPCActive Origin (x, y, z) " << GetActiveLArTPCX() << " cm "
                  << GetActiveLArTPCY() << " cm " << GetActiveLArTPCZ() << " cm" << std::endl;
        std::cout << "LArTPCActive Size (H, W, L) " << GetActiveLArTPCHalfWidth() << " cm "
                  << GetActiveLArTPCHalfHeight() << " cm " << GetActiveLArTPCLength() << " cm"
                  << std::endl;
      }

      std::cout << "------------------------------" << std::endl;
      std::cout << "ND-GAr Geometry" << std::endl;
      std::cout << "ND-GAr Origin (x, y, z) " << GetMPDX() << " cm " << GetMPDY() << " cm "
                << GetMPDZ() << " cm" << std::endl;
      std::cout << "ND-GAr Size (H, W, L) " << GetMPDHalfWidth() << " cm " << GetMPDHalfHeight()
                << " cm " << GetMPDLength() << " cm" << std::endl;

      if (this->HasGasTPCDetector()) {
        std::cout << "------------------------------" << std::endl;
        std::cout << "TPC Geometry" << std::endl;
        std::cout << "TPC Origin (x, y, z) " << TPCXCent() << " cm " << TPCYCent() << " cm "
                  << TPCZCent() << " cm" << std::endl;
        std::cout << "TPC Active Volume Size (R, L) " << TPCRadius() << " cm " << TPCLength()
                  << " cm" << std::endl;
      ...
      if (this->HasECALDetector()) {
        ...
        std::cout << "ECAL Barrel inner radius: " << GetECALInnerBarrelRadius() << " cm"
                  << std::endl;
        std::cout << "ECAL Barrel outer radius: " << GetECALOuterBarrelRadius() << " cm"
                  << std::endl;
        std::cout << "ECAL Endcap inside PV: " << !fECALEndcapOutside << std::endl;
        std::cout << "ECAL Endcap inner radius: " << GetECALInnerEndcapRadius() << " cm"
                  << std::endl;
        std::cout << "ECAL Endcap outer radius: " << GetECALOuterEndcapRadius() << " cm"
                  << std::endl;
        std::cout << "ECAL Barrel inner symmetry: " << GetECALInnerSymmetry() << std::endl;
        std::cout << "ECAL Barrel polyhedra angle: " << GetECALInnerAngle() * 180 / M_PI << " deg"
                  << std::endl;
        std::cout << "ECAL Barrel polyhedra side length: " << GetECALBarrelSideLength() << " cm"
                  << std::endl;
        std::cout << "ECAL Barrel polyhedra apothem length: " << GetECALBarrelApothemLength()
                  << " cm" << std::endl;
        if (fECALEndcapOutside) {
          std::cout << "ECAL Endcap polyhedra side length: " << GetECALEndcapSideLength() << " cm"
                    << std::endl;
          std::cout << "ECAL Endcap polyhedra apothem length: " << GetECALEndcapApothemLength()
                    << " cm" << std::endl;
        }
        ...
        std::cout << "Distance from TPC to ECAL Endcap: " << GetECALEndcapStartX() - GetTPCOuterX()
                  << " cm" << std::endl;
      }
      ...
      if (this->HasTrackerScDetector()) {
        ...
        std::cout << "ND-GAr Lite Origin (x, y, z) " << GArLiteXCent() << " cm " << GArLiteYCent()
                  << " cm " << GArLiteZCent() << " cm" << std::endl;
        std::cout << "ND-GAr Lite Active Volume Size (R, L) " << GArLiteRadius() << " cm "
                  << GArLiteLength() << " cm" << std::endl;
        std::cout << "Number of planes (from Segmentation alg) " << GetNLayers("TrackerSc")
                  << std::endl;
      }
      ...
      if (this->HasMuonDetector()) {
        ...
        std::cout << "MuID Barrel inner radius: " << GetMuIDInnerBarrelRadius() << " cm"
                  << std::endl;
        std::cout << "MuID Barrel outer radius: " << GetMuIDOuterBarrelRadius() << " cm"
                  << std::endl;
        std::cout << "MuID Barrel inner symmetry: " << GetMuIDInnerSymmetry() << std::endl;
        std::cout << "MuID Barrel polyhedra angle: " << GetMuIDInnerAngle() * 180 / M_PI << " deg"
                  << std::endl;
        std::cout << "MuID Barrel polyhedra side length: " << GetMuIDBarrelSideLength() << " cm"
                  << std::endl;
        std::cout << "MuID Barrel polyhedra apothem length: " << GetMuIDBarrelApothemLength()
                  << " cm" << std::endl;
    ...
    bool GeometryCore::FindActiveTPCVolume()
    {
      ...
      if (fDetectorName == "toad") {
        ...
        if (path.size() == 0) path = this->FindVolumePath("volTPCChamber");
        if (path.size() == 0) return false;
        ...
        if (path.size() == 0) path = this->FindVolumePath("volTPCGas");
        if (path.size() == 0) return false;
        GArTPC_node = path.at(path.size() - 1);
        if (GArTPC_node == nullptr) {
      ...
      if (fDetectorName == "toad") { fTPCNumDriftVols = 1; }
      else {
        fTPCNumDriftVols = 2;
      }
    ...
    bool GeometryCore::FindECALVolume()
    {
      ...
      if (!vol) vol = gGeoManager->FindVolumeFast("volBarrelECal");
      if (!vol) return false;
    ...
    bool GeometryCore::FindECALInnerEndcapRadius()
    {
      ...
      if (!vol) vol = gGeoManager->FindVolumeFast("volEndcapECal");
      if (!vol) return false;
    ...
    bool GeometryCore::FindPVThickness()
    {
      ...
      if (!vol) vol = gGeoManager->FindVolumeFast("volPVBarrel");
      if (!vol) {
        fPVThickness = 0.;
        return false;
      }
    ...
    { // We probably never used the name GArTPC_vol in a gdml file, but just in case
      ...
      if (!vol_tpc) vol_tpc = gGeoManager->FindVolumeFast("GArTPC_vol");
      if (!vol_tpc) {
        fTPCOuterX = 0.;
        return false;
      }
    ...
    bool GeometryCore::FindECALEndcapStartX()
    {
      ...
      if (!vol_e) vol_e = gGeoManager->FindVolumeFast("volEndcapECal");
      if (!vol_e) return false;
    ...
    {
      if (fECALSegmentationAlg) fECALnLayers = fECALSegmentationAlg->nLayers();
    ...
    {
      if (det.compare("ECAL") == 0) return fECALnLayers;

      if (det.compare("TrackerSc") == 0) return fTrackerScnPlanes;

      if (det.compare("MuID") == 0) return fMuIDnLayers;
    ...
    {
      //Barrel
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout] =
        std::shared_ptr<gar::geo::LayeredCalorimeterData>(new LayeredCalorimeterData());

      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout]->layoutType =
        gar::geo::LayeredCalorimeterData::BarrelLayout;
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout]->inner_symmetry =
        0;
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout]->outer_symmetry =
      ...
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout]->extent[0] =
        GetECALInnerBarrelRadius();
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout]->extent[1] =
        GetECALOuterBarrelRadius();
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout]->extent[2] = 0.f;
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout]->extent[3] =
        GetECALEndcapStartX();
      ...
      for (unsigned int i = 0; i < GetNLayers("ECAL"); i++) {
        ...
        TGeoVolume* volLayer = gGeoManager->FindVolumeFast(
          TString::Format("BarrelECal_stave01_module01_layer_%02i_vol", i + 1));
        ...
        if (volLayer) {
          for (int islice = 0; islice < volLayer->GetNdaughters(); islice++) {
            ...
            MF_LOG_DEBUG("GeometryCore::FindECALLayeredCalorimeterData")
              << " Slice " << islice << " RadLen " << nRadiationLengths << " IntLen "
              << nInteractionLengths << " ThicknessSum " << thickness_sum << " Material "
              << mat->GetName();

            if (strcmp(material, "Copper") == 0 || strcmp(material, "Steel") == 0 ||
                strcmp(material, "Lead") == 0) {
          ...
          fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::BarrelLayout]
            ->layers.push_back(caloLayer);
        }
      }

      //Endcap
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout] =
        std::shared_ptr<gar::geo::LayeredCalorimeterData>(new LayeredCalorimeterData());

      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout]->layoutType =
        gar::geo::LayeredCalorimeterData::EndcapLayout;
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout]->inner_symmetry =
        0;
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout]->outer_symmetry =
        GetECALInnerSymmetry();
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout]->phi0 = 0;

      /// extent of the calorimeter in the r-z-plane [ rmin, rmax, zmin, zmax ].
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout]->extent[0] =
        GetECALInnerEndcapRadius();
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout]->extent[1] =
        GetECALOuterEndcapRadius();
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout]->extent[2] =
        GetECALEndcapStartX();
      fECALLayeredCalorimeterData[gar::geo::LayeredCalorimeterData::EndcapLayout]->extent[3] =
        GetECALEndcapOuterX();
      ...
      for (unsigned int i = 0; i < GetNLayers("ECAL"); i++) {
        ...
        TGeoVolume* volLayer = gGeoManager->FindVolumeFast(
          TString::Format("EndcapECal_stave01_module00_layer_%02i_vol", i + 1));
    ...
    bool GeometryCore::FindMuIDInnerBarrelRadius()
    {
      ...
      if (!vol) vol = gGeoManager->FindVolumeFast("volYokeBarrel");
      if (!vol) return false;
    ...
    {
      if (fMuIDSegmentationAlg) fMuIDnLayers = fMuIDSegmentationAlg->nLayers();
    ...
    {
      if (fMinervaSegmentationAlg) fTrackerScnPlanes = fMinervaSegmentationAlg->nPlanes();
