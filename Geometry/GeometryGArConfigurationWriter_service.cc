...
private:
  ...
  {
    return std::make_unique<gar::sumdata::GeometryConfigurationInfo>(info);
  }
...
auto gar::geo::GeometryGArConfigurationWriter::extractInfoFromGeometry() -> InfoPtr_t
{
  ...
  gar::sumdata::GeometryConfigurationInfo confInfo =
    art::ServiceHandle<gar::geo::GeometryGAr>()->configurationInfo();
...
auto gar::geo::GeometryGArConfigurationWriter::readGeometryInformation(art::Run& run) const
  -> InfoPtr_t
{
  auto infoHandle = run.getHandle<gar::sumdata::GeometryConfigurationInfo>(
    art::InputTag{"GeometryGArConfigurationWriter"});
  if (infoHandle) { return makeInfoPtr(*infoHandle); }
  return InfoPtr_t{};
...
// -----------------------------------------------------------------------------
gar::sumdata::RunData const* gar::geo::GeometryGArConfigurationWriter::readRunData(
  art::Run& run) const
{
...
// -----------------------------------------------------------------------------
auto gar::geo::GeometryGArConfigurationWriter::convertRunDataToGeometryInformation(
  gar::sumdata::RunData const& data) -> InfoPtr_t
{
