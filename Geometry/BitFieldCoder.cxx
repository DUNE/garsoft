...
namespace gar {
  namespace geo {
    ...
    BitFieldValue::BitFieldValue(const std::string& fieldName,
      ...
      , _name(fieldName)
    {
      ...
      if (_offset > 63 || _offset + _width > 64) {
        ...
        std::stringstream s;
        s << " BitFieldValue '" << _name << "': out of range -  offset : " << _offset << " width "
          << _width;
    ...
    //----------------------------------------------------------------------------
    void BitFieldValue::set(long64& field, long64 in) const
    {
      ...
      if (in < _minVal || in > _maxVal) {
        ...
        std::stringstream s;
        s << " BitFieldValue '" << _name << "': out of range : " << in << " for width " << _width;
    ...
    void BitFieldCoder::addField(const std::string& name, unsigned offset, int width)
    {
      ...
      _fields.push_back(BitFieldValue(name, offset, width));

      BitFieldValue& bfv = _fields.back();
    ...
    void BitFieldCoder::init(const std::string& initString)
    {
      ...
      for (unsigned i = 0; i < fieldDescriptors.size(); i++) {
        ...
        default:
          ...
          std::stringstream s;
          s << " BitFieldCoder: invalid number of subfields " << fieldDescriptors[i];
