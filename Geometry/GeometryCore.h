  ...
  namespace geo {
    ...
    namespace details {
      ...
      public:
        ...
        static_assert(std::is_base_of<geometry_iterator_base, id_iterator_t>::value,
        ...
        geometry_element_iterator(gar::geo::GeometryCore const* geom) : id_iter(geom) {}
        ...
        operator bool() const { return bool(id_iterator()) && (id_iterator().get() != nullptr); }
      ...
      protected:
        friend bool geo::details::operator==
    ...
    public:
      ...
      IteratorBox(GeometryCore const* geom) : b((geom->*BeginFunc)()), e((geom->*EndFunc)()) {}
    ...
    public:
      ...
      std::map<gar::geo::LayeredCalorimeterData::LayoutType,
               std::shared_ptr<gar::geo::LayeredCalorimeterData>>
      GetECALLayeredCalorimeterData() const
      {
      ...
      void ApplyECALSegmentationAlg(
        std::shared_ptr<gar::geo::seg::SegmentationAlg> pECALSegmentationAlg);

      void ApplyMinervaSegmentationAlg(
        std::shared_ptr<gar::geo::seg::SegmentationAlg> pMinervaSegmentationAlg);

      void ApplyMuIDSegmentationAlg(
        std::shared_ptr<gar::geo::seg::SegmentationAlg> pMuIDSegmentationAlg);
      ...
      gar::raw::CellID_t GetCellID(const TGeoNode* node,
      ...
      double getStripLength(const std::array<double, 3>& point,
      ...
      std::pair<TVector3, TVector3> GetStripEnds(const std::array<double, 3>& point,
                                                 const gar::raw::CellID_t& cID) const;

      std::pair<float, float> CalculateLightPropagation(const std::array<double, 3>& point,
                                                        const std::array<double, 3>& local,
                                                        const gar::raw::CellID_t& cID) const;

      std::array<double, 3> ReconstructStripHitPosition(const std::array<double, 3>& point,
                                                        const std::array<double, 3>& local,
                                                        const float& xlocal,
                                                        const gar::raw::CellID_t& cID) const;
    ...
    private:
      ...
      std::map<std::string, std::vector<const TGeoNode*>>
        fECALNodePath; ///< Stored map of vectors of nodes for the ecal to speedup node searching
      ...
      ECALSegmentationAlgPtr
        fECALSegmentationAlg; ///< Object containing the segmentation for the ECAL

      typedef std::shared_ptr<const gar::geo::seg::SegmentationAlg> MinervaSegmentationAlgPtr;
      MinervaSegmentationAlgPtr
        fMinervaSegmentationAlg; ///< Object containing the segmentation for the Sc Tracker

      typedef std::shared_ptr<const gar::geo::seg::SegmentationAlg> MuIDSegmentationAlgPtr;
      MuIDSegmentationAlgPtr
        fMuIDSegmentationAlg; ///< Object containing the segmentation for the Sc Tracker

      //ECAL layered struct containing all informations about the ECAL layers
      typedef std::map<gar::geo::LayeredCalorimeterData::LayoutType,
                       std::shared_ptr<gar::geo::LayeredCalorimeterData>>
        ECALLayeredCalorimeterData;
