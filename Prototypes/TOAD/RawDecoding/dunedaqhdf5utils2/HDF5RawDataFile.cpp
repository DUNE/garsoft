 ...
 */
    HDF5RawDataFile::HDF5RawDataFile(std::string file_name,
                                     ...
                                     unsigned open_flags)
      : m_bare_file_name(file_name), m_open_flags(open_flags)
    {

      // check and make sure that the file isn't ReadOnly
      if (m_open_flags == HighFive::File::ReadOnly) {
        throw cet::exception("HDF5RawDataFile")
          << " Incompatible Open Flags " << file_name << " " << m_open_flags;
      }
      ...
      try {
        ...
        throw cet::exception("HDF5RawDataFile")
          << " File Open Failed " << filename_to_open << " " << excpt.what();
      }
      ...
      MF_LOG_DEBUG("HDF5RawDataFile.cpp")
        << "Created HDF5 file (" << file_name << ") at time " << file_creation_timestamp << " .";
      ...
      HDF5SourceIDHandler::populate_source_id_geo_id_map(srcid_geoid_map,
                                                         m_file_level_source_id_geo_id_map);
      HDF5SourceIDHandler::store_file_level_geo_id_info(*m_file_ptr,
                                                        m_file_level_source_id_geo_id_map);
 ...
 */
    void HDF5RawDataFile::write(const daqdataformats::TriggerRecord& tr)
    {
      ...
      for (auto const& source_id_path : source_id_path_map) {
        HDF5SourceIDHandler::store_record_header_source_id(record_level_group,
      ...
      HDF5SourceIDHandler::store_record_level_fragment_type_map(record_level_group,
                                                                fragment_type_source_id_map);
      HDF5SourceIDHandler::store_record_level_subdetector_map(record_level_group,
                                                              subdetector_source_id_map);
    }
 ...
 */
    void HDF5RawDataFile::write(const daqdataformats::TimeSlice& ts)
    {
      // the source_id_path map that we will build up as we write the TR header
      // and fragments (and then write the map into the HDF5 TR_record Group)
      HDF5SourceIDHandler::source_id_path_map_t source_id_path_map;
 ...
 */
    HighFive::Group HDF5RawDataFile::write(const daqdataformats::TriggerRecordHeader& trh,
                                           HDF5SourceIDHandler::source_id_path_map_t& path_map)
    {
      ...
      HDF5SourceIDHandler::add_source_id_path_to_map(
        path_map, trh.get_header().element_id, std::get<1>(write_results));
 ...
 */
    HighFive::Group HDF5RawDataFile::write(const daqdataformats::TimeSliceHeader& tsh,
                                           HDF5SourceIDHandler::source_id_path_map_t& path_map)
    {
      std::tuple<size_t, std::string, HighFive::Group> write_results =
      ...
      m_recorded_size += std::get<0>(write_results);
      HDF5SourceIDHandler::add_source_id_path_to_map(
        path_map, tsh.element_id, std::get<1>(write_results));
      return std::get<2>(write_results);
    }
 ...
 */
                                ...
                                HDF5SourceIDHandler::source_id_path_map_t& path_map)
    {
      ...
      HDF5SourceIDHandler::add_source_id_path_to_map(
        path_map, source_id, std::get<1>(write_results));
    }
 ...
 */
    std::tuple<size_t, std::string, HighFive::Group> HDF5RawDataFile::do_write(
      std::vector<std::string> const& group_and_dataset_path_elements,
      ...
      size_t raw_data_size_bytes)
    {
      ...
      if (!m_file_ptr->exist(top_level_group_name)) m_file_ptr->createGroup(top_level_group_name);
      ...
      for (size_t idx = 1; idx < group_and_dataset_path_elements.size() - 1; ++idx) {
        ...
        if (!sub_group.exist(child_group_name)) { sub_group.createGroup(child_group_name); }
      ...
      auto data_set = sub_group.createDataSet<char>(
        dataset_name, data_space, data_set_create_props, data_set_access_props);
      if (data_set.isValid()) {
        ...
        throw cet::exception("HDF5RawDataFile")
          << " Invalid HDF5 Dataset: " << dataset_name << " " << m_file_ptr->getName();
      }
    }
 ...
 */
      ...
      : m_open_flags(HighFive::File::ReadOnly)
    {
      ...
      try {
        ...
        throw cet::exception("HDF5RawDataFile")
          << " File open failure: " << file_name << " " << excpt.what();
      }
    ...
    void HDF5RawDataFile::check_file_layout()
    {
      ...
      if (record_type.compare(m_file_layout_ptr->get_record_name_prefix()) != 0)
        throw cet::exception("HDF5RawDataFile.cpp") << "Bad Record Type: " << record_type << " "
                                                    << m_file_layout_ptr->get_record_name_prefix();
    }
    ...
    void HDF5RawDataFile::check_record_type(std::string rt_name)
    {
      ...
      if (m_file_layout_ptr->get_record_name_prefix().compare(rt_name) != 0)
        throw cet::exception("HDF5RawDataFile.cpp")
          << "Wrong Record Type Requested: " << rt_name << " "
          << m_file_layout_ptr->get_record_name_prefix();
    }

    // HDF5 Utility function to recursively traverse a file
    void HDF5RawDataFile::explore_subgroup(const HighFive::Group& parent_group,
                                           ...
                                           std::vector<std::string>& path_list)
    {
      ...
      for (auto& child_name : childNames) {
        ...
        if (child_type == HighFive::ObjectType::Dataset) { path_list.push_back(full_path); }
    ...
    void HDF5RawDataFile::add_record_level_info_to_caches_if_needed(record_id_t rid)
    {
      ...
      HDF5SourceIDHandler::source_id_geo_id_map_t local_source_id_geo_id_map =
        m_file_level_source_id_geo_id_map;
      ...
      for (auto const& source_id_path : source_id_path_map) {
        ...
        if (source_id_path.first != rh_sid) { fragment_source_id_set.insert(source_id_path.first); }
 ...
 */
    ...
    {
      if (top_level_group_name.empty()) top_level_group_name = m_file_ptr->getPath();
      ...
      if (!parent_group.isValid())
        throw cet::exception("HDF5RawDataFile.cpp")
          << "Invalid HDF5 Group: " << top_level_group_name;
 ...
 */
    ...
    {
      if (!m_all_record_ids_in_file.empty()) return m_all_record_ids_in_file;
      ...
      for (auto const& name : childNames) {
        ...
        if (loc == std::string::npos) {
          ...
          m_all_record_ids_in_file.insert(
            std::make_pair(std::stoi(rec_num_string), std::stoi(seq_num_string)));
        }
    ...
    std::set<uint64_t> HDF5RawDataFile::get_all_record_numbers() // NOLINT(build/unsigned)
    {
      MF_LOG_WARNING("HDF5RawDataFile.cpp")
        << "Deprecated usage, get_all_record_numbers().  Use get_all_record_ids(),  which returns "
           "a record_number,sequence_number pair.";
    ...
    HDF5RawDataFile::record_id_set HDF5RawDataFile::get_all_trigger_record_ids()
    {
    ...
    {
      MF_LOG_WARNING("HDF5RawDataFile.cpp")
        << "Deprecated usage, get_all_trigger_record_numbers().  Use get_all_trigger_record_ids(), "
           " which returns a record_number,sequence_number pair.";

      return get_all_record_numbers();
    }

    HDF5RawDataFile::record_id_set HDF5RawDataFile::get_all_timeslice_ids()
    {
      check_record_type("TimeSlice");
      return get_all_record_ids();
    }

    std::set<daqdataformats::timeslice_number_t> HDF5RawDataFile::get_all_timeslice_numbers()
    {
      check_record_type("TimeSlice");
      return get_all_record_numbers();
    }
 ...
 */
    std::vector<std::string> HDF5RawDataFile::get_record_header_dataset_paths()
    {
    ...
    std::vector<std::string> HDF5RawDataFile::get_trigger_record_header_dataset_paths()
    {
      check_record_type("TriggerRecord");
      return get_record_header_dataset_paths();
    }

    std::vector<std::string> HDF5RawDataFile::get_timeslice_header_dataset_paths()
    {
      check_record_type("TimeSlice");
      return get_record_header_dataset_paths();
    }

    std::string HDF5RawDataFile::get_record_header_dataset_path(const record_id_t& rid)
    {
      ...
      if (rec_id == allrecids.end())
        throw cet::exception("HDF5RawDataFile.cpp")
          << "Record ID Not Found: " << rid.first << " " << rid.second;

      if (get_version() <= 2) {
        return (m_file_ptr->getPath() +
                m_file_layout_ptr->get_record_header_path(rid.first, rid.second));
      }
      else {
    ...
    std::string HDF5RawDataFile::get_record_header_dataset_path(
      const uint64_t rec_num, // NOLINT (build/unsigned)
      const daqdataformats::sequence_number_t seq_num)
    {
      return get_record_header_dataset_path(std::make_pair(rec_num, seq_num));
    }

    std::string HDF5RawDataFile::get_trigger_record_header_dataset_path(const record_id_t& rid)
    {
      check_record_type("TriggerRecord");
      return get_record_header_dataset_path(rid);
    }

    std::string HDF5RawDataFile::get_trigger_record_header_dataset_path(
      const daqdataformats::trigger_number_t trig_num,
      const daqdataformats::sequence_number_t seq_num)
    {
      check_record_type("TriggerRecord");
    ...
    std::string HDF5RawDataFile::get_timeslice_header_dataset_path(const record_id_t& rid)
    {
      check_record_type("TimeSlice");
      return get_record_header_dataset_path(rid.first, 0);
    }

    std::string HDF5RawDataFile::get_timeslice_header_dataset_path(
      const daqdataformats::timeslice_number_t ts_num)
    {
      check_record_type("TimeSlice");
 ...
 */
    std::vector<std::string> HDF5RawDataFile::get_all_fragment_dataset_paths()
    {
    ...
    std::vector<std::string> HDF5RawDataFile::get_fragment_dataset_paths(const record_id_t& rid)
    {
      auto allrecids = get_all_record_ids();
      auto rec_id = allrecids.find(rid);
      if (rec_id == allrecids.end())
    ...
    // get all fragment dataset paths for given record ID
    std::vector<std::string> HDF5RawDataFile::get_fragment_dataset_paths(
      const uint64_t rec_num, // NOLINT (build/unsigned)
      const daqdataformats::sequence_number_t seq_num)
    {
      ...
      const daqdataformats::SourceID::Subsystem subsystem)
    {
      ...
      for (auto const& rid : get_all_record_ids()) {
        if (get_version() <= 2) {
          auto datasets =
            get_dataset_paths(m_file_ptr->getPath() + m_file_layout_ptr->get_fragment_type_path(
                                                        rid.first, rid.second, subsystem));
          frag_paths.insert(frag_paths.end(), datasets.begin(), datasets.end());
        }
        else {
          std::set<daqdataformats::SourceID> source_id_list =
            get_source_ids_for_subsystem(rid, subsystem);
          for (auto const& source_id : source_id_list) {
            frag_paths.push_back(m_source_id_path_cache[rid][source_id]);
          }
        }
      ...
      const std::string& subsystem_name)
    {
      daqdataformats::SourceID::Subsystem subsystem =
        daqdataformats::SourceID::string_to_subsystem(subsystem_name);
      ...
      const daqdataformats::SourceID::Subsystem subsystem)
    {
      ...
      if (get_version() <= 2) {
        return get_dataset_paths(m_file_ptr->getPath() + m_file_layout_ptr->get_fragment_type_path(
                                                           rid.first, rid.second, subsystem));
      }
      else {
        std::vector<std::string> frag_paths;
...
HDF5RawDataFile::get_source_ids(std::vector<std::string> const& frag_dataset_paths)
{
  ...
  return source_ids;
                                                 ...
                                                 const detdataformats::DetID::Subdetector subdet)
    {
      ...
      for (auto const& map_entry : m_source_id_geo_id_cache[rid]) {
        for (auto const& geo_id : map_entry.second) {
          ...
          if (geoinfodetid == static_cast<uint16_t>(subdet)) { set_of_geo_ids.insert(geo_id); }
        }
      }
      return set_of_geo_ids;
    }
    ...
    std::set<daqdataformats::SourceID> HDF5RawDataFile::get_source_ids(const record_id_t& rid)
    {
      auto allrecids = get_all_record_ids();
      auto rec_id = allrecids.find(rid);
      if (rec_id == allrecids.end())
    ...
    daqdataformats::SourceID HDF5RawDataFile::get_record_header_source_id(const record_id_t& rid)
    {
      auto allrecids = get_all_record_ids();
      auto rec_id = allrecids.find(rid);
      if (rec_id == allrecids.end())
    ...
    std::set<daqdataformats::SourceID> HDF5RawDataFile::get_fragment_source_ids(
      const record_id_t& rid)
    {
      auto allrecids = get_all_record_ids();
      auto rec_id = allrecids.find(rid);
    ...
    std::set<daqdataformats::SourceID> HDF5RawDataFile::get_source_ids_for_subsystem(
      const record_id_t& rid,
      const daqdataformats::SourceID::Subsystem subsystem)
    {
      auto allrecids = get_all_record_ids();
    ...
    std::set<daqdataformats::SourceID> HDF5RawDataFile::get_source_ids_for_fragment_type(
      const record_id_t& rid,
    ...
    std::set<daqdataformats::SourceID> HDF5RawDataFile::get_source_ids_for_subdetector(
      const record_id_t& rid,
      const detdataformats::DetID::Subdetector subdet)
    {
      auto allrecids = get_all_record_ids();
    ...
    std::unique_ptr<char[]> HDF5RawDataFile::get_dataset_raw_data(const std::string& dataset_path)
    {
      ...
      if (!data_set.isValid())
        throw cet::exception("HDF5RawDataFile.cpp")
          << "Invalid HDF5 Dataset: " << dataset_path << " " << get_file_name();
    ...
    std::unique_ptr<daqdataformats::Fragment> HDF5RawDataFile::get_frag_ptr(
      ...
      const daqdataformats::SourceID& source_id)
    {
      if (get_version() < 2)
        throw cet::exception("HDF5RawDataFile.cpp")
          << "Incompatible File Layout Version: " << get_version() << " 2 "
    ...
    std::unique_ptr<daqdataformats::Fragment> HDF5RawDataFile::get_frag_ptr(
      ...
      const uint32_t id) // NOLINT(build/unsigned)
    {
      daqdataformats::SourceID source_id(daqdataformats::SourceID::string_to_subsystem(typestring),
                                         id);
      return get_frag_ptr(rid, source_id);
    }
    ...
    std::unique_ptr<daqdataformats::TriggerRecordHeader> HDF5RawDataFile::get_trh_ptr(
      const std::string& dataset_name)
    {
      auto membuffer = get_dataset_raw_data(dataset_name);
      auto trh_ptr =
        std::make_unique<daqdataformats::TriggerRecordHeader>(membuffer.release(), true);
      return trh_ptr;
    }
    ...
    std::unique_ptr<daqdataformats::TimeSliceHeader> HDF5RawDataFile::get_tsh_ptr(
      const std::string& dataset_name)
    {
      auto membuffer = get_dataset_raw_data(dataset_name);
    ...
    daqdataformats::TriggerRecord HDF5RawDataFile::get_trigger_record(const record_id_t& rid)
    {
    ...
    daqdataformats::TimeSlice HDF5RawDataFile::get_timeslice(
      const daqdataformats::timeslice_number_t ts_num)
    {
    ...
    HDF5RawDataFile::get_geo_ids_for_source_id(const record_id_t& rid,
                                               const daqdataformats::SourceID& source_id)
    {
      auto allrecids = get_all_record_ids();
      auto rec_id = allrecids.find(rid);
    ...
    daqdataformats::SourceID HDF5RawDataFile::get_source_id_for_geo_id(
      const record_id_t& rid,
      const uint64_t requested_geo_id) // NOLINT(build/unsigned)
    {
      ...
      for (auto const& map_entry : m_source_id_geo_id_cache[rid]) {
        ...
        for (auto const& geoid_from_list : geoid_list) {
          if (geoid_from_list == requested_geo_id) { return map_entry.first; }
        }
      }
