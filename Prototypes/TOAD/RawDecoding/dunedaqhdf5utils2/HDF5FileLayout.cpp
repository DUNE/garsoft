...
namespace dunedaq {
  namespace hdf5libs {
    ...
    HDF5FileLayout::HDF5FileLayout(hdf5filelayout::FileLayoutParams conf,
                                   uint32_t version) // NOLINT(build/unsigned)
      : m_conf_params(conf), m_version(version)
    {
      if (m_version < 2) m_conf_params = get_v0_file_layout_params();
    ...
    void HDF5FileLayout::check_config()
    {
      ...
      if (m_conf_params.record_name_prefix.compare("TriggerRecord") == 0) {
        ...
        if (m_conf_params.digits_for_sequence_number != 0) {
          MF_LOG_WARNING("HDF5FileLayout.cpp")
            << " Invalid Sequence Digits " << m_conf_params.record_name_prefix << " 0";
          m_conf_params.digits_for_sequence_number = 0;
        }
      }
      else {
        throw cet::exception("HDF5FileLayout.cpp")
          << " Invalid Record Name " << m_conf_params.record_name_prefix;
      }
    }

    hdf5filelayout::PathParams HDF5FileLayout::get_path_params(
      daqdataformats::SourceID::Subsystem type) const
    {
      try {
        ...
        throw cet::exception("HDF5FileLayout.cpp")
          << " FileLayoutUnconfiguredSubsystem " << type << " "
          << daqdataformats::SourceID::subsystem_to_string(type);
      }
    }

    std::string HDF5FileLayout::get_record_number_string(
      uint64_t record_number, // NOLINT(build/unsigned)
      daqdataformats::sequence_number_t seq_num) const
    {
      ...
      if (record_number >= m_powers_ten[m_conf_params.digits_for_record_number]) {
        MF_LOG_WARNING("HDF5FileLayout.cpp")
          << " File Layout Not Enough Digits For Path " << record_number << " "
          << m_conf_params.digits_for_record_number;
        width = 0; // tells it to revert to normal width
      }

      record_number_string << m_conf_params.record_name_prefix << std::setw(width)
      ...
      if (m_conf_params.digits_for_sequence_number > 0) {
        ...
        if (seq_num >= m_powers_ten[m_conf_params.digits_for_sequence_number]) {
          MF_LOG_WARNING("HDF5FileLayout.cpp")
            << " File Layout Not Enough Digits For Path " << seq_num << " "
            << m_conf_params.digits_for_sequence_number;
          width = 0; // tells it to revert to normal width
        }
    ...
    std::string HDF5FileLayout::get_trigger_number_string(
      daqdataformats::trigger_number_t trig_num,
      daqdataformats::sequence_number_t seq_num) const
    {
      return get_record_number_string(trig_num, seq_num);
    }

    std::string HDF5FileLayout::get_timeslice_number_string(
      daqdataformats::timeslice_number_t ts_num) const
    {
 ...
 */
    std::vector<std::string> HDF5FileLayout::get_path_elements(
      const daqdataformats::TriggerRecordHeader& trh) const
    {
      ...
      path_elements.push_back(
        get_trigger_number_string(trh.get_trigger_number(), trh.get_sequence_number()));
 ...
 */
    std::vector<std::string> HDF5FileLayout::get_path_elements(
      const daqdataformats::TimeSliceHeader& tsh) const
    {
      ...
      // then the SourceID plus record header name
      path_elements.push_back(tsh.element_id.to_string() + "_" +
                              m_conf_params.record_header_dataset_name);
 ...
 */
    std::vector<std::string> HDF5FileLayout::get_path_elements(
      const daqdataformats::FragmentHeader& fh) const
    {
      ...
      path_elements.push_back(fh.element_id.to_string() + "_" +
                              daqdataformats::fragment_type_to_string(
                                static_cast<daqdataformats::FragmentType>(fh.fragment_type)));
 ...
 */
    std::string HDF5FileLayout::get_record_header_path(
      uint64_t rec_num, // NOLINT (build/unsigned)
      daqdataformats::sequence_number_t seq_num) const
    {
 ...
 */
    std::string HDF5FileLayout::get_trigger_record_header_path(
      daqdataformats::trigger_number_t trig_num,
      daqdataformats::sequence_number_t seq_num) const
    {
 ...
 */
    std::string HDF5FileLayout::get_timeslice_header_path(
      daqdataformats::timeslice_number_t ts_num) const
    {
 ...
 */
                                                  ...
                                                  daqdataformats::SourceID element_id) const
    {
      ...
      path_string << get_trigger_number_string(trig_num, seq_num) << "/"
                  << path_params.detector_group_name << "/" << path_params.element_name_prefix
                  << std::setw(path_params.digits_for_element_number) << std::setfill('0')
                  << element_id.id;
 ...
 */
    std::string HDF5FileLayout::get_fragment_path(
      uint64_t trig_num, // NOLINT(build/unsigned)
      daqdataformats::sequence_number_t seq_num,
 ...
 */
    std::string HDF5FileLayout::get_fragment_path(
      ...
      uint32_t element_id) const // NOLINT(build/unsigned)
    {
      daqdataformats::SourceID sid{daqdataformats::SourceID::string_to_subsystem(typestring),
                                   element_id};
      return get_fragment_path(trig_num, seq_num, sid);
    }
 ...
 */
    std::string HDF5FileLayout::get_fragment_type_path(
      uint64_t trig_num, // NOLINT(build/unsigned)
      daqdataformats::sequence_number_t seq_num,
      daqdataformats::SourceID::Subsystem type) const
    {
 ...
 */
    ...
    {
      return get_fragment_type_path(
        trig_num, seq_num, daqdataformats::SourceID::string_to_subsystem(typestring));
    }

    daqdataformats::SourceID HDF5FileLayout::get_source_id_from_path_elements(
      std::vector<std::string> const& path_elements) const
    {
      // ignore first path element, which is for the record group
      // second path element is detector name.
      daqdataformats::SourceID::Subsystem systype =
        m_detector_group_name_to_type_map.at(path_elements[1]);
    ...
    void HDF5FileLayout::fill_path_params_maps(hdf5filelayout::FileLayoutParams const& flp)
    {
      for (auto const& path_param : flp.path_param_list) {
        auto sys_type =
          daqdataformats::SourceID::string_to_subsystem(path_param.detector_group_type);

        if (sys_type == daqdataformats::SourceID::Subsystem::kUnknown)
          throw cet::exception("HDF5FileLayout.cpp")
            << " File Layout Invalid Subsystem " << path_param.detector_group_type;
