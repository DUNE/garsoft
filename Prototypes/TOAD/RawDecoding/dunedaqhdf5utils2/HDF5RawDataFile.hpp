 ...
 */
    ...
    public:
      enum { TLVL_BASIC = 2, TLVL_FILE_SIZE = 5 };

      // define a record number type
      // that is a pair of the trigger record or timeslice number and sequence number
      typedef std::pair<uint64_t, daqdataformats::sequence_number_t>
        record_id_t; // NOLINT(build/unsigned)
    ...
    public:
      ...
      std::string get_record_header_dataset_path(
      ...
      std::string get_trigger_record_header_dataset_path(
        const daqdataformats::trigger_number_t trig_num,
        const daqdataformats::sequence_number_t seq_num = 0);
      std::string get_timeslice_header_dataset_path(const record_id_t& rid);
      std::string get_timeslice_header_dataset_path(
        const daqdataformats::timeslice_number_t trig_num);
      ...
      std::vector<std::string> get_fragment_dataset_paths(
        const uint64_t rec_num, // NOLINT (build/unsigned)
        const daqdataformats::sequence_number_t seq_num = 0);
  ...
  std::set<daqdataformats::SourceID> get_source_ids(std::vector<std::string> const& frag_dataset_paths);
      ...
      std::set<uint64_t> get_geo_ids_for_subdetector(
        const record_id_t& rid, // NOLINT(build/unsigned)
      ...
      {
        detdataformats::DetID::Subdetector subdet =
          detdataformats::DetID::string_to_subdetector(subdet_name);
      ...
      std::set<daqdataformats::SourceID> get_source_ids(
        const uint64_t rec_num, // NOLINT(build/unsigned)
        const daqdataformats::sequence_number_t seq_num = 0)
      {
      ...
      daqdataformats::SourceID get_record_header_source_id(
        const uint64_t rec_num, // NOLINT(build/unsigned)
        const daqdataformats::sequence_number_t seq_num = 0)
      {
      ...
      std::set<daqdataformats::SourceID> get_fragment_source_ids(
        const uint64_t rec_num, // NOLINT(build/unsigned)
        const daqdataformats::sequence_number_t seq_num = 0)
      {
      ...
      std::set<daqdataformats::SourceID> get_source_ids_for_subsystem(
        const record_id_t& rid,
        const daqdataformats::SourceID::Subsystem subsystem);
      std::set<daqdataformats::SourceID> get_source_ids_for_subsystem(
        const record_id_t& rid,
      ...
      std::set<daqdataformats::SourceID> get_source_ids_for_fragment_type(
        const record_id_t& rid,
      ...
      {
        daqdataformats::FragmentType frag_type =
          daqdataformats::string_to_fragment_type(frag_type_name);
      ...
      std::set<daqdataformats::SourceID> get_source_ids_for_subdetector(
        const record_id_t& rid,
        const detdataformats::DetID::Subdetector subdet);
      std::set<daqdataformats::SourceID> get_source_ids_for_subdetector(
        const record_id_t& rid,
  ...
  std::set<daqdataformats::SourceID> get_source_ids(const std::string& typestring)
  {
    return get_source_ids(get_fragment_dataset_paths(typestring));
      ...
      std::unique_ptr<daqdataformats::TriggerRecordHeader> get_trh_ptr(
        const std::string& dataset_name);
      std::unique_ptr<daqdataformats::TimeSliceHeader> get_tsh_ptr(const std::string& dataset_name);

      std::unique_ptr<daqdataformats::Fragment> get_frag_ptr(
        const record_id_t& rid,
      ...
      std::unique_ptr<daqdataformats::TimeSliceHeader> get_tsh_ptr(
      ...
      daqdataformats::TriggerRecord get_trigger_record(
        const daqdataformats::trigger_number_t trig_num,
        const daqdataformats::sequence_number_t seq_num = 0)
      {
      ...
      daqdataformats::TimeSlice get_timeslice(const record_id_t& rid)
      {
        return get_timeslice(rid.first);
      }

      std::vector<uint64_t> get_geo_ids_for_source_id(
        const record_id_t& rid, // NOLINT(build/unsigned)
        const daqdataformats::SourceID& source_id);

      daqdataformats::SourceID get_source_id_for_geo_id(
        const record_id_t& rid,
        const uint64_t geo_id); // NOLINT(build/unsigned)
    ...
    private:
      ...
      std::map<record_id_t, HDF5SourceIDHandler::subsystem_source_id_map_t>
        m_subsystem_source_id_cache;
      std::map<record_id_t, HDF5SourceIDHandler::fragment_type_source_id_map_t>
        m_fragment_type_source_id_cache;
      std::map<record_id_t, HDF5SourceIDHandler::subdetector_source_id_map_t>
        m_subdetector_source_id_cache;
    };
