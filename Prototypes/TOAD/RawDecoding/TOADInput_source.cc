...
gar::TOADInputDetail::TOADInputDetail(fhicl::ParameterSet const& ps,
...
void gar::TOADInputDetail::readFile(std::string const& filename, art::FileBlock*& fb)
{
  ...
  MF_LOG_INFO("HDF5") << "HDF5 opened HDF file with run number " << run_number << " and "
                      << fUnprocessedEventRecordIDs.size() << " events";
  if (fLogLevel > 0) {
    for (const auto& e : fUnprocessedEventRecordIDs)
      MF_LOG_INFO("HDF5") << e.first << " " << e.second;
  }
                                    ...
                                    art::EventPrincipal*& outE)
{
  ...
  if (run_id != run_id2) {
    throw cet::exception("TOADInput_source.cc")
      << " Inconsistent run IDs in file attribute and trigger record header " << run_id << " "
      << run_id2;
  }
  ...
  if (nextEventRecordID.first != trhtn) {
    throw cet::exception("TOADInput_source.cc")
      << " Inconsistent trigger record numbers: " << nextEventRecordID.first << " " << trhtn;
  }
  ...
  if (fLogLevel > 0) { std::cout << "TOADInput_source: getTrigTime: " << getTrigTime << std::endl; }
  ...
  if (inR == 0 || inR->run() != run_id) { outR = pmaker.makeRunPrincipal(run_id, artTrigStamp); }
  ...
  if (fHandleSequenceOption == "increment") {
    event = fLastEvent + 1;
    if (fLogLevel > 0) {
      std::cout << "TOADInput_source: assigning event number via incrment: " << event
                << " orig: " << nextEventRecordID.first << " " << nextEventRecordID.second;
    }
  }
  else if (fHandleSequenceOption == "shiftadd") {
    event = event * fTrnScale + sequenceno;
    if (fLogLevel > 0) {
      std::cout << "TOADInput_source: assigning event number via shfitadd: " << event
                << " orig: " << nextEventRecordID.first << " " << nextEventRecordID.second;
    }
  }
  else if (fHandleSequenceOption == "ignore") {
    if (fLogLevel > 0) {
      std::cout << "TOADInput_source: assigning event number ignoring seqID: " << event
                << " orig: " << nextEventRecordID.first << " " << nextEventRecordID.second;
    }
  }
  else {
    throw cet::exception("TOADInput_source.cc")
      << "Ununderstood HandleSequenceOption: " << fHandleSequenceOption
      << " use: ignore, increment or shiftadd";
  }
  fLastEvent = event;
  ...
  std::unique_ptr<std::vector<gar::raw::RawDigit>> rdcol =
    std::make_unique<std::vector<gar::raw::RawDigit>>();
  ...
  for (const auto& source_id : sourceids) {
    ...
    for (const auto& gid : gids) {
      ...
      if (fLogLevel > 1) {
        ...
        std::cout << "TOADInputSource looking for subdet: "
                  << "ND_GAr" << std::endl;
      }
      ...
      if (subdetector_string == "ND_GAr") {
        ...
        while (frag_interval < (int)(frag_size)) {
          ...
          auto toad_frame = reinterpret_cast<dunedaq::detdataformats::toad::TOADFrameOverlay*>(
            static_cast<char*>(fdp));
          ...
          if (adc_vector[0] == 0) { continue; }

          if (frag_interval == 0) { t0 = (ULong64_t)(toad_frame->tstmp); }
