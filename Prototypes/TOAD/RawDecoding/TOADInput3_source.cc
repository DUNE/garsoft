...
gar::TOADInput3Detail::TOADInput3Detail(fhicl::ParameterSet const& ps,
...
void gar::TOADInput3Detail::readFile(std::string const& filename, art::FileBlock*& fb)
{
                                     ...
                                     art::EventPrincipal*& outE)
{
  ...
  if (run_id != run_id2) {
    throw cet::exception("TOADInput3_source.cc")
  ...
  if (fHandleSequenceOption == "increment") {
    event = fLastEvent + 1;
    if (fLogLevel > 0) {
      std::cout << "TOADInput3_source: assigning event number via incrment: " << event
    ...
    if (fLogLevel > 0) {
      std::cout << "TOADInput3_source: assigning event number via shfitadd: " << event
                << " orig: " << nextEventRecordID.first << " " << nextEventRecordID.second;
    }
  }
  else if (fHandleSequenceOption == "ignore") {
    if (fLogLevel > 0) {
      std::cout << "TOADInput3_source: assigning event number ignoring seqID: " << event
                << " orig: " << nextEventRecordID.first << " " << nextEventRecordID.second;
    }
  }
  else {
  ...
  for (const auto& source_id : sourceids) {
    ...
    for (const auto& gid : gids) {
      ...
      if (fLogLevel > 1) {
        ...
        std::cout << "TOADInput3Source looking for subdet: "
                  << "ND_GAr" << std::endl;
      }
